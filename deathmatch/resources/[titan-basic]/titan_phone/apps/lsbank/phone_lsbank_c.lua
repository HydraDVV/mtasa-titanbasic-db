	...
	if state == "left" then
		...
		elseif source == BankGUIs[7] then
			...
			if not amount or amount <= 0 or math.ceil( amount ) ~= amount then
				outputChatBox("#575757"..exports["titan_pool"]:getServerName()..": #ffffffLütfen pozitif değer giriniz.", 255, 0, 0, true)
			elseif (amount>money) then
				outputChatBox("#575757"..exports["titan_pool"]:getServerName()..": #ffffffBanka hesabınızda yeterli para yok.", 255, 0, 0, true)
			elseif playername == "" then
				outputChatBox("#575757"..exports["titan_pool"]:getServerName()..": #ffffffLütfen parayı göndermek istediğiniz kişinin adını tam girin.", 255, 0, 0, true)
			else
