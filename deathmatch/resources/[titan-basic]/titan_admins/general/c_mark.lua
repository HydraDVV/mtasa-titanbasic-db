...
addCommandHandler( "mark",
	function( commandName, ... )
		if exports.titan_integration:isPlayerDeveloper(localPlayer) then
			...
			-- success message
			outputChatBox( "[-]#f9f9f9" .. ( name and "'" .. name .. "' " or "" ) .. " isimli yer bildirimi oluşturumunuz başarıyla kaydedildi.", 184, 255, 112, true )
			
			else
					outputChatBox( "[-]#f9f9f9 Bu komutu politikalar gereğince artık yetkililer kullanamayacak.", 255, 61, 61,true )			
		end
	end
)
		...
		if exports.titan_integration:isPlayerDeveloper(localPlayer) then
			...
			if not name then
				if default then
					...
					outputChatBox( "[-]#f9f9f9 Öncelikle bir mark kaydetmelisiniz.", 255, 61, 61,true )
				end
			else
				if marks[ name ] then
					triggerServerEvent( "gotoMark", localPlayer, unpack( marks[ name ] ) )
				else
					outputChatBox( "[-]#f9f9f9 İlk olarak bir mark kaydetmelisiniz, /mark.", 255, 61, 61,true )
				end
			end
			else
					outputChatBox( "[-]#f9f9f9 Bu komutu politikalar gereğince artık yetkililer kullanamayacak.", 255, 61, 61,true )			
	...
	function( commandName, ... )
		if exports.titan_integration:isPlayerDeveloper(localPlayer) then
			...
			if not name then
				if default then
					default = false
					outputChatBox( "[-]#f9f9f9 Ana yer bildiriminiz başarıyla silindi.", 184, 255, 112, true )
					saveMarks( )
				else
					outputChatBox( "[-]#f9f9f9 İlk olarak her hangi bir yere /mark kaydetmelisiniz.", 255, 74, 74,true )
				end
			else
				if marks[ name ] then
					marks[ name ] = nil
