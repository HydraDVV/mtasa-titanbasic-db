...
function banAPlayer(thePlayer, commandName, targetPlayer, hours, ...)
	if exports["titan_integration"]:isPlayerSeniorAdmin(thePlayer) then
		if not (targetPlayer) or not (hours) or not tonumber(hours) or tonumber(hours)<0 or not (...) then
			outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffff /" .. commandName .. " [Oyuncu İsmi / ID] [0 = Sınırsız, Saat girin] [Açıklama]", thePlayer, 0, 55, 255, true)
		else
			...
			elseif (hours>168) then
				...
				if (targetPlayerPower <= thePlayerPower) then -- Check the admin isn't banning someone higher rank them him
					...
					exports['titan_admins']:addAdminHistory(targetPlayer, thePlayer, reason, 2 , rhours)
					local banId = nil
					if (seconds == 0) then
					...
					adminTitle = exports.titan_global:getAdminTitle1(thePlayer)
					if (hiddenAdmin==1) then
						adminTitle = "Gizli Yetkili"
					end

					if string.lower(commandName) == "sban" then
						exports.titan_global:sendMessageToAdmins("[SESSIZ BAN] " .. adminTitle .. " isimli yetkili " .. targetPlayerName .. " isimli oyuncuyu yasakladı. (" .. hours .. ")")
						exports.titan_global:sendMessageToAdmins("[SESSIZ BAN] Açıklama: " .. reason .. ".")
					...
					exports.titan_global:sendMessageToAdmins("/showban yazarak detaylara bakabilirsiniz.")
				else
					outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffffYetkisi sizden yüksek.", thePlayer, 255, 0, 0, true)
				end
			end
		end
	end
...
function karakterBanla(thePlayer, commandName, targetPlayer, ...)
	...
	local message = nil
	if(logged==1) and (exports.titan_integration:isPlayerSeniorAdmin(thePlayer)) then
		if not (...) then
			outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffff/cban [OyuncuID/Isim] [Mesaj]", thePlayer, 0, 125, 255, true)
		else
			local targetPlayer, targetPlayerName = exports.titan_global:findPlayerByPartialNick(thePlayer, targetPlayer)
			if targetPlayer then
				...
				exports['titan_admins']:addAdminHistory(targetPlayer, thePlayer, message.." | Sınırsız. (( "..targetPlayerName.." ))", 2 , 0)
				...
				exports.titan_global:updateTableCache("characters", getElementData(targetPlayer, "dbid"), {["active"] = 0, ["activeDescription"] = message})
...
addEventHandler("selfckPlayer", root,
	function(targetPlayer)
		...
		local saat = time.hour
		local saniye = time.minute
		exports['titan_admins']:addAdminHistory(targetPlayer, targetPlayer, message.." | Sınırsız. (( "..targetPlayerName.." ))", 2 , 0)
...
addCommandHandler("unsban",
	function(thePlayer, commandName, username)
		if exports.titan_integration:isPlayerSeniorAdmin(thePlayer) then
			...
			local row = exports.titan_global:getAccountDetails(username)
			if row then
...
function offlineBanAPlayer(thePlayer, commandName, targetUsername, hours, ...)
	if exports.titan_integration:isPlayerSeniorAdmin(thePlayer) then
		if not (targetUsername) or not (hours) or not tonumber(hours) or (tonumber(hours)<0) or not (...) then
			...
			local user = exports.titan_global:getCache("accounts", targetUsername, "username")
			if user and user['id'] and tonumber(user['id']) then
				...
				if (tonumber(user['admin']) > thePlayerPower) then
					...
					exports.titan_global:sendMessageToAdmins("AdmWrn: "..adminTitle.." attempted to execute the ban command on higher admin '"..targetUsername.."'.")
					return false
				end
				...
				reason = reason .. " (" .. hours .. ")"
				exports['titan_admins']:addAdminHistory(user['id'], thePlayer, reason, 2, rhours)
				...
				local adminUsername = getElementData(thePlayer, "player.username")
				local adminUserID = getElementData(thePlayer, "player.dbid")
				adminTitle = exports.titan_global:getPlayerAdminTitle(thePlayer)
				...
				if string.lower(commandName) == "soban" then
					exports.titan_global:sendMessageToAdmins("[ÇEVRİMDIŞI-BAN]: " .. adminTitle .. " " .. adminUsername .. " isimli yetkili " .. targetUsername .. " kullanici adli oyuncuyu yasakladi. (" .. hours .. ")")
					exports.titan_global:sendMessageToAdmins("[ÇEVRİMDIŞI-BAN]: Açıklama: " .. reason .. ".")
				else
				...
				outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffffKullanıcı adı tespit edilemedi.", thePlayer, 255, 0, 0, true)
				return false
			end
		end
	end
	...
	if (exports.titan_integration:isPlayerSeniorAdmin(thePlayer)) then
		if not serial or not string.len(serial) or not string.len(serial) == 32 or not (...) then
			...
			if id and tonumber(id) then
				...
				exports.titan_global:sendMessageToAdmins("[BAN] "..exports.titan_global:getPlayerFullIdentity(thePlayer).." has banned serial number '"..serial.."' permanently for '"..reason.."'. /showban for details.")
	...
	if (exports.titan_integration:isPlayerSeniorAdmin(thePlayer)) then
		if not ip or not string.len(ip) or string.len(ip) > 15 or not (...) then
			...
			if id and tonumber(id) then
				...
				exports.titan_global:sendMessageToAdmins("[BAN] "..exports.titan_global:getPlayerFullIdentity(thePlayer).." has banned IP Address '"..ip.."' permanently for '"..reason.."'. /showban for details.")
...
function banPlayerAccount(thePlayer, commandName, account, ...)
	if (exports.titan_integration:isPlayerSeniorAdmin(thePlayer)) then
		if not account or not (...) then
			...
			local account = exports.titan_global:getCache("accounts", account, "username")
			...
			if id and tonumber(id) then
				...
				exports.titan_global:sendMessageToAdmins("[BAN] "..exports.titan_global:getPlayerFullIdentity(thePlayer).." has banned account '"..(account.username).."' permanently for '"..reason.."'. /showban for details.")
					...
					if ban and ban['id'] and tonumber(ban['id']) then
						...
						if dbExec(mysql:getConnection(), "DELETE FROM bans WHERE id='"..id.."'") then
							...
							if ban['account'] ~= nil then
								exports['titan_admins']:addAdminHistory(ban['account'], thePlayer, "UNBAN", 2 , 0)
							end
							local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
							exports.titan_global:sendMessageToAdmins("[UNBAN] "..exports.titan_global:getPlayerFullIdentity(thePlayer).." has removed ban record #"..ban['id']..". /showban for details.")
						end
					end
				end
			end
...
function showBanDetails(thePlayer, commandName, clue)
	if exports.titan_integration:isPlayerSeniorAdmin(thePlayer) then
		if clue then
			clue = (clue)
			local bans = {}
			if tostring(clue) and exports.titan_global:getCache("accounts", clue, "username", true) then
				clue = exports.titan_global:getCache("accounts", clue, "username", true).id
			end
...
function printBanInfo(thePlayer, result)
	...
	local bannedAccount = exports.titan_cache:getUsernameFromId(result['account'])
	...
	local banningAdmin = exports.titan_cache:getUsernameFromId(result['admin'])
    ...
    dbQuery(
        function(qh, source)
            ...
            if rows > 0 then
                ...
                exports.titan_global:sendMessageToAdmins("[BAN] "..(bannedSerial and (" serial: '"..tostring(bannedSerial).."'") or "" ).." "..(bannedIp and (" IP: '"..tostring(bannedIp).."'") or "").." oyuna girmeye çalıştı, /showban komutuyla detayları öğrenin.")
                return true
            end
        end,
