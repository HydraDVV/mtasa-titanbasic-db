...
function deleteUniqueHandling(theVehicle, player)
	...
	if dbid and tonumber(dbid) then
		...
		if existed then
			...
			exports.titan_logs:dbLog(player, 6, { player, theVehicle }, " Reset unique handling for vehicle #"..dbid..".")
			exports.titan_global:sendMessageToAdmins("[vehicle_manager]: "..getElementData(player, "account:username").." has reset unique handling for vehicle #"..dbid..".")
			exports["titan_vehicle"]:reloadVehicle(tonumber(dbid))
...
function saveHandling(theVehicle1, saveToSQL, who)
	...
	if saveToSQL == 0 then -- SAVE TO ALL INSTANCES
		...
		exports.titan_logs:dbLog(who, 6, { who }, "CHANGED HANDLINGS OF ALL INSTANCES OF VEHICLE SHOP ID #"..id)
		exports.titan_global:sendMessageToAdmins("[vehicle_manager]: "..getElementData(who, "account:username").." has changed handlings of all instances of vehicle shop id #"..id)
	...
	elseif saveToSQL == 1 then --SAVE TO 1 INSTANCE (UNIQUE)
		...
		dbQuery(
			function(qh, who)
				...
				if res > 0 then
					local row = res[1]
					local veh = exports.titan_pool:getElement('vehicle', tonumber(dbid))
					...
					exports.titan_logs:dbLog(who, 6, { who, theVehicle1 }, " SAVED UNIQUE HANDLINGS FOR VEHICLE ID #"..dbid)
					exports.titan_global:sendMessageToAdmins("[vehicle_manager]: "..getElementData(who, "account:username").." saved unique handlings for vehicle id #"..dbid)
				end
			end,
...
function reloadAllInstancesOf(vehicle_shop_id)
	...
	exports.titan_logs:dbLog(client, 6, { client }, "RELOADED "..count.." INSTANCES OF VEHICLE SHOP ID #"..vehicle_shop_id.." IN GAME")
	exports.titan_global:sendMessageToAdmins("[vehicle_manager]: "..getElementData(client, "account:username").." has reloaded "..count.." instances of vehicle shop id #"..vehicle_shop_id.." in game.")
end
...
function getInfoFromVehShopID(vehShopID)
	...
	--local query = dbQuery(exports.titan_mysql:getConnection(), "SELECT * FROM `vehicles_shop` WHERE `id` ='"..vehShopID.."'")	
...
function createTestVehicle(vehShopID, thePed, fromVehLib)
	...
	if not fromVehLib then
		if exports.titan_global:takeMoney(client, 25) then
			exports.titan_global:sendLocalText(client, "* "..exports.titan_global:getPlayerName(client).." gives "..getElementData(thePed, "name").." some money.", 255, 51, 102, 30, {}, true)
		else
	...
	exports.titan_global:fadeToBlack(client)
	...
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "dbid", dbid)
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "fuel", exports["titan_vehicle_fuel"]:getMaxFuel(veh), false)
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "Impounded", 0)
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "engine", 0, false)
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "oldx", x, false)
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "oldy", y, false)
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "oldz", z, false)
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "faction", -1)
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "owner", -1, false)
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "job", 0, false)
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "handbrake", 0, true)
	--Custom properties
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "brand", vehShopData.vehbrand, true)
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "model", vehShopData.vehmodel, true)
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "year", vehShopData.vehyear, true)
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "vehicle_shop_id", vehShopData.id, true)
	
	exports.titan_anticheat:changeProtectedElementDataEx(veh, "isTestDriveCar", client)
	
	exports['titan_vehicle_interiors']:add( veh )
	
	exports.titan_anticheat:changeProtectedElementDataEx(client, "testdrivepos", savepos , false)
...
function loadHandlingToVeh(veh, handlingTable)
	...
	if veh and handlingTable and h then
		...
		--setVehicleHandling(veh, handlings[29][1], h[29]) -- I don't know why this isn't working in 1.4. Temporarily disable this stat / Hanz
...
function finishTestDrive(veh, thePed, orderFinish, forSpawn)
	...
	for i = 0, getVehicleMaxPassengers( veh ) do
		...
		if player then
			...
			exports.titan_anticheat:changeProtectedElementDataEx(player, "realinvehicle", 0, false)
			...
			if savedpos and (not forSpawn or player ~= client) then
				...
				setTimer(function ()
					exports.titan_global:fadeFromBlack(player)
				end, 2000, 1)
				found = true
				break
			end
	...
	if not found and not forSpawn then
		...
		if savedpos then
			...
			setTimer(function ()
				exports.titan_global:fadeFromBlack(client)
			end, 2000, 1)
		end
	end
...
function orderVehicle(vehShopID, thePed)
	...
	exports.titan_anticheat:changeProtectedElementDataEx(source, "carshop:grotti:orderedvehicle:"..getElementData(thePed,"carshop"), tonumber(vehShopID), true)
...
function cancelOrderVehicle(shopname)
	if shopname then
		exports.titan_anticheat:changeProtectedElementDataEx(client, "carshop:grotti:orderedvehicle:"..shopname, false, true)
	else
	--[[
		for i, v in pairs(exports["titan_vehicle_shop"]:getCarShops()) do
			exports.titan_anticheat:changeProtectedElementDataEx(client, "carshop:grotti:orderedvehicle:"..(v.name), false)
		end
		]]
	end
end
