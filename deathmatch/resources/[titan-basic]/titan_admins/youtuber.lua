	...
	if getElementData(thePlayer, "account:username") == "Hanz" then
		if not targetPlayerName or not tonumber(etiketLevel)  then
			...
			elseif getElementData( targetPlayer, "loggedin" ) ~= 1 then
				...
				dbExec(mysql:getConnection(),"UPDATE `characters` SET `youtuber`="..etiketLevel.." WHERE `id`='"..getElementData(targetPlayer, "dbid").."'")
				setElementData(targetPlayer, "youtuber", tonumber(etiketLevel))
			end
		end
	end
end
addCommandHandler("youtuberver", setetiket)