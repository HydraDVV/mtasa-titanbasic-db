...
function giveCarLicense(usingGC)
	...
	exports.titan_hud:sendBottomNotification(source, "Karatekin Sürücü Kursu", "Tebrikler! Başarıyla tüm gereken şeyleri tamamladınız." )
	exports["titan_infobox"]:addBox(source, "success", "Başarıyla ehliyet aldınız!")
	exports.titan_global:giveItem(source, 133, getPlayerName(source):gsub("_"," "))
	...
	if getElementData(source, "owner") == -2 and getElementData(source, "faction") == -1 and getElementModel(source) == 410 then
		if getElementData(player,"license.car") == 3 then
			if getElementData(player, "license.car.cangetin") then
				exports.titan_hud:sendBottomNotification(player, "Karatekin Sürücü Kursu", "'J' tuşuna basarak aracı çalıştırır, 'G' tuşuna basarak el frenini indirebilirsiniz." )
			else
				exports.titan_hud:sendBottomNotification(player, "Karatekin Sürücü Kursu", "Bu aracı sadece ehliyet sorularını dolduranlar kullanabilir." )
				if not exports["titan_integration"]:isPlayerHeadAdmin(player) then
					cancelEvent()
				end
			end
		elseif seat > 0 then
