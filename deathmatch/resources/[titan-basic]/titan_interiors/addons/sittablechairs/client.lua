...
function useChair(chair)
	...
	outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Şu anda sandalyeye oturdun. M'ye basıp kendine sağ tıklarsan sandalyeden kalkabilirsin.", 0, 255, 0, true)
end

function attemptToSitOnChair(chair)
	if ( canISitOnChair(chair) ) then
		if ( chairPerson[chair] ~= nil ) then
			outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Şu an bu koltukta zaten bir başkası oturuyor.", 255, 0, 0, true)
		else
			useChair(chair)
		end
	else
		outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Oturmak istediğin sandalye çok uzağında. Azıcık yaklaş.", 255, 0, 0, true)
	end
end
