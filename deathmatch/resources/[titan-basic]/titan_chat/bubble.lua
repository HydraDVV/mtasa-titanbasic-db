...
function checkForChat()
	if not (getElementAlpha(getLocalPlayer()) == 0) then
		if (isChatBoxInputActive() and not chatting) then
			chatting = true
			setElementData(localPlayer, "written", true)
		elseif (not isChatBoxInputActive() and chatting) then
			chatting = false
			setElementData(localPlayer, "written", false)
		end
	end
end
	...
	function(message, command)
		if source ~= localPlayer or selfVisible then
			if command == "ado" or command == "ame" then
				addText(source, message, { 136, 87, 201 }, font, false, command)
			else
...
addEventHandler("addChatBubbleee", root,
	function(message, command)
		if source ~= localPlayer or selfVisible then
			if command == "ado" or command == "ame" then
				addText(source, message, { 108, 161, 106 }, font, false, command)
			else
				addText(source, message, { 108, 161, 106 })
			end
		end
	end
)
...
function updateStatus(source, n)
	...
	if getElementData(source, "chat:status") then
		addText(source, getElementData(source, "chat:status"), {136, 87, 201}, font, true, n)
	end
end
				...
				for i, v in ipairs(texts) do
					if tick < v.expires or v.sticky then
						...
						if getDistanceBetweenPoints3D(x, y, z, px, py, pz) < distanceVisible and isLineOfSightClear ( x, y, z, px, py, pz, true, not isPedInVehicle(player), false, true) and pdim == dim and pint == int then
							...
							dxDrawText(v.text, sx - (0.1 * width) - 313, yPos+120, sx +350, yPos - (i * fontHeight) + 300, tocolor(unpack(v.color)), 1, font, "center", "top", false, false, false)
							elseif ( a == 1600 ) and ( b == 900 ) then
							dxDrawText(v.text, sx - (0.1 * width) - 345, yPos+120, sx +350, yPos - (i * fontHeight) + 300, tocolor(unpack(v.color)), 1, font, "center", "top", false, false, false)
							elseif ( a == 1366 ) and ( b == 768 ) then
							dxDrawText(v.text, sx - (0.1 * width) - 342, yPos+120, sx +350, yPos - (i * fontHeight) + 300, tocolor(unpack(v.color)), 1, font, "center", "top", false, false, false)
							elseif ( a == 1280 ) and ( b == 720 ) then
							dxDrawText(v.text, sx - (0.1 * width) - 347, yPos+120, sx +350, yPos - (i * fontHeight) + 300, tocolor(unpack(v.color)), 1, font, "center", "top", false, false, false)
							else
							dxDrawText(v.text, sx - (0.1 * width) - 347, yPos+120, sx +350, yPos - (i * fontHeight) + 300, tocolor(unpack(v.color)), 1, font, "center", "top", false, false, false)
