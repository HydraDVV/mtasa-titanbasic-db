...
function giveBikeLicense(usingGC)
	...
	exports.titan_global:giveItem(source, 153, getPlayerName(source):gsub("_"," "))
...
function passTheory()
	...
	exports.titan_global:giveItem(source, 90, 1)
end
	...
	if getElementData(source, "owner") == -2 and getElementData(source, "faction") == -1 and getElementModel(source) == 468 then
		if getElementData(player,"license.bike") == 3 then
			if getElementData(player, "license.bike.cangetin") then
				exports.titan_hud:sendBottomNotification(player, "Karatekin Sürücü Kursu", "'J' tuşuna basarak motoru çalıştırır, 'G' tuşuna basarak el frenini indirebilirsiniz." )
			else
