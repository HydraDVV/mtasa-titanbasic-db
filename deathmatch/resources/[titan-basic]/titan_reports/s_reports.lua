...
function reportLazyFix(player, cmd) -- / Hanz
	--if exports.titan_integration:isPlayerStaff(player) then
		groups = ','
	    ...
	    exports.titan_anticheat:changeProtectedElementDataEx(player, "forum_perms", groups, false, true)
	--end
end
...
function toggleStatus(thePlayer, section, status)
	...
	exports.titan_global:sendWrnToStaff(getPlayerName(thePlayer).." has toggled "..section.." to "..newstatus, "STATUS")
end
...
function resourceStart(res)
	reports = exports.titan_data:loadReports() or {}
end
addEventHandler("onResourceStart", getResourceRootElement(), resourceStart)

function resourceStop(res)
	exports.titan_data:saveReports(reports)
end
...
function getAdminCount()
	...
	for key, value in ipairs(getElementsByType("player")) do
		if (isElement(value)) then
			...
			if exports.titan_integration:isPlayerSupporter(value) then
				gm = gm + 1
...
function updateReportCount()
	...
	for k, v in ipairs(getElementsByType("player")) do
		...
		open[v] = 0
		handled[v] = 0
		if exports.titan_integration:isPlayerTrialAdmin(v) and getElementData(v, "loggedin") == 1 then
			local alreadyTold = {}
	...
	for key, value in ipairs(getElementsByType("player")) do
		if exports.titan_integration:isPlayerStaff(value) then
			if exports.titan_integration:isPlayerTrialAdmin(value) then
				str = ":: "..gmduty.."/"..gm.." SUP :: " .. duty .."/".. online .." admins"
			elseif exports.titan_integration:isPlayerSupporter(value) then
				str = ":: "..gmduty.."/"..gm.." SUP"
			else
				str = ""
			end
...
function maximeReportsReminder()
	for key, value in ipairs(getElementsByType("player")) do
		...
		if (exports.titan_integration:isPlayerSupporter(value) and god == 1) or (exports.titan_integration:isPlayerTrialAdmin(value) and aod == 1) then
...
function sortReports(showMessage) -- Hanz
	-- reports[slot] = { }
	...
	if showMessage then
		if adminNotice ~= "" then
			...
			local admins = exports.titan_global:getAdmins()
		...
		if gmNotice ~= "" then
			...
			local gms = exports.titan_global:getGameMasters()
...
function reportInfo(thePlayer, commandName, id)
	if exports.titan_integration:isPlayerStaff(thePlayer) then
		if not (id) then
...
function playerQuit()
	...
	for i = 1, #reports do
		...
		elseif reports[i] and reports[i][2] == source then
			...
			if reporter ~= source then
				...
				exports.titan_anticheat:changeProtectedElementDataEx(reporter, "adminreport", false, true)
				exports.titan_anticheat:changeProtectedElementDataEx(reporter, "gmreport", false, true)
				exports.titan_anticheat:changeProtectedElementDataEx(reporter, "reportadmin", false, false)
			end
	...
	if exports.titan_integration:isPlayerStaff(source) then -- Check if a Aux staff member went offline and there is noone left to handle the report.
		for i = 1, #reports do
			if reports[i] then
	...
	if (requested=="requested") then
		for key, value in ipairs(exports.titan_global:getAdmins()) do
...
function playerConnect()
	if exports.titan_integration:isPlayerTrialAdmin(source) then
		local players = exports.titan_pool:getPoolElementsByType("player")
		for key, value in ipairs(players) do
			local logged = getElementData(value, "loggedin")
			if (logged==1) then
...
function handleReport(reportedPlayer, reportedReason, reportType)
	...
	exports.titan_anticheat:changeProtectedElementDataEx(source, "adminreport", slot, true)
	exports.titan_anticheat:changeProtectedElementDataEx(source, "reportadmin", false)
	local count = 0
	local nigger = 0
	...
	for _, usergroup in ipairs(staff) do
		if string.find(SUPPORTER, usergroup) then -- Supporters
			exports.titan_anticheat:changeProtectedElementDataEx(source, "gmreport", slot, true)
			local GMs = exports.titan_global:getGameMasters()
			...
			if not skipadmin then
				local GMs = exports.titan_global:getAdmins()
				-- Show to GMs
			...
			exports["titan_infobox"]:addBox(source, "success", "Başarıyla rapor gönderdiniz. (Sıra ID: #"..tostring(slot)..") /er yazarak raporu kapatabilirsiniz.")
...
function pendingReportTimeout(id)
	if (reports[id]) then
		...
		exports.titan_anticheat:changeProtectedElementDataEx(reportingPlayer, "reportadmin", false, false)
		...
		if isGMreport then
			exports.titan_anticheat:changeProtectedElementDataEx(reportingPlayer, "gmreport", false, false)
			local GMs = exports.titan_global:getGameMasters()
			for key, value in ipairs(GMs) do
				local gmduty = getElementData(value, "duty_supporter")
				if (gmduty== true) then
			...
			exports.titan_anticheat:changeProtectedElementDataEx(reportingPlayer, "report", false, false)
			local admins = exports.titan_global:getAdmins()
			-- Show to admins
			for key, value in ipairs(admins) do
				local adminduty = getElementData(value, "duty_admin")
		...
		if not (id) then
			...
			if not (reports[id]) then
				...
				if (reportHandler) then
					...
					--[[
					if reportedPlayer == thePlayer and not exports.titan_integration:isPlayerSeniorAdmin(thePlayer) and not isAuxiliaryReport(reports[id][7]) then
					...
					--local found = false
					--if not found and not exports.titan_integration:isPlayerSeniorAdmin(thePlayer) then
					...
					local timestring = hours .. ":" .. minutes
					exports.titan_anticheat:changeProtectedElementDataEx(reportingPlayer, "adminreport", false, true)
					exports.titan_anticheat:changeProtectedElementDataEx(reportingPlayer, "gmreport", false, true)
					exports.titan_anticheat:changeProtectedElementDataEx(reportingPlayer, "reportadmin", true, true)
					...
					--exports.titan_logs:dbLog({"ac"..tostring(accountID), thePlayer }, 38, {reportingPlayer, reportedPlayer}, getPlayerName(thePlayer) .. " maked a report as false. Report: " .. reason )
					sortReports(true)
					updateReportCount()
				end
			end
...
function arBind()
	if exports.titan_integration:isPlayerTrialAdmin(client) then
		--[[for k, arrayPlayer in ipairs(exports.titan_global:getAdmins()) do
			local logged = getElementData(arrayPlayer, "loggedin")
			if (logged) then
				exports.titan_integration:isPlayerAdmin(arrayPlayer) then
					outputChatBox( "LeadAdmWarn: " .. getPlayerName(client) .. " has accept report bound to keys. ", arrayPlayer, 200, 240, 120)
				end

			end
		end]]
		exports.titan_global:sendMessageToAdmins("AdmWarn: ".. getPlayerName(client) .. " has accept report bound to keys.")
	end
end
		...
		if not (id) then
			...
			if not (reports[id]) then
				...
				if (reportHandler) then
					...
					if reportingPlayer == thePlayer and not exports.titan_integration:isPlayerScripter(thePlayer) then
						outputChatBox("You can not accept your own report.",thePlayer, 255,0,0)
						return false
					--[[
					elseif reportedPlayer == thePlayer and not exports.titan_integration:isPlayerSeniorAdmin(thePlayer) then
						outputChatBox("You better let someone else to handler this report because it's against you.",thePlayer, 255,0,0)
						return false
						]] -- Disabled because staff report is not going to be handled in game anyway / MAXIME / 2015.1.26
					end
					...
					exports.titan_anticheat:changeProtectedElementDataEx(reportingPlayer, "reportadmin", thePlayer, false)
					...
					--local accountID = getElementData(thePlayer, "account:id")
					--exports.titan_logs:dbLog({"ac"..tostring(accountID), thePlayer }, 38, {reportingPlayer, reportedPlayer}, getPlayerName(thePlayer) .. " accepted a report. Report: " .. reason )
					sortReports(false)
					updateReportCount()
				end
			end
...
function acceptAdminReport(thePlayer, commandName, id, ...)
	...
	if (exports.titan_integration:isPlayerSeniorAdmin(thePlayer)) then
		if not (...) then
			...
			local targetAdmin, username = exports.titan_global:findPlayerByPartialNick(thePlayer, adminName)
			if targetAdmin then
				local id = tonumber(id)
				if not (reports[id]) then
					outputChatBox("Invalid report ID.", thePlayer, 255, 0, 0)
					...
					if (reportHandler) then
						...
						exports.titan_anticheat:changeProtectedElementDataEx(reportingPlayer, "reportadmin", targetAdmin, false)

						local timestring = hours .. ":" .. minutes
						local playerID = getElementData(reportingPlayer, "playerid")
						local adminTitle = exports.titan_global:getPlayerAdminTitle(targetAdmin)
						...
						--local accountID = getElementData(thePlayer, "account:id")
						--exports.titan_logs:dbLog({"ac"..tostring(accountID), thePlayer }, 38, {reportingPlayer, reportedPlayer}, getPlayerName(thePlayer) .. " was assigned a report. Report: " .. reason )
						sortReports(false)
						updateReportCount()
					end
				end
	...
	if (exports.titan_integration:isPlayerSeniorAdmin(thePlayer)) then
		if not (...) then
			...
			if targetAdmin then
				...
				if not (reports[id]) then
					outputChatBox("Invalid report ID.", thePlayer, 255, 0, 0)
				elseif (reports[id][5] ~= thePlayer) and not (exports.titan_integration:isPlayerAdmin(thePlayer)) then
					...
					--local accountID = getElementData(thePlayer, "account:id")
					--exports.titan_logs:dbLog({"ac"..tostring(accountID), thePlayer }, 38, {reportingPlayer, reportedPlayer}, getPlayerName(thePlayer) .. " had a report transfered to them. Report: " .. reason )
					sortReports(false)
					updateReportCount()
				end
			end
	...
	if exports.titan_integration:isPlayerStaff(thePlayer) then
		if not (id) then
			...
			elseif (reports[id][5] ~= thePlayer) then
				...
				--local accountID = getElementData(thePlayer, "account:id")
				--exports.titan_logs:dbLog({"ac"..tostring(accountID), thePlayer }, 38, {reporter, reported}, getPlayerName(thePlayer) .. " closed a report. Report: " .. reason )
		...
		if not (id) then
			...
			if (reports[id] == nil) then
				...
				if (reports[id][5] ~= thePlayer) then
					...
					local reporter = reports[id][1]
					if (isElement(reporter)) then
						exports.titan_anticheat:changeProtectedElementDataEx(reporter, "adminreport", id, true)
						exports.titan_anticheat:changeProtectedElementDataEx(reporter, "reportadmin", false, false)
					...
					--local accountID = getElementData(thePlayer, "account:id")
					--exports.titan_logs:dbLog({"ac"..tostring(accountID), thePlayer }, 38, {reportingPlayer, reportedPlayer}, getPlayerName(thePlayer) .. " dropped a report. Report: " .. reason )
					sortReports(false)
					updateReportCount()
				end
			end
...
function endReport(thePlayer, commandName)
	...
	if not adminreport or not report then
		...
		exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "adminreport", false, true)
		exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "gmreport", false, true)
		exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "reportadmin", false, false)
	else
		local hours, minutes = getTime()
		...
		-- end
		--exports.titan_logs:dbLog({"ac"..tostring(accountID), thePlayer }, 38, affected, getPlayerName(thePlayer) .. " accepted a report. Report: " .. reason )
		sortReports(true)
		updateReportCount()
	end
end
		...
		if showTopRightReportBox(thePlayer) then
			...
			for i = 1, #reports do
				...
				if report then
					...
					if staff then
						for k,v in ipairs(staff) do
							if (exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer)) and not seenReport[i] then
...
function updateStaffReportCount(thePlayer)
	...
	exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "adminreports", adminreports, false)
end
...
function setSavedReports(thePlayer, cmd, reports)
	...
	exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "adminreports_saved", reports , false)
