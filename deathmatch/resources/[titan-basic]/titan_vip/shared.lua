...
function SmallestID()
    local query = dbQuery(mysql:getConnection(), "SELECT MIN(e1.id+1) AS nextID FROM `vipPlayers` AS e1 LEFT JOIN `vipPlayers` AS e2 ON e1.id +1 = e2.id WHERE e2.id IS NULL")
