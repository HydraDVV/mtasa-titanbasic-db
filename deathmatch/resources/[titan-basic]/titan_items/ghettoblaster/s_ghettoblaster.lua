...
local function updateWorldItemValue(item, station, volume)
    if station < 0 or station > #exports.titan_vehicle_radio:getStreams() then return end
    ...
    exports.titan_anticheat:changeProtectedElementDataEx(item, "itemValue", newvalue)
    dbExec(exports.titan_mysql:getConnection(), "UPDATE worlditems SET itemvalue='" .. newvalue .. "' WHERE id=" .. getElementData(item, "id"))
    triggerClientEvent("toggleSound", item)
end

function changeTrack(item, step)
    local streams = exports.titan_vehicle_radio:getStreams()
	...
	if not tuned[item] then
		exports.titan_global:sendLocalMeAction(source, "retunes the Ghettoblaster.")
		tuned[item] = true
	else
