-- Configuration
local background_color = tocolor( 125, 125, 125, 125 )
...
--local font = dxCreateFont(":titan_fonts/files/Roboto.ttf", 11)
local font = exports["titan_fonts"]:getFont("Roboto",9)
...
local function getHoverElement( resource, force )
	...
	for _, acceptProtected in ipairs({false, true}) do
		...
		if element and getElementParent(getElementParent(element)) == getResourceRootElement(getResourceFromName(resource or "titan_item-world")) then
			return element
		elseif b and c and d and not resource then
			element = nil
			local x, y, z = nil
			local maxdist = 0.34
			for key, value in ipairs(getElementsByType("object",getResourceRootElement(getResourceFromName(resource or "titan_item-world")))) do
...
local function getImage( itemID, itemValue )
	if not itemID or not tonumber(itemID) then --Stop bugging out int when picking up items too fast / Maxime
		return ":titan_account/img/nil.png"
	end
	...
	if itemID == 16 then -- Clothes
		...
		if not tmp or not tonumber(tmp) or tonumber(tmp) == 999 then
			...
			return ":titan_account/img/" .. tmp .. ".png"
		end
...
local function getOverlayText( itemID, itemValue, isGrouped )
	...
	elseif itemID == 134 then -- Money / Hanz
		text = "$"..exports.titan_global:formatMoney(itemValue)
...
local function getTooltipFor( itemID, itemValue, isGrouped, isProtected )
	...
	if itemID == 134 then
		if itemValue and itemValue ~= 1 and #tostring(itemValue) > 0 and not g_items[itemID][2]:find("#v") and itemValue ~= itemName then -- if #v is there, it's put into the description already
			name = name .. " ($" .. exports.titan_global:formatMoney(itemValue) .. ")"
		end
		...
		elseif not guiGetInputEnabled( ) and not isMTAWindowActive( ) and isCursorShowing( ) then
			...
			if show then
				...
				for i = irows, jrows do
					...
					dxDrawImage( boxx+15, boxy+10, box-25, box-25, getImage( icon[1] ) )
				...
				for i = 1, columns * 5 do
					...
					if item then
						...
						dxDrawRectangle( boxx, boxy, box, box, tocolor(20,20,20,100) )
						dxDrawText("BOŞ", boxx,boxy,boxx+box,boxy+box,tocolor(255,255,255,100),1,font,"center","center")					
					end
				end
			end
				...
				if clickWorldItem then
					...
					if isCursorOverInventory then
						...
						elseif hoverElement then
							...
							elseif getElementModel( hoverElement ) == 2942 and item [ 1 ] == 150 then -- ATM card into ATM/ Hanz
								color = background_movetoelement_color
							...
							elseif getElementModel( hoverElement ) == 3761 then -- shelf - Hanz
							...
							elseif tonumber(getElementData(hoverElement, "itemID")) == 166 and item[1] == 165 then -- Video disc into video player /Hanz
								color = background_movetoelement_color
							else
								color = full_color
							end
			...
			elseif not tooltipYet and getPedOccupiedVehicle(localPlayer) then
				...
				if element then
					...
					if not any and getElementType(element) == "vehicle" and element ~= getPedOccupiedVehicle( localPlayer ) and exports['titan_vehicle']:hasVehiclePlates( element ) and getDistanceBetweenPoints3D( px, py, pz, getElementPosition( element ) ) < 10 then
...
addEventHandler( "onClientClick", getRootElement( ),
	function( button, state, cursorX, cursorY, worldX, worldY, worldZ )
		if not waitingForItemDrop then
			if button == "left" or ( button == "middle" and exports.titan_integration:isPlayerTrialAdmin( getLocalPlayer( ) ) ) then
						...
						elseif not clickItemSlot.group then
							...
							elseif hoverAction == ACTION_SHOW then
								...
								elseif item[1] == 150 then--ATM card
									...
									local owner = exports.titan_cache:getCharacterNameFromID(itemExploded[2])
				...
				elseif button == "left" and isCursorOverInventory and hoverAction and state == "down" then
					if show then
						...
						elseif activeTab == hoverAction then
							show = false
							showCursor( false )
							--exports["titan_realism-system"]:showSpeedo()
						else
							activeTab = hoverAction
						end
					else
...
bindKey( "i", "down",
	function( )
		if getElementData(localPlayer, "loggedin") == 1 then
			...
			elseif ((not getElementData(localPlayer, "adminjailed")) or exports.titan_integration:isPlayerTrialAdmin(localPlayer)) and getElementData(getLocalPlayer(), "viewingInterior")~=1 then -- Chaos | Allowed admins to view inventory in jail, not in previewing interiors
				...
				inventory = false
				showCursor( true )
				--exports["titan_realism-system"]:hideSpeedo()
				playSoundInvOpen()
...
function renameWeapon(item)
	...
	if (checkString == " (D)")  then
		...
		guiEditSetMaxLength(nameEdit, 25)
			if weaponName ~= name and not exports.titan_integration:isPlayerAdmin(localPlayer) then 
		...
		elseif calibre and string.len(calibre) > 3 and not exports.titan_integration:isPlayerAdmin(localPlayer) then
			guiSetEnabled(calibreEdit, false)
		end
		if calibre then
...
function playSoundInvOpen()
	setSoundVolume(playSound(":titan_resources/sounds/inv_open.mp3"), 0)
end

function playSoundInvClose()
	setSoundVolume(playSound(":titan_resources/sounds/inv_close.mp3"), 0)
end
