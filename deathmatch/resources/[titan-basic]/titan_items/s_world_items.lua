...
function canPickup(player, item)
	...
	elseif isProtected(player, item) then
		...
		if isElement(item) then
			if exports['titan_item-world']:can(player, "pickup", item) then
				return true
			else
				return false
			end
...
function canMove(player, item)
	...
	elseif isProtected(player, item) then
		...
		if isElement(item) then
			if exports['titan_item-world']:can(player, "move", item) then
				return true
			else
				return false
			end
...
function protectItem(faction, item, slot)
	...
	if getElementData(source, "itemID") then
		...
		if type(faction) == "number" and exports.titan_global:isStaffOnDuty(client) then
			local protected = getElementData(source, "protected")
			local out = 0
			if protected then
				exports.titan_anticheat:changeProtectedElementDataEx(source, "protected", false)
				outputChatBox("Unset", client, 0, 255, 0)
				out = 0
			else
				exports.titan_anticheat:changeProtectedElementDataEx(source, "protected", faction)
...
function toggleDropPick(player, cmd, state)
	if exports.titan_integration:isPlayerScripter(player) then
...
function dropItem(itemID, x, y, z, ammo, keepammo)
	...
	if not ammo then
		...
		--ANTI ALT-ALT / MAXIME
		if not exports.titan_global:isStaffOnDuty(source) then
		...
		if itemID == 162 then
			...
			if hoursPlayed < 9999999 then
				...
				exports.titan_global:sendMessageToAdmins("|| "..exports["titan_pool"]:getServerName().." Roleplay || BUG Koruma > ".. getPlayerName(client) .. ", Yere Body Armor atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
				exports.titan_global:takeMoney(source, itemValue)
			end
		end
		...
		if itemID == 56 then
			...
			if hoursPlayed < 9999999 then
				outputChatBox("Bu nesneyi yere atmaniz sunucu tarafindan yasaklanmistir.", source, 255, 0, 0)
				exports.titan_global:sendMessageToAdmins("|| "..exports["titan_pool"]:getServerName().." Roleplay || BUG Koruma > ".. getPlayerName(client) .. ", Yere Yüz Maskesi atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
				exports.titan_global:takeMoney(source, itemValue)
		...
		if itemID == 134 then
			...
			if hoursPlayed < 9999999 then
				...
				exports.titan_global:sendMessageToAdmins("|| "..exports["titan_pool"]:getServerName().." Roleplay || BUG Koruma > ".. getPlayerName(client) .. ", Yere para atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
				exports.titan_global:takeMoney(source, itemValue)
		...
		if itemID == 254 then
			...
			--outputDebugString("Check")
			if hoursPlayed < 9999999 then
				outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffffMalesef, bu eşyayı yere atamazsınız.", source, 255, 0, 0, true)
				--outputChatBox("Yetkililere bunu yapmaya calistigin bildirildi.", source, 255, 0, 0)
				--exports.titan_global:sendMessageToAdmins("|| Inception Roleplay || BUG Koruma > ".. getPlayerName(client) .. ", Yere para atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
				exports.titan_global:takeMoney(source, itemValue)
		...
		if itemID == 116 then
			...
			if hoursPlayed < 9999999 then
				outputChatBox("Bu nesneyi yere atmaniz sunucu tarafindan yasaklanmistir.", source, 255, 0, 0)
				outputChatBox("Yetkililere bunu yapmaya calistigin bildirildi.", source, 255, 0, 0)
				exports.titan_global:sendMessageToAdmins("|| "..exports["titan_pool"]:getServerName().." Roleplay || BUG Koruma > ".. getPlayerName(client) .. ", Yere mermi atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
				exports.titan_global:takeMoney(source, itemValue)
		...
		if itemID == 115 then
			local val = exports.titan_global:explode(":", itemValue)
			...
			--exports.titan_global:sendMessageToAdmins("|| Inception Roleplay || BUG Koruma > ".. getPlayerName(client) .. ", Yere silah atmaya calisti.")
			--return
			triggerClientEvent( source, "finishItemDrop", source )
		end
		...
		if ( itemID == 115) then -- Weapons
			local itemCheckExplode = exports.titan_global:explode(":", itemValue)
			-- itemCheckExplode: [1] = gta weapon id, [2] = serial number, [3] = weapon name
			local weaponDetails = exports.titan_global:retrieveWeaponDetails( itemCheckExplode[2]  )
		...
		elseif ( itemID == 116) then -- Ammo
			local ammoDetails = exports.titan_global:explode( ":", itemValue  )
		...
		if itemID == 573 or itemID == 574 or itemID == 580 or itemID == 589 or itemID == 590 or itemID == 581 or itemID == 134 or itemID == 556 or itemID == 557 or itemID == 558 or itemID == 559 or itemID == 555 or itemID == 228 or itemID == 57 or itemID == 17 or itemID == 45 or itemID == 64 or itemID == 16 or itemID == 575 or itemID == 576 or itemID == 577 or itemID == 569 or itemID == 570 or itemID == 571 or itemID == 572 or itemID == 60 or itemID == 137 or itemID == 138 or itemID == 115 or itemID == 116 or itemID == 175 and not exports.integration:isPlayerAdmin(source) then
			outputChatBox( "#822828"..exports["titan_pool"]:getServerName()..": #ffffffBu eşya yere atılamaz.", source, 255, 0, 0, true)
		elseif ( itemID == 81 or itemID == 103 ) and dimension == 0 then
			outputChatBox( "#822828"..exports["titan_pool"]:getServerName()..": #ffffffBu öğeyi bir ev/işyeri içine bırakmanız gerekiyor.", source, 255, 0, 0, true)
		...
		elseif itemID == 139 then
			...
			--[[if not (hasItem(source, itemID)) then
				...
				exports.titan_logs:logMessage(haxStr, 32)
				exports.titan_global:sendMessageToAdmins("AdmWrn: " .. haxStr)		
			end]]
			...
			elseif itemID == 134 then
				if not exports.titan_global:takeMoney(source, itemValue) then
					outputDebugString(getPlayerName(source) .. ' (' .. getPlayerSerial(source) .. ') tried dropping money he doesnt have')

					while exports['titan_items']:takeItem(source, 134) do
					end

					local amount = exports.titan_global:getMoney(source)
					if amount > 0 then
						exports.titan_global:giveItem(source, 134, amount)
					end
			...
			elseif (itemID == 147) then -- Picture Frame
				local dimension = getElementDimension(source)
				if dimension > 0 or (exports.titan_integration:isPlayerDeveloper(source) and exports.titan_global:isAdminOnDuty(source)) or exports.titan_integration:isPlayerScripter(source) then
					local split = exports.titan_global:explode(";", itemValue)
					local url = split[1]
					local texture = split[2]
					if url and texture then
						if exports['titan_textures']:newTexture(source, url, texture) then
			...
			elseif itemID == 169 then --Keypad Door Lock / Maxime
				...
				if not validInterior then
					exports.titan_hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "The interior that this device is made for is no longer existed. Thus far this device is not usable anymore.")
					triggerClientEvent( source, "finishItemDrop", source )
					return false
				end

				if not validDropper then
					exports.titan_hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "Interior '"..interiorName.."' is no longer owned by you. Thus far this device is not usable anymore.")
					triggerClientEvent( source, "finishItemDrop", source )
					return false
				end
				...
				if isIntLocked then
					exports.titan_hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "Interior '"..interiorName.."' is locked, please unlock it first.")
					triggerClientEvent( source, "finishItemDrop", source )
					return false
				end
				...
				if not doorOutside or not doorInside then
					exports.titan_hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "Interior '"..interiorName.."' is defected, please notify scripters.")
					triggerClientEvent( source, "finishItemDrop", source )
					return false
				end
				...
				if interior == doorOutside[4] and dimension == doorOutside[5] then
					if getDistanceBetweenPoints3D(x,y,z,doorOutside[1], doorOutside[2], doorOutside[3]) > maxRange then
						exports.titan_hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "This device can only be installed within "..math.floor(maxRange/2).." meters from its out-door.")
						triggerClientEvent( source, "finishItemDrop", source )
						return false
					end
				...
				elseif interior == doorInside[4] and dimension == tonumber(itemValue) then
					if getDistanceBetweenPoints3D(x,y,z,doorInside[1], doorInside[2], doorInside[3]) > maxRange then
						exports.titan_hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "This device can only be installed within "..math.floor(maxRange/2).." meters from its in-door.")
						triggerClientEvent( source, "finishItemDrop", source )
						return false
					end
				else
					exports.titan_hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "This device can only be installed within "..math.floor(maxRange/2).." meters from its door.")
					triggerClientEvent( source, "finishItemDrop", source )
					return false
				end
			end
			...
			if insert then
				...
				-- Animation
				exports.titan_global:applyAnimation(source, "CARRY", "putdwn", 500, false, false, true)
				...
				if itemID == 134 then
					outputChatBox("" .. exports.titan_global:formatMoney(itemValue) .. "$ yere bırakır.", source, 255, 194, 14)
				...
				local obj = exports['titan_item-world']:createItem(id, itemID, itemValue, modelid, x, y, z + zoffset - 0.05, rx, ry, rz+rz2)
				exports.titan_pool:allocateElement(obj)
				...
				exports.titan_anticheat:changeProtectedElementDataEx(obj, "creator", getElementData(source, "dbid"), false)

				local permissions = { use = 1, move = 1, pickup = 1, useData = {}, moveData = {}, pickupData = {} }
				exports.titan_anticheat:changeProtectedElementDataEx(obj, "worlditem.permissions", permissions)
				...
				--exports.titan_logs:logMessage( getElementData(source, "account:username").."\\"..getPlayerName(source) .. " -> ~World #" .. getElementID(obj) .. " - " .. getItemName( itemID ) .. " - " .. itemValue, 17)
				exports.titan_logs:dbLog(source, 39, source, getPlayerName(source) .. " -> ~World #" .. getElementID(obj) .. " - " .. getItemName( itemID ) .. " - " .. itemValue )
				...
				if itemID == 134 then
					triggerEvent('sendAme', source, "" .. exports.titan_global:formatMoney(itemValue) .. "$ bırakır.")
		...
		if tonumber(getElementData(source, "duty")) > 0 then
			outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffffSilahlarınızı görev başındayken bırakamazsınız.", source, 255, 0, 0, true)
		elseif tonumber(getElementData(source, "job")) == 4 and itemID == 41 then
			outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffffBu sprey kutusunu bırakamazsın.", source, 255, 0, 0, true)
		else
			...
			--[[local totalAmmoPosessed = exports.titan_global:getWeaponCount(source, itemID) 
			...
			if query then
				...
				exports.titan_global:takeWeapon(source, itemID)
				if keepammo then
					exports.titan_global:giveWeapon(source, itemID, keepammo)
				end
				...
				local obj = exports['titan_item-world']:createItem(id, -itemID, ammo, modelid, x, y, z - 0.4, 75, -10, rz2)
				exports.titan_pool:allocateElement(obj)
				...
				--exports.titan_logs:logMessage( getElementData(source, "account:username").."\\"..getPlayerName(source) .. " -> ~World #" .. getElementID(obj) .. " - " .. getItemName( -itemID ) .. " - " .. ammo, 17)
				exports.titan_logs:dbLog(source, 39, source, getPlayerName(source).. " -> ~World #" .. getElementID(obj) .. " - " .. getItemName( -itemID ) .. " - " .. ammo)
			end
		end
	end
...
function doItemGiveawayChecks(player, itemID, itemValue)
	...
	if mask and getElementData(source, mask[1]) and not hasItem(source, itemID) then
		...
		exports.titan_anticheat:changeProtectedElementDataEx(source, mask[1])
	end
	...
	--if itemID == 16 and not hasItem(source, 16, getPedSkin(source)) then
		...
		exports.titan_anticheat:changeProtectedElementDataEx(source, 'clothing:id', nil, true)
	...
	if badge and getElementData(source, badge[1]) and not hasItem(source, itemID, removeOOC(getElementData(source, badge[1]))) then
		...
		exports.titan_anticheat:changeProtectedElementDataEx(source, badge[1])
		exports.titan_global:updateNametagColor(source)
	end
...
local function moveItem(item, x, y, z)
	...
	if not exports.titan_integration:isPlayerTrialAdmin(source) and (itemID == 81 or itemID == 103) then
		return
	end
	
	if itemID == 138 then
		if not exports.titan_integration:isPlayerAdmin(source) then
			outputChatBox("Only a Lead+ admin can move this item.", source, 255, 0, 0)
			return
		end
	end
	
	if itemID == 139 and not exports.titan_integration:isPlayerTrialAdmin(source) then
...
addEventHandler('item:move:save', root,
	function(x, y, z, rx, ry, rz)
		...
		if exports.titan_global:isStaffOnDuty(client) or exports.titan_integration:isPlayerDeveloper(client) or exports.titan_integration:isPlayerScripter(client) then
			...
			local itemID = getElementData(source, "itemID")
			if itemID == 138 and not exports.titan_integration:isPlayerDeveloper(client) then
				outputChatBox("Only a Lead+ admin can move this item.", client, 255, 0, 0)
				reset()
				return
			end
			
			if itemID == 139 and not exports.titan_integration:isPlayerDeveloper(client) then
...
local function rotateItem(item, rz)
	if not exports.titan_integration:isPlayerTrialAdmin(source) then
		return
	end
...
function pickupItem(object, leftammo)
	...
	exports.titan_anticheat:setEld(object, "transfering", true, "none") --No sync at all.
	...
	if not canPickup(source, object) then
		outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffffBu öğeyi alamazsınız. F2 ile bir yetkiliye başvurun.", source, 255, 0, 0, true)
		exports.titan_anticheat:changeEld(object, "transfering", nil)
		return
	end
	...
	if ((itemID >= 31) and itemID <= 43) or itemBannedByAltAltChecker[itemID] then
		...
		if hoursPlayedTo and hoursPlayedTo < 10 and not exports.titan_global:isStaffOnDuty(source) then
			outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffff"..getItemName( itemID ).." yerden alabilmek için 10 saat oynaman olmalıdır.", source, 255, 0, 0, true)
			exports.titan_anticheat:changeEld(object, "transfering", nil)
			return false
		end
		...
		if creator ~= picker then
			local accountCreator = exports.titan_cache:getAccountFromCharacterId(creator)
			if tonumber(accountCreator.id) == getElementData(source, "account:id") then
				outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffffVarlıkların aynı hesaptaki karakterleri veya bir oyuncunun sahip olduğu çoklu hesaplar arasında aktarılması kesinlikle yasaktır! ", source, 255, 0, 0, true)
				exports.titan_global:sendMessageToAdmins("[ANTI-ALT->ALT]: Detected illegal assets transferring on account name '"..getElementData(source, "account:username").."'.")
				exports.titan_global:sendMessageToAdmins("[ANTI-ALT->ALT]: Suspect is trying to transfer a "..getItemName( itemID ).." between alternatives.")
				exports.titan_logs:dbLog(source, 5, {object}, "TRIED TO ALT-ALT " .. getItemName( itemID ) .. " between alternatives.")
				exports.titan_anticheat:changeEld(object, "transfering", nil)
				return false
			end
		end
	...
	if itemID == 115 then
		if isThisGunDuplicated(itemValue, source) then
			destroyElement(object)
			dbExec(mysql:getConnection(), "DELETE FROM worlditems WHERE id='" .. id .. "'")
			outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffffSilah çoğaltılması tespit edilerek silahınız silindi, üzgünüz.", source, 255,0,0, true)
			return false
		end
	end
	...
	if itemID == 138 then
		if not exports.titan_integration:isPlayerAdmin(source) then
			outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffffBu öğeyi sadece bir yetkili alabilir.", source, 255, 0, 0, true)
			exports.titan_anticheat:changeEld(object, "transfering", nil)
			return false
		end
	end
	...
	exports.titan_global:applyAnimation(source, "CARRY", "liftup", 600, false, true, true)
	if itemID > 0 then
		dbExec(mysql:getConnection(), "DELETE FROM worlditems WHERE id='" .. id .. "'")
		...
		if itemID == -100 then
			...
			if leftammo and itemValue > leftammo then
				...
				exports.titan_anticheat:changeProtectedElementDataEx(object, "itemValue", itemValue)
				dbExec(mysql:getConnection(), "UPDATE worlditems SET itemvalue=" .. itemValue .. " WHERE id=" .. id)
				itemValue = leftammo
			else
				dbExec(mysql:getConnection(), "DELETE FROM worlditems WHERE id='" .. id .. "'")
			end
			exports.titan_global:giveWeapon(source, -itemID, itemValue, true)
		end
	end

	exports.titan_logs:dbLog(source, 39, {object}, "~World #" ..getElementID(object) .. "->" .. getPlayerName(source) .. " - " ..getItemName(itemID).. " - " ..itemValue)
	destroyElement(object)
	
	if itemID == 134 then -- money
		exports.titan_global:giveMoney(source, itemValue)
		outputChatBox("" .. exports.titan_global:formatMoney(itemValue) .. "$ alır.", source, 255, 194, 14)
		triggerEvent('sendAme', source, "" .. exports.titan_global:formatMoney(itemValue) .. "$ alır.")
	elseif itemID == 178 then
		local yup = split(itemValue, ":")
...
function removeItemTransferingState()
	for i, object in pairs(exports.titan_pool:getPoolElementsByType("object")) do
		exports.titan_anticheat:changeEld(object, "transfering", nil)
	end
end
