...
local function output(from, to, itemID, itemValue, evenIfSamePlayer)
	...
	if getElementType(from) == "player" and getElementType(to) == "player" then
		--exports.titan_global:sendLocalMeAction( from, "gives " .. getPlayerName( to ):gsub("_", " ") .. " a " .. getItemName( itemID, itemValue ) .. "." )
		exports.titan_global:sendLocalMeAction(from, "sağ elinde bulunan ".. getItemName( itemID, itemValue ).." isimli öğeyi "..getPlayerName( to ):gsub("_", " ").." kişisine verir.")
	-- player to item
	...
	elseif getElementType(to) == "player" then
		...
		if itemID == 150 then --ATM card / MAXIME
			...
			--exports["titan_log"]:logKayit(dbid,getPlayerName( source ) , getItemName( itemID,itemValue )  ,name, "Aldi")
			end
		end
	end
	...
	if not ammo then  
		...
		if item then
			...
			if ( (item[1] >= 31 and item[1] <= 43) or itemBannedByAltAltChecker[item[1]]) and not (getElementModel(element) == 2942 and item[1] == 150) then 
				...
				if not exports.titan_global:isStaffOnDuty(source) and not exports.titan_global:isStaffOnDuty(element) then
			...
			if not (getElementModel( element ) == 2942) and not hasSpaceForItem( element, item[1], item[2] ) then --Except for ATM Machine / MAXIME
				...
				if (item[1] == 115) then -- Weapons
					local itemCheckExplode = exports.titan_global:explode(":", item[2])
				...
				elseif (item[1] == 116) then -- Ammo
					local ammoDetails = exports.titan_global:explode( ":", item[2]  )
					-- itemCheckExplode: [1] = gta weapon id, [2] = serial number, [3] = weapon name
				...
				elseif (item[1] == 179 and getElementType(element) == "vehicle") then --vehicle texture
					...
					local veh = element
					if(exports.titan_global:isStaffOnDuty(source) or exports.titan_integration:isPlayerScripter(source) or exports.titan_global:hasItem(source, 3, tonumber(vehID)) or (getElementData(veh, "faction") > 0 and exports.titan_factions:isPlayerInFaction(source, getElementData(veh, "faction"))) ) then
						outputDebugString("access granted")
						local itemExploded = exports.titan_global:explode(";", item[2])
						local url = itemExploded[1]
						local texName = itemExploded[2]
						if url and texName then
							local res = exports["titan_item-texture"]:addVehicleTexture(source, veh, texName, url)
							if res then
						...
						if elementItemID then
							if elementItemID == 166 then --video player
								if item[1] ~= 165 then --if item being moved to video player is not a valid video item
									exports.titan_hud:sendBottomNotification(source, "Video Player", "That is not a valid disc.")
									triggerClientEvent( source, event or "finishItemMove", source )
									return									
								end
							end
					...
					if not success then
						...
						if elementItemID then
							if elementItemID == 166 then --video system
								exports.titan_hud:sendBottomNotification(source, "Video Player", "There is already a disc inside. Eject old disc first.")
								fakeReturned = true
							end
						end
						if not fakeReturned then --only check by model IDs if we didnt already find a match on itemID
							if getElementModel(element) == 2942 then
								exports.titan_hud:sendBottomNotification(source, "ATM Machine", "There is another ATM stuck inside the ATM machine's slot. Right-click for interactions.")
							end
						end
						outputDebugString( "Item Moving failed: " .. tostring( reason ))
					else
						if getElementModel(element) == 2942 then
							exports.titan_bank:playAtmInsert(element)
						elseif item[1] == 165 then --video disc
							if exports.titan_clubtec:isVideoPlayer(element) then
						...
						--exports.titan_logs:logMessage( getPlayerName( source ) .. "->" .. name .. " #" .. getElementID(element) .. " - " .. getItemName( item[1] ) .. " - " .. item[2], 17)
						--exports.titan_logs:dbLog(source, 39, source, getPlayerName( source ) .. "->" .. name .. " #" .. getElementID(element) .. " - " .. getItemName( item[1] ) .. " - " .. item[2] )
						doItemGiveawayChecks( source, item[1] )
						output(source, element, item[1], item[2])
					end
				if getElementType( element ) == "vehicle" then
              --  exports["titan_log"]:logKayit(getElementID(element),getPlayerName( source ) , getItemName( item[1],item[2] )  ,name,"Koydu")
                exports.titan_logs:dbLog(source, 39, source, getPlayerName( source ) .. "->" .. name .. " #" .. getElementID(element) .. " - " .. getItemName( item[1] ) .. " - " .. item[2] )
                elseif getElementType( element ) == "player" then
                -- exports["titan_log"]:logKayitt(getElementID(source),getPlayerName( element ) , getItemName( item[1],item[2] )  ,getPlayerName(source),"Aldı")
               -- exports["titan_log"]:logKayit(getElementID(element),getPlayerName( source ) , getItemName( item[1],item[2] )  ,getPlayerName(element),"Verdi")
                end			
				end
		end
	else -- IF AMMO
			...
			elseif tonumber(getElementData(source, "job")) == 4 and slot == 41 then
				...
				if slot == -100 then 	
					...
					if ammo > 0 then
						...
						--exports.titan_logs:logMessage( getPlayerName( source ) .. "->" .. name .. " #" .. getElementID(element) .. " - " .. getItemName( slot ) .. " - " .. ammo, 17)
						exports.titan_logs:dbLog(source, 39, source, getPlayerName(source) .. " moved " .. getItemName(slot) " - " .. ammo .. " #" .. getElementID(element) )
						output(source, element, -100)
					end
				else
					local getCurrentMaxAmmo = exports.titan_global:getWeaponCount(source, slot)
					if ammo > getCurrentMaxAmmo then
						exports.titan_global:sendMessageToAdmins("[items\moveToElement] Possible duplication of gun from '"..getPlayerName(source).."' // " .. getItemName( -slot ) )
						--exports.titan_logs:logMessage( getPlayerName( source ) .. "->" .. name .. " #" .. getElementID(element) .. " - " .. getItemName( -slot ) .. " - " .. ammo .. " BLOCKED DUE POSSIBLE DUPING", 17)
						exports.titan_logs:dbLog(source, 39, source, getPlayerName(source) .. " moved " .. getItemName(-slot) " -  #" .. getElementID(element) .. " - BLOCKED DUE POSSIBLE DUPING" )
						triggerClientEvent( source, event or "finishItemMove", source )
						return
					end
					exports.titan_global:takeWeapon( source, slot )
					if ammo > 0 then
						giveItem( element, -slot, ammo )
						--exports.titan_logs:logMessage( getPlayerName( source ) .. "->" .. name .. " #" .. getElementID(element) .. " - " .. getItemName( -slot ) .. " - " .. ammo, 17)
						exports.titan_logs:dbLog(source, 39, source, getPlayerName(source) .. " moved " .. getItemName(-slot) " - " .. ammo .. " #" .. getElementID(element) )
...
local function moveWorldItemToElement( item, element )
	...
	if giveItem( element, itemID, itemValue ) then
		...
		if itemID == 166 then --video player
			local videoplayerDisc = exports.titan_clubtec:getVideoPlayerCurrentVideoDisc(item) or 2
		...
		--exports.titan_logs:logMessage( getPlayerName( source ) .. " put item #" .. id .. " (" .. itemID .. ":" .. getItemName( itemID ) .. ") - " .. itemValue .. " in " .. name .. " #" .. getElementID(element), 17)
		exports.titan_logs:dbLog( source, 39, source, getPlayerName( source ) .. " put item #" .. id .. " (" .. itemID .. ":" .. getItemName( itemID ) .. ") - " .. itemValue .. " in " .. name .. " #" .. getElementID(element))
...
local function moveFromElement( element, slot, ammo, index )
	...
	if item and item[3] == index then
		...
		if not exports.titan_integration:isPlayerTrialAdmin( source ) and getElementType( element ) == "vehicle" and ( item[1] == 61 or item[1] == 85  or item[1] == 117 or item[1] == 140) then
			outputChatBox( "Please contact an admin via F2 to move this item.", source, 255, 0, 0 )
		elseif not exports.titan_integration:isPlayerAdmin(source) and (item[1] == 138) then
			outputChatBox("This item requires a regular admin to be moved.", source, 255, 0, 0)
		elseif not exports.titan_integration:isPlayerTrialAdmin(source) and (item[1] == 139) then
		...
		elseif item[1] > 0 then			
			if moveItem( element, source, slot ) then
				...
				exports.titan_logs:dbLog(source, 39, source, name .. " #" .. getElementID(element) .. "->" .. getPlayerName( source ) .. " - " .. getItemName( item[1] ) .. " - " .. item[2])
		...
		elseif item[1] == -100 then
			...
			if armor == 0 then
				...
				--exports.titan_logs:logMessage( name .. " #" .. getElementID(element) .. "->" .. getPlayerName( source ) .. " - " .. getItemName( item[1] ) .. " - " .. ( math.min( item[2], armor ) ), 17)
				exports.titan_logs:dbLog(source, 39, source, name .. " #" .. getElementID(element) .. "->" .. getPlayerName( source ) .. " - " .. getItemName( item[1] ) .. " - " .. ( math.min( item[2], armor ) ))
			end
			triggerClientEvent( source, "forceElementMoveUpdate", source )
		else
			takeItemFromSlot( element, slot )
			output( element, source, item[1])
			if ammo < item[2] then
				exports.titan_global:giveWeapon( source, -item[1], ammo )
				giveItem( element, item[1], item[2] - ammo )
				--exports.titan_logs:logMessage( name .. " #" .. getElementID(element) .. "->" .. getPlayerName( source ) .. " - " .. getItemName( item[1] ) .. " - " .. ( item[2] - ammo ), 17)
				exports.titan_logs:dbLog(source, 39, source, name .. " #" .. getElementID(element) .. "->" .. getPlayerName( source ) .. " - " .. getItemName( item[1] ) .. " - " .. ( item[2] - ammo ))
			else
				exports.titan_global:giveWeapon( source, -item[1], item[2] )
				--exports.titan_logs:logMessage( name .. " #" .. getElementID(element) .. "->" .. getPlayerName( source ) .. " - " .. getItemName( item[1] ) .. " - " .. item[2], 17)
				exports.titan_logs:dbLog(source, 39, {source, element}, name .. " #" .. getElementID(element) .. "->" .. getPlayerName( source ) .. " - " .. getItemName( item[1] ) .. " - " .. item[2])
			end
			triggerClientEvent( source, "forceElementMoveUpdate", source )
		end
	end
...
function getName(element)
	...
	if getElementParent(getElementParent(element)) == getResourceRootElement(getResourceFromName("titan_item-world")) then
	...
	if getElementType( element ) == "vehicle" then
		...
		return exports.titan_global:getVehicleName(element)
	end
