...
local staffTitles = exports.titan_integration:getStaffTitles()
function getStaffInfo(username, error)
	local accounts = exports.titan_account:getTableInformations()
	...
	dbQuery(
		function(qh, source, user)
			...
			--if rows > 0 then
				...
				exports.titan_account:updateTables()
...
function getTeamsData()
	staffTitles = exports.titan_integration:getStaffTitles()
	local users = {}
	dbQuery(
		function(qh, source)
			local res, rows, err = dbPoll(qh, 0)
...
function editStaff(userid, ranks, details)
	...
	if ranks[1] and ranks[1] ~= tonumber(user.admin) then
		...
		exports.titan_global:sendMessageToStaff("[Yetki Güncellemesi] "..exports.titan_global:getPlayerFullIdentity(source, 1, true).." adlı yetkili "..(ranks[1] > tonumber(user.admin) and "promoted" or "demoted").." '"..user.username.."' from "..staffTitles[1][tonumber(user.admin)].." to "..staffTitles[1][ranks[1]]..".", true)
		if target then exports.titan_anticheat:changeProtectedElementDataEx(target, "admin_level", ranks[1], true) end
	end
	if ranks[2] and ranks[2] ~= tonumber(user.supporter) then
		...
		exports.titan_global:sendMessageToStaff("[Yetki Güncellemesi] "..exports.titan_global:getPlayerFullIdentity(source, 1, true).." adlı yetkili "..(ranks[2] > tonumber(user.supporter) and "promoted" or "demoted").." '"..user.username.."' from "..staffTitles[2][tonumber(user.supporter)].." to "..staffTitles[2][ranks[2]]..".", true)
		if target then exports.titan_anticheat:changeProtectedElementDataEx(target, "supporter_level", ranks[2], true) end
	end
	if ranks[3] and ranks[3] ~= tonumber(user.vct) then
		...
		exports.titan_global:sendMessageToStaff("[Yetki Güncellemesi] "..exports.titan_global:getPlayerFullIdentity(source, 1, true).." adlı yetkili "..(ranks[3] > tonumber(user.vct) and "promoted" or "demoted").." '"..user.username.."' from "..staffTitles[3][tonumber(user.vct)].." to "..staffTitles[3][ranks[3]]..".", true)
		if target then exports.titan_anticheat:changeProtectedElementDataEx(target, "vct_level", ranks[3], true) end
	...
	if ranks[4] and ranks[4] ~= tonumber(user.scripter) then
		...
		exports.titan_global:sendMessageToStaff("[Yetki Güncellemesi] "..exports.titan_global:getPlayerFullIdentity(source, 1, true).." adlı yetkili "..(ranks[4] > tonumber(user.scripter) and "promoted" or "demoted").." '"..user.username.."' from "..staffTitles[4][tonumber(user.scripter)].." to "..staffTitles[4][ranks[4]]..".", true)
		if target then exports.titan_anticheat:changeProtectedElementDataEx(target, "scripter_level", ranks[4], true) end
		 exports.titan_account:updateTables()
	...
	if ranks[5] and ranks[5] ~= tonumber(user.mapper) then
		...
		exports.titan_global:sendMessageToStaff("[Yetki Güncellemesi] "..exports.titan_global:getPlayerFullIdentity(source, 1, true).." adlı yetkili "..(ranks[5] > tonumber(user.mapper) and "promoted" or "demoted").." '"..user.username.."' from "..staffTitles[5][tonumber(user.mapper)].." to "..staffTitles[5][ranks[5]]..".", true)
		if target then exports.titan_anticheat:changeProtectedElementDataEx(target, "mapper_level", ranks[5], true) end
	end
	if tail ~= '' then
