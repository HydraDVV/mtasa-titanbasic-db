				...
				if getElementData(vehicle, "speedcamera:state") then
					...
					if not isCameraExisting(player) then
						...
						if speed then
							...
							outputChatBox("KULLANIM: /" .. cmd .. " [60 or more]", player, 255, 180, 20, false)
						end
					else
							...
							setTimer(function(hitElement, x, y, z, kmh)
								...
								if isVerifiedElement(radar, "vehicle") and getElementData(radar, "speedcamera:state") then
									...
									for seat,player in pairs(getVehicleOccupants(radar)) do
										outputChatBox("[RADAR] A " .. getColorName(c1, c2) .. " " .. exports.titan_global:getVehicleName(hitElement) .. " was clocked travelling at " .. tonumber(kmh) .. " km/h and is heading " .. direction .. ".", player, 255, 180, 20, false)
