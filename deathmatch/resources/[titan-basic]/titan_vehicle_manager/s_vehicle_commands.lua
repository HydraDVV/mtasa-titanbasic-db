...
function getVehicleName(vehicle)
	return exports.titan_global:getVehicleName(vehicle)
end
	...
	if exports.titan_integration:isPlayerAdmin( thePlayer ) then
		...
		if not vehID or not tonumber(vehID) or (tonumber(vehID) % 1 ~= 0 ) then
			...
			if veh then
				...
				if not vehID then
					exports["titan_infobox"]:addBox(thePlayer, "error", "Bir araçta olmalısın. /reloadveh [Araç ID]")
					return false
				end
			end
		end
		...
		exports["titan_vehicle"]:reloadVehicle(tonumber(vehID))
		exports["titan_infobox"]:addBox(thePlayer, "success", vehID.." ID'li aracı tekrar yükledin.")

		addVehicleLogs(tonumber(vehID), commandName, thePlayer)
		exports.titan_logs:dbLog(thePlayer, 4, { veh, thePlayer }, commandName)
		return true
	end
end
...
function togVehReg(admin, command, target, status)
	if (exports.titan_integration:isPlayerAdmin(admin)) then
		if not (target) or not (status) then
			...
			local pv = exports.titan_pool:getElement("vehicle", tonumber(target))
			...
			if (pv) then
					...
					if (stat == 0) then
						...
						exports.titan_logs:dbLog(admin, 4, { pv, admin }, command.." OFF")
					elseif (stat == 1) then
						...
						exports.titan_logs:dbLog(admin, 4, { pv, admin }, command.." ON")
					end
				else
...
function spinCarOut(thePlayer, commandName, targetPlayer, round)
	if (exports.titan_integration:isPlayerAdmin(thePlayer)) then
		if not targetPlayer then
				exports["titan_infobox"]:addBox(thePlayer, "info", "/spinout [Karakter Adı & ID] [Çevireceğin Sayı]")
		else
			if not round or not tonumber(round) or tonumber( round ) % 1 ~= 0 or tonumber( round ) > 100 then
				round = 1
			end
			local targetPlayer = exports.titan_global:findPlayerByPartialNick(thePlayer, targetPlayer)
			local targetVehicle = getPedOccupiedVehicle(targetPlayer)
			if targetVehicle == false then
				exports["titan_infobox"]:addBox(thePlayer, "error", "Hedef oyuncu aracın içerisinde değil.")
			else
				exports["titan_infobox"]:addBox(thePlayer, "success", getPlayerName(targetPlayer).." isimli oyuncunun aracını "..tostring(round).." kez çevirdin.")
...
function unflipCar(thePlayer, commandName, targetPlayer)
	if exports.titan_integration:isPlayerTrialAdmin(thePlayer) or getElementData(thePlayer, "faction") == 4 or exports.titan_integration:isPlayerSupporter(thePlayer) then
		if not targetPlayer or not exports.titan_integration:isPlayerTrialAdmin(thePlayer) then
			if not (isPedInVehicle(thePlayer)) then
				exports["titan_infobox"]:addBox(thePlayer, "error", "Bir aracın içerisinde değilsin.")
			else
				local veh = getPedOccupiedVehicle(thePlayer)
				local rx, ry, rz = getVehicleRotation(veh)
				setVehicleRotation(veh, 0, ry, rz)
				exports["titan_infobox"]:addBox(thePlayer, "success", "Başarıyla aracını düz çevirdin.")
			...
			if targetPlayer then
				...
				if (logged==0) then
					exports["titan_infobox"]:addBox(thePlayer, "error", "Bu oyuncu oyuna giriş yapmamış.")
				else
					...
					if pveh then
						...
						if getElementData(thePlayer, "hiddenadmin") == 1 then
							exports["titan_infobox"]:addBox(targetPlayer, "success", "Gizli bir yetkili aracınızı düz çevirdi.")
						else
							exports["titan_infobox"]:addBox(targetPlayer, "success", username.." isimli yetkili aracınızı düz çevirdi.")
						end
						outputChatBox("You unflipped " .. targetPlayerName:gsub("_"," ") .. "'s car.", thePlayer, 0, 255, 0)
						exports["titan_infobox"]:addBox(thePlayer, "success", targetPlayerName:gsub("_"," ").." isimli oyuncunun aracını düz çevirdiniz.")

						addVehicleLogs(getElementData(pveh, "dbid"), commandName, thePlayer)
						exports.titan_logs:dbLog(thePlayer, 4, { pveh, thePlayer }, command)
					else
						exports["titan_infobox"]:addBox(thePlayer, "error", targetPlayerName:gsub("_"," ") .." isimli oyuncu aracın içerisinde değil.")
					end
				end
			end
		end
...
function flipCar(thePlayer, commandName, targetPlayer)
	if exports.titan_integration:isPlayerTrialAdmin(thePlayer) or getElementData(thePlayer,"faction") == 4 or exports.titan_integration:isPlayerSupporter(thePlayer) then -- SFTR, working on motorbikes etc
		if not targetPlayer or not exports.titan_integration:isPlayerTrialAdmin(thePlayer) then
			if not (isPedInVehicle(thePlayer)) then
...
function unlockAllCivilianCars(thePlayer, commandName)
	if (exports.titan_integration:isPlayerTrialAdmin(thePlayer)) then
		local count = 0
		for key, value in ipairs(exports.titan_pool:getPoolElementsByType("vehicle")) do
		...
		--addVehicleLogs(getElementData(pveh, "dbid"), commandName, thePlayer)
		exports.titan_logs:dbLog(thePlayer, 4, { thePlayer }, commandName)
	end
end
...
function createTempVehicle(thePlayer, commandName, vehShopID)
	if exports.titan_integration:isPlayerSeniorAdmin(thePlayer) then
		if not vehShopID or not tonumber(vehShopID) then
			exports["titan_infobox"]:addBox(thePlayer, "info", "/veh [Araç ID (/vehlib)]")
			return false
		else
		...
		exports.titan_pool:allocateElement(veh, dbid)
		...
		setVehicleVariant(veh, exports['titan_vehicle']:getRandomVariant(getElementModel(veh)))

		setElementData(veh, "dbid", dbid)
		setElementData(veh, "fuel", exports["titan_vehicle_fuel"]:getMaxFuel(veh))
		...
		exports['titan_vehicle_interiors']:add( veh )
		...
		exports.titan_logs:dbLog(thePlayer, 6, thePlayer, "VEH ".. vehShopID .. " created with ID " .. dbid)
		exports["titan_infobox"]:addBox(thePlayer, "success", getVehicleName(veh).." isimli aracı spawnladınız. ID: (#"..dbid..")")
	else
		exports["titan_infobox"]:addBox(thePlayer, "error", "Bu komutu kullanabilmek için A4 Lider Yetkili ve üstü olmalısın.")
	end
end
...
function getOldCarID(thePlayer, commandName, targetPlayerName)
	...
	if exports.titan_integration:isPlayerTrialAdmin(thePlayer) and targetPlayerName then
		targetPlayer = exports.titan_global:findPlayerByPartialNick(thePlayer, targetPlayerName)
		if targetPlayer then
...
function gotoCar(thePlayer, commandName, id)
	if exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer) or exports.titan_integration:isPlayerScripter(thePlayer) or exports.titan_integration:isPlayerVehicleConsultant(thePlayer) then
		if not (id) then
			exports["titan_infobox"]:addBox(thePlayer, "info", "Kullanım: /gotocar [Araç ID]")
		else
			local theVehicle = exports.titan_pool:getElement("vehicle", tonumber(id))
			if theVehicle then
				...
				exports.titan_logs:dbLog(thePlayer, 6, theVehicle, commandName)

				addVehicleLogs(id, commandName, thePlayer)

				exports["titan_infobox"]:addBox(thePlayer, "success", "Başarıyla aracın yanına ışınlandın.")
			else
				exports["titan_infobox"]:addBox(thePlayer, "error", "Hatalı Araç ID'si girdin.")
			end
		end
	end
end
...
function getCar(thePlayer, commandName, id)
	if exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer) or exports.titan_integration:isPlayerScripter(thePlayer) or exports.titan_integration:isPlayerVehicleConsultant(thePlayer) then
		if not (id) then
			exports["titan_infobox"]:addBox(thePlayer, "info", "Kullanım: /getcar [Araç ID]")
		else
			local theVehicle = exports.titan_pool:getElement("vehicle", tonumber(id))
			if theVehicle then
				local r = getPedRotation(thePlayer)
				...
				exports["titan_infobox"]:addBox(thePlayer, "success", "Başarıyla aracı yanına getirdin.")
			else
				exports["titan_infobox"]:addBox(thePlayer, "error", "Hatalı Araç ID'si girdin.")
			end
		end
...
function sendCar(thePlayer, commandName, id, toPlayer)
	if exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer) or exports.titan_integration:isPlayerVehicleConsultant(thePlayer) then
		if not (id) or not (toPlayer) then
			exports["titan_infobox"]:addBox(thePlayer, "info", "Kullanım: /sendcar [Araç ID] [Karakter Adı & ID]")
		else
			local theVehicle = exports.titan_pool:getElement("vehicle", tonumber(id))
			local targetPlayer, targetPlayerName = exports.titan_global:findPlayerByPartialNick(thePlayer, toPlayer)
			if theVehicle then
				...
				exports.titan_logs:dbLog(thePlayer, 6, theVehicle, commandName.." to "..targetPlayerName)

				addVehicleLogs(id, commandName, thePlayer)

				exports["titan_infobox"]:addBox(thePlayer, "success", "Aracı "..targetPlayerName.." isimli oyuncunun yanına gönderdin.")
				if getElementData(thePlayer, "hiddenadmin") == 1 then
					exports["titan_infobox"]:addBox(targetPlayer, "success", "Gizli bir yetkili aracınızı yanınıza gönderdi.")
				else
					exports["titan_infobox"]:addBox(targetPlayer, "success", exports.titan_global:getPlayerFullIdentity(thePlayer).." isimli yetkili aracınızı yanınıza gönderdi.")
				end
			else
				exports["titan_infobox"]:addBox(thePlayer, "error", "Hatalı Araç ID'si girdin.")
			end
...
function sendPlayerToVehicle(thePlayer, commandName, toPlayer, id)
	if exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer) then
		if not (id) or not (toPlayer) then
			exports["titan_infobox"]:addBox(thePlayer, "info", "Kullanım: /sendtoveh [Araç ID] [Karakter Adı & ID]")
		else
			local theVehicle = exports.titan_pool:getElement("vehicle", tonumber(id))
			local targetPlayer, targetPlayerName = exports.titan_global:findPlayerByPartialNick(thePlayer, toPlayer)
			if theVehicle then
				...
				exports.titan_logs:dbLog(thePlayer, 6, theVehicle, commandName.." from "..targetPlayerName)

				addVehicleLogs(id, commandName, thePlayer)

				outputChatBox("Player "..targetPlayerName.." teleported to vehicle.", thePlayer, 255, 194, 14)
				if getElementData(thePlayer, "hiddenadmin") == 1 then
					exports["titan_infobox"]:addBox(targetPlayer, "success", "Gizli bir yetkili seni aracının yanına gönderdi.")
				else
					exports["titan_infobox"]:addBox(targetPlayer, "success", exports.titan_global:getPlayerFullIdentity(thePlayer).." isimli yetkili aracının yanına gönderdi.")
				end
			else
				exports["titan_infobox"]:addBox(thePlayer, "error", "Hatalı Araç ID'si girdin.")
			end
...
function getNearbyVehicles(thePlayer, commandName)
	if exports.titan_integration:isPlayerTrialAdmin(thePlayer) then
		...
		for index, nearbyVehicle in ipairs( exports.titan_global:getNearbyElements(thePlayer, "vehicle") ) do
			...
			if thisvehid then
				...
				if faction then
					...
					elseif (owner>0) then
						ownerName = exports['titan_cache']:getCharacterName(owner, true)
					else
						ownerName = "Civilian"
					end
				else
		...
		if not (id) then
			...
			if theVehicle then
				...
				if (dbid<0) then -- TEMP vehicle
					...
					exports.titan_logs:dbLog(thePlayer, 6, theVehicle, "RESPAWN")
	...
	if (exports.titan_integration:isPlayerTrialAdmin(thePlayer)) then
		if commandName then
			if isTimer(respawnTimer) then
				...
				for k, arrayPlayer in ipairs(exports.titan_global:getAdmins()) do
					local logged = getElementData(arrayPlayer, "loggedin")
					if (logged) then
						if exports.titan_integration:isPlayerAdmin(arrayPlayer) then
		...
		local vehicles = getElementsByType("vehicle")--exports.titan_pool:getPoolElementsByType("vehicle")
		local counter = 0
		local tempcounter = 0
					...
					if (dimensions[dbid + 20000]) or (pass1) or (pass2) or (pass3) or (driver) or (getVehicleTowingVehicle(theVehicle)) or #getAttachedElements(theVehicle) > 0 then
						...
						if getElementData(theVehicle, "faction") > 0 then
							...
							if getElementData(theVehicle, "respawnposition") then
								...
								if (round(checkx, 6) == x) and (round(checky, 6) == y) then
									...
									if not getElementData(theVehicle, "carshop") then
										if isElementWithinColShape(theVehicle, wangs1Col) or isElementWithinColShape(theVehicle, wangs2Col) or isElementWithinColShape(theVehicle, wangs3Col) or isElementWithinColShape(theVehicle, bikeCol) then
											...
											call( getResourceFromName( "titan_items" ), "deleteAll", 3, dbid )
											call( getResourceFromName( "titan_items" ), "clearItems", theVehicle )
											exports.titan_logs:dbLog(thePlayer, 6, { theVehicle }, "CarShop Delete" )
											destroyElement(theVehicle)
											deleted = deleted + 1
										else
											counter = counter + 1
								...
								exports.titan_global:sendMessageToAdmins("[RESPAWN-ALL] Vehicle #" .. dbid .. " has not been /park'ed!")
								triggerEvent("onVehicleDelete", theVehicle)
								dbExec(mysql:getConnection(), "UPDATE `vehicles` SET `deleted`='1' WHERE `id`='" .. (dbid) .. "'")
								call( getResourceFromName( "titan_items" ), "deleteAll", 3, dbid )
								call( getResourceFromName( "titan_items" ), "clearItems", theVehicle )
...
function respawnVehiclesStop(thePlayer, commandName)
	if exports.titan_integration:isPlayerTrialAdmin(thePlayer) and isTimer(respawnTimer) then
...
function respawnAllCivVehicles(thePlayer, commandName)
    if exports.titan_integration:isPlayerTrialAdmin( thePlayer ) or exports.titan_integration:isPlayerSupporter(thePlayer) then
        local vehicles = exports.titan_pool:getPoolElementsByType("vehicle")
        local counter = 0
            ...
            if dbid and dbid > 0 then
                if getElementData(theVehicle, "owner") == -2 or getElementData(theVehicle, "owner") == -1  then
                    ...
                    if not pass1 and not pass2 and not pass3 and not driver and not getVehicleTowingVehicle(theVehicle) and #getAttachedElements(theVehicle) == 0 then
                        ...
                        exports.titan_anticheat:changeProtectedElementDataEx(theVehicle, 'i:left')
                        exports.titan_anticheat:changeProtectedElementDataEx(theVehicle, 'i:right')
                        respawnTheVehicle(theVehicle)
                        setVehicleLocked(theVehicle, false)
                        setElementInterior(theVehicle, 0)
                        setElementDimension(theVehicle, 0)
                        exports.titan_anticheat:changeProtectedElementDataEx(theVehicle, "vehicle:radio", 0, true)
                        counter = counter + 1
                        end
                    end
                end
...
function respawnAllInteriorVehicles(thePlayer, commandName, repair)
	local repair = tonumber( repair ) == 1 and exports.titan_integration:isPlayerTrialAdmin( thePlayer )
	local dimension = getElementDimension(thePlayer)
	if dimension > 0 and ( exports.titan_global:hasItem(thePlayer, 4, dimension) or exports.titan_global:hasItem(thePlayer, 5, dimension) ) then
		local vehicles = exports.titan_pool:getPoolElementsByType("vehicle")
		local counter = 0
				...
				if dbid and dbid > 0 then
					...
					if not pass1 and not pass2 and not pass3 and not driver and not getVehicleTowingVehicle(theVehicle) and #getAttachedElements(theVehicle) == 0 then
						...
						if getElementData(theVehicle, "respawnposition") then
							...
							exports.titan_global:sendMessageToAdmins("[Respawn] There's something wrong with vehicle "..dbid)
						end
					end
				end
			end
...
function respawnDistrictVehicles(thePlayer, commandName)
	if exports.titan_integration:isPlayerTrialAdmin( thePlayer ) then
		local zoneName = exports.titan_global:getElementZoneName(thePlayer)
		local vehicles = exports.titan_pool:getPoolElementsByType("vehicle")
		local counter = 0
		local deleted = 0

		for k, theVehicle in ipairs(vehicles) do
			local vehicleZoneName = exports.titan_global:getElementZoneName(theVehicle)
		...
		exports.titan_global:sendMessageToAdmins("AdmWrn: ".. getPlayerName(thePlayer) .." respawned " .. counter .. " and deleted " .. deleted .. " district vehicles in '"..zoneName.."'.", thePlayer)
	end
end
				...
				if not (isPedInVehicle(targetPlayer)) then
					...
					if paintjobID == getVehiclePaintjob(theVehicle) then
						...
						if (success) then
							...
							exports.titan_logs:dbLog(thePlayer, 6, { targetPlayer, theVehicle  }, "PAINTJOB ".. paintjobID )
							...
							exports['titan_vehicle']:saveVehicleMods(theVehicle)
						else
	...
	if (exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer) or exports.titan_integration:isPlayerScripter(thePlayer) or exports.titan_integration:isPlayerVehicleConsultant(thePlayer)) then
		if not (target) then
			...
			if targetPlayer then
				local logged = getElementData(targetPlayer, "loggedin")
				if (logged==0) then
					outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Bu oyuncu giriş yapmamış.", thePlayer, 255, 0, 0, true)
				else
		  ...
		  if (birlik==1) or (birlik==2) and (birlik==3) and (birlik==4) and (birlik==20) and (birlik==50) then
		   ...
		   if (getElementData(veh, "Impounded") == 0) then
						...
						exports.titan_logs:dbLog(thePlayer, 6, { oyuncu, veh  }, "FIXVEH")
					...
					if money < 200 and bankmoney < 200 then
						...
						outputChatBox("#7eff57[-] #f6f6f6Bir yetkili tarafından tamir isteği aldın, onaylamak için /fixonayla yazınız. (200 TL)", targetPlayer, 255, 255, 255, true)
...
local veh = getPedOccupiedVehicle(oyuncu)
		if (veh) then
		 ...
		 exports.titan_global:takeMoney(oyuncu, 200)
		 fixVehicle(veh)
		...
		if not (target) or not (amount) then
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					local veh = getPedOccupiedVehicle(targetPlayer)
					if (veh) then
						if exports["titan_vehicle_fuel"]:getMaxFuel(getElementModel(veh))<amount or amount==0 then
							amount = exports["titan_vehicle_fuel"]:getMaxFuel(getElementModel(veh))
						end
...
function fuelAllVehicles(thePlayer, commandName)
	if (exports.titan_integration:isPlayerTrialAdmin(thePlayer)) then
		local username = getPlayerName(thePlayer)
		for key, value in ipairs(exports.titan_pool:getPoolElementsByType("vehicle")) do
			setElementData(value, "fuel", exports["titan_vehicle_fuel"]:getMaxFuel(getElementModel(value)), false)
		end
		...
		exports.titan_logs:dbLog(thePlayer, 6, { thePlayer  }, "FUELVEHS" )
	end
end
...
function setPlayerVehicleColor(thePlayer, commandName, target, ...)
	if (exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerScripter(thePlayer)) then
		if not tonumber(target) or not (...) then
			...
			local username = getPlayerName(thePlayer)
			for i,c in ipairs(exports.titan_pool:getPoolElementsByType("vehicle")) do
			...
			if theVehicle then
				...
				if set then
					...
					exports['titan_vehicle']:saveVehicleMods(theVehicle)
					exports.titan_logs:dbLog(thePlayer, 6, {  theVehicle  }, "SETVEHICLECOLOR ".. table.concat({...}, " ") )
				...
				if rows > 0 then
					for index, row in ipairs(res) do
						if row["deleted"] == "0" then
							...
							local theVehicle = exports["titan_vehicle"]:loadOneVehicle(dbid, false, true)
							if theVehicle then
...
--								outputDebugString("[VEH MANAGER] Failed to clear vehicle's inventory.")
							...
							if hiddenAdmin == 0 then
								exports.titan_global:sendMessageToAdmins("[VEHICLE]: "..adminTitle.." ".. getPlayerName(thePlayer):gsub("_", " ").. " ("..adminUsername..") has removed vehicle ID: #" .. dbid .. " completely from SQL.")
							else
								exports.titan_global:sendMessageToAdmins("[VEHICLE]: A hidden admin has removed vehicle ID: #" .. dbid .. " completely from SQL.")
							end
							return true
						end
					end
...
function clearVehicleInventory(theVehicle)
	if theVehicle then
		local count = 0
		for key, item in pairs(exports["titan_items"]:getItems(theVehicle)) do
			exports.titan_global:takeItem(theVehicle, item[1], item[2])
			count = count + 1
		end
		return count
	else
...
function restoreVehicle(thePlayer, commandName, id)
	if exports.titan_integration:isPlayerTrialAdmin(thePlayer) then
		...
		if not (dbid) then
			...
			local theVehicle = exports.titan_pool:getElement("vehicle", dbid)
			local adminUsername = getElementData(thePlayer, "account:username")
			local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
			local adminTitle = exports.titan_global:getPlayerAdminTitle(thePlayer)
			...
			if not theVehicle then
				if dbExec(mysql:getConnection(), "UPDATE `vehicles` SET `deleted`='0', `chopped`='0' WHERE `id`='" .. (dbid) .. "'") then
					call( getResourceFromName( "titan_vehicle" ), "loadOneVehicle", dbid )
					outputChatBox("   Restoring vehicle ID #"..dbid.."...", thePlayer)
					setTimer(function()
						outputChatBox("   Restoring vehicle ID #"..dbid.."...Done!", thePlayer)
						local theVehicle1 = exports.titan_pool:getElement("vehicle", dbid)
						exports.titan_logs:dbLog(thePlayer, 6, { theVehicle1 }, "RESTOREVEH" )
						...
						if hiddenAdmin == 0 then
						exports.titan_global:sendMessageToAdmins("[VEHICLE]: "..adminTitle.." ".. getPlayerName(thePlayer):gsub("_", " ").. " ("..adminUsername..") has restore a " .. vehicleName .. " (ID: #" .. dbid .. " - Owner: " .. ownerName..").")
						else
							exports.titan_global:sendMessageToAdmins("[VEHICLE]: A hidden admin has restore a " .. vehicleName .. " (ID: #" .. dbid .. " - Owner: " .. ownerName..").")
						end
					end, 2000,1)
	...
	if exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerScripter(thePlayer) then
		...
		if not (dbid) then
			...
			if theVehicle then
	            ...
	            if exports.titan_data:load(getElementData(thePlayer, "account:id").."/"..commandName) ~= dbid then
	            	local inactiveText = ""
	                local owner_last_login = getElementData(theVehicle, "owner_last_login")
					if owner_last_login and tonumber(owner_last_login) then
						local owner_last_login_text, owner_last_login_sec = exports.titan_datetime:formatTimeInterval(owner_last_login)
					...
					if lastused and tonumber(lastused) then
						local lastusedText, lastusedSeconds = exports.titan_datetime:formatTimeInterval(lastused)
					...
					exports.titan_data:save(dbid, getElementData(thePlayer, "account:id").."/"..commandName)
					return false
	            end
				...
				if (dbid<0) then -- TEMP vehicle
					...
					exports.titan_logs:dbLog(thePlayer, 6, { theVehicle }, "DELVEH" )
					destroyElement(theVehicle)

					if hiddenAdmin == 0 then
						exports.titan_global:sendMessageToAdmins("[VEHICLE]: "..adminTitle.." ".. getPlayerName(thePlayer):gsub("_", " ").. " ("..adminUsername..") has deleted a " .. vehicleName .. " (ID: #" .. dbid .. " - Owner: " .. ownerName..").")
					else
						exports.titan_global:sendMessageToAdmins("[VEHICLE]: A hidden admin has deleted a " .. vehicleName .. " (ID: #" .. dbid .. " - Owner: " .. ownerName..").")
					end
					addVehicleLogs(dbid, commandName, thePlayer)
					...
					for k, theObject in ipairs(getElementsByType("object", getResourceRootElement(getResourceFromName("titan_item-world")))) do
...
function setVehicleFaction(thePlayer, theCommand, vehicleID, factionID)
	if exports.titan_integration:isPlayerSeniorAdmin(thePlayer) then
		if not (vehicleID) or not (factionID) then
			...
			local owner = -1
			local theVehicle = exports.titan_pool:getElement("vehicle", vehicleID)
			local factionElement = exports.titan_pool:getElement("team", factionID)
			if theVehicle then
				...
				exports['titan_vehicle']:reloadVehicle(tonumber(vehicleID))
				local newVehicleElement = exports.titan_pool:getElement("vehicle", vehicleID)
				...
				exports.titan_logs:dbLog(thePlayer, 4, { pveh, theVehicle }, theCommand.." "..factionID)
...
function vergiFiyat(thePlayer)
	...
	local veh = getPedOccupiedVehicle(thePlayer)
		if not veh then
				exports["titan_infobox"]:addBox(thePlayer, "error", "Bu komutu sadece aracın içerisinde kullanabilirsin.")
		return end
			local vehicleID = getElementData(veh, "dbid")
				...
				exports["titan_infobox"]:addBox(thePlayer, "success", "Başarıyla aracın vergi borcunu sıfırladın.")
	end
end
...
function setVehTint(admin, command, target, status)
	if exports.titan_integration:isPlayerHeadAdmin(admin) then
		if not (target) or not (status) then
			outputChatBox("KULLANIM: /" .. command .. " [player] [0- Off, 1- On]", admin, 255, 194, 14)
		else
			local username = getPlayerName(admin):gsub("_"," ")
			local targetPlayer, targetPlayerName = exports.titan_global:findPlayerByPartialNick(admin, target)
			...
			if (targetPlayer) then
				...
				if (pv) then
					...
					if (stat == 1) then
						...
						exports.titan_logs:dbLog(admin, 6, {pv, targetPlayer}, "SETVEHTINT 1" )
					...
					elseif (stat == 0) then
						...
						exports.titan_logs:dbLog(admin, 4, {pv, targetPlayer}, "SETVEHTINT 0" )
...
function setVehiclePlate(thePlayer, theCommand, vehicleID, ...)
	if exports.titan_integration:isPlayerHeadAdmin(thePlayer) or exports.titan_integration:isPlayerLeadScripter(thePlayer) or exports.titan_integration:isPlayerVehicleConsultant(thePlayer) then
		if not (vehicleID) or not (...) then
			outputChatBox("KULLANIM: /" .. theCommand .. " [vehicleID] [Text]", thePlayer, 255, 194, 14)
		else
			local theVehicle = exports.titan_pool:getElement("vehicle", vehicleID)
			if theVehicle then
				--if exports['titan_vehicle']:hasVehiclePlates(theVehicle) then
					local plateText = table.concat({...}, " ")
					if (exports.titan_vehicle_plate:checkPlate(plateText)) then
...
function enterCar(thePlayer, commandName, targetPlayerName, targetVehicle, seat)
	if exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer) then
		...
		if targetPlayerName and targetVehicle then
			...
			if targetPlayer then
				local theVehicle = exports.titan_pool:getElement("vehicle", targetVehicle)
				if theVehicle then
					if seat then
	...
	if not tonumber(seat) then
		...
		if theVehicle then
			...
			if seat <= maxSeats then
				...
				if not occupant then
					if seat == 0 then
						if not getElementData(thePlayer, "license.car.cangetin") and getElementData(theVehicle, "owner") == -2 then -- Fixed your script, Hanz. - Adams
							outputChatBox("(( This DoL Car is for the Driving Test only. ))", thePlayer, 255, 194, 14)
							return false
						end

						local job = getElementData(theVehicle, "job")
						if job ~= 0 then -- Fixed your script, Hanz. - Adams
...
function damageproofVehicle(thePlayer, theCommand, theFaggot)
	if exports.titan_integration:isPlayerSeniorAdmin(thePlayer) then
		if not (theFaggot) then
			...
			local targetPlayer, targetPlayerName = exports.titan_global:findPlayerByPartialNick(thePlayer, theFaggot)
			local targetVehicle = getPedOccupiedVehicle(targetPlayer)
			...
			if targetVehicle then
				...
				if isVehicleDamageProof(targetVehicle) then
					...
					exports.titan_logs:dbLog("ac"..tostring(getElementData(thePlayer, "dbid")), 4, "veh"..vehID, " Removed vehicle damage proof ")
				else
					...
					exports.titan_logs:dbLog("ac"..tostring(getElementData(thePlayer, "dbid")), 4, "veh"..vehID, " Enabled vehicle damage proof ")
				end
			end
		end
	end
