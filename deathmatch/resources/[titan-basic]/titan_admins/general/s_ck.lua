...
function ckPlayer(thePlayer, commandName, targetPlayer, ...)
	if exports.titan_integration:isPlayerGameAdmin(thePlayer) then
		if not (targetPlayer) or not (...) then
			outputChatBox("* "..exports["titan_pool"]:getServerName().." Roleplay * /" .. commandName .. " [Player Partial Nick / ID] [Cause of Death]", thePlayer, 255, 194, 14)
		else
			...
			if targetPlayer then
				...
				elseif (logged==1) then
					info = table.concat({...}, " ")
					local query = dbExec(mysql:getConnection(), "UPDATE `characters` SET `cked`='1', `ck_info`='" .. (tostring(info)) .. "', `death_date`=NOW() WHERE `id` = " .. getElementData(targetPlayer, "dbid")) -- Hanz
					...
					local desc = look[5]
					call( getResourceFromName( "titan_realism" ), "addCharacterKillBody", x, y, z, rotation, skin, getElementData(targetPlayer, "dbid"), targetPlayerName, getElementInterior(targetPlayer), getElementDimension(targetPlayer), getElementData(targetPlayer, "age"), getElementData(targetPlayer, "race"), getElementData(targetPlayer, "weight"), getElementData(targetPlayer, "height"), desc, info, getElementData(targetPlayer, "gender"))
					...
					exports.titan_logs:dbLog(thePlayer, 4, targetPlayer, "CK with reason: "..(tostring(info)))
...
function unckPlayer(thePlayer, commandName, ...)
	if exports.titan_integration:isPlayerGameAdmin(thePlayer) then
		if not (...) then
			outputChatBox("* "..exports["titan_pool"]:getServerName().." Roleplay * /" .. commandName .. " [Full Player Name]", thePlayer, 255, 194, 14)
		else
			...
			elseif (mysql:num_rows(result)==0) then
				...
				--exports.titan_logs:logMessage("[/UNCK] " .. getElementData(thePlayer, "account:username") .. "/".. getPlayerName(thePlayer) .." UNCK'ED ".. targetPlayer , 4)
				exports.titan_logs:dbLog(thePlayer, 4, "ch"..row["id"], "UNCK")
			end
