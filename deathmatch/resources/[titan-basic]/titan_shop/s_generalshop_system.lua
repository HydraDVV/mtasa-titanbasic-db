...
function createShopKeeper(x,y,z,interior,dimension,id,shoptype,rotation, skin, sPendingWage, sIncome, sCapacity, currentCap, sSales, pedName, sContactInfo, faction_belong, faction_access) 
	...
	exports.titan_pool:allocateElement(ped)
	
	if shoptype == 17 then
		setElementData(ped, "customshop", true)
	elseif shoptype == 18 or shoptype == 19 then --Faction Drop NPCs
		exports.titan_anticheat:changeProtectedElementDataEx(ped, "faction_belong", faction_belong, true)
		exports.titan_anticheat:changeProtectedElementDataEx(ped, "faction_access", faction_access, true)
	end 
...
function createGeneralshop(thePlayer, commandName, shoptype, skin, ...)
	if exports.titan_integration:isPlayerLeadAdmin(thePlayer) then
		...
		if not skin or skin == -1 then --Random
			skin = exports.titan_global:getRandomSkin()
		end
		...
		if not pedName or pedName=="" or (tonumber(pedName) and tonumber(pedName) == -1) then
			pedName = exports.titan_global:createRandomMaleName()
		...
		local id = false
		id = dbExec(mysql:getConnection(), "INSERT INTO shops SET pedName='"..exports.titan_global:toSQL(pedName).."', x='" .. (x) .. "', y='" .. (y) .. "', z='" .. (z) .. "', dimension='" .. (dimension) .. "', interior='" .. (interior) .. "', shoptype='" .. (shoptype) .. "', rotationz='" .. (rotation) .. "', skin='".. (skin).."' ")
		dbQuery(
			function(qh)
...
function getInteriorOwner( dimension )
	...
	local dbid, theEntrance, theExit, interiorType, interiorElement = exports["titan_interiors"]:findProperty(source)
...
addEventHandler( "shop:buy", resourceRoot, function( index )
	...
	_G['shop'] = shop
	if not shop then
		outputChatBox("Error " .. error .. "-NE, report at https:/www."..exports["titan_pool"]:getServerName().."roleplay.com.", client, 255, 0, 0 )
		return
	end
	...
	if not item then
		outputChatBox("Error " .. error .. "-NEI-" .. index .. ", report at https:/www."..exports["titan_pool"]:getServerName().."roleplay.com.", client, 255, 0, 0 )
		return
	end
	...
	if not exports.titan_global:hasMoney( client, price ) then
	...
	elseif itemID == 68 then -- Lottery Tickets
		--[[
		if not exports.titan_integration:isPlayerScripter(client) then
		...
		suppliesToTake = item.supplies or math.ceil( 3.5 * exports['titan_items']:getItemWeight( itemID, itemValue ) )
		...
		if not exports["titan_lottery-system"]:canThisPlayerBuyTicket(client) then
		...
		local lotteryJackpot = exports['titan_lottery-system']:getLotteryJackpot()
		if tonumber(lotteryJackpot) == -1 then
			...
			return
		elseif not exports.titan_global:hasSpaceForItem( client, itemID, itemValue ) then
			outputChatBox("Your inventory is full.", client, 255, 0, 0)
		else
			local updatedJackpot = tonumber(lotteryJackpot) + math.ceil(price * 2 / 3)
			exports['titan_lottery-system']:updateLotteryJackpot(updatedJackpot)
			...
			if tonumber(lotteryTicketNumber) == tonumber(exports['titan_lottery-system']:getLotteryNumber()) then
				setTimer(function(player, jp) exports['titan_global']:giveMoney(player, jp) end, 100, 1, client, updatedJackpot)
				outputChatBox( "You won! Jackpot: $" .. exports.titan_global:formatMoney(updatedJackpot) .. ".", client, 0, 255, 0 )

				exports['titan_lottery-system']:lotteryDraw()

				for key, value in ipairs(exports.titan_pool:getPoolElementsByType("player")) do
					if (getElementData(value, "loggedin")==1) then
						outputChatBox("[NEWS] " .. getPlayerName(client):gsub("_"," ") .. " won the lottery jackpot of $" .. exports.titan_global:formatMoney(updatedJackpot) .. ".", value, 200, 100, 200)
					end
				end
				exports['titan_lottery-system']:updateLotteryJackpot(-1)
				-- Timer to re-enable lottery 10 minutes after a ticket has been drawn.
				setTimer(function ()
					exports['titan_lottery-system']:updateLotteryJackpot(0)
				end, 600000, 1)
		...
		if item.license and getElementData( client, "license.gun" ) ~= 1 then
			...
			if itemID == 115 then
				...
				if item.license then -- licensed weapon, thus needs a serial
					...
					serial = exports.titan_global:createWeaponSerial( 3, characterDatabaseID, characterDatabaseID )
				end
...
--				addPurchaseLogs(tonumber(getElementData(source, "dbid")), client, tostring( getWeaponNameFromID( w ) ), itemValue, price, serial, "N/A", FORUM_AMMUNATION)
			elseif itemID == 116 then
				local amount = item.ammo or exports.titan_weaponcap:getGTACap( itemValue ) or 1
	...
	if dimension > 0 then -- is even in any interior -- calculate the supplies amount /Hanz
		--suppliesToTake = item.supplies or math.ceil(item.price/(profitRate*2)) or math.ceil( 3.5 * exports['titan_items']:getItemWeight( itemID, itemValue ) )
		suppliesToTake = item.supplies or math.ceil( 3.5 * exports['titan_items']:getItemWeight( itemID, itemValue ) )
	...
	if wonTheLottery or exports.titan_global:giveItem( client, itemID, itemValue ) then
		-- Money
		local playerMoney = getElementData(client, "money")
		for i = 134, 134 do
			exports['titan_items']:takeItem(client, i)
		end
		if tonumber(playerMoney) > 0 then
			exports.titan_global:giveItem(client, 134, tonumber(playerMoney)-tonumber(price))
		end
		exports.titan_global:takeMoney( client, price ) -- this is assumed not to fail as we checked with :hasMoney before.
		-- and now for what happens after buying?
		--outputChatBox( "You bought this " .. item.name .. " for $" .. exports.titan_global:formatMoney( price ) .. ".", client, 0, 255, 0 )
		exports["titan_infobox"]:addBox(client, "buy", "Başarıyla " ..item.name.." adlı ürünü "..exports.titan_global:formatMoney( price ) .." TL'ye satın aldınız.")
		...
		elseif itemID == 115 then -- log weapon purchases
			exports.titan_logs:dbLog( client, 36, client, "bought WEAPON - " .. itemValue )
			
			local govMoney = math.floor( price / 2 )
			exports.titan_global:giveMoney(getTeamFromName("Government of Los Santos"), govMoney)
			price = price - govMoney -- you'd obviously get less if the gov asks for percentage.
		elseif itemID == 116 then -- log weapon purchases
			exports.titan_logs:dbLog( client, 36, client, "bought AMMO - " .. itemValue )
		...
		if price > 0 and dimension > 0 then
			...
			local ownerNoti = "A customer bought a "..item.name.." for $"..exports.titan_global:formatMoney(price).."."
			local logString = "- "..timeString.." : A customer bought a "..item.name.." for $"..exports.titan_global:formatMoney(price)..".\n"
			
			exports.titan_global:sendLocalText(client, "* "..pedName.." gave "..playerName.." a "..item.name..".", 255, 51, 102, 30, {}, true)
