...
function toggleRecon(state, targetPlayer)
	if state then
		local cur = exports.titan_data:load("reconCurpos")
		if not cur then
			cur = {}
		...
		exports.titan_data:save(cur, "reconCurpos")
		...
		if cur then
			...
			exports.titan_data:save(nil, "reconCurpos")
			reconTarget = nil
...
function reconPlayer(commandName, targetPlayer)
	...
	if getElementData(localPlayer, "loggedin") == 1 and exports.titan_integration:isPlayerAdminI(localPlayer) then
		local reconx = getElementData(localPlayer, "reconx")
		if not (targetPlayer) then
			if not reconx then
				return outputChatBox("|| "..exports["titan_pool"]:getServerName().." Roleplay || /" .. commandName .. " [Player Partial Nick]", 255, 194, 14)
			end
			...
			local targetPlayer, targetPlayerName = exports.titan_global:findPlayerByPartialNick(localPlayer, targetPlayer)
			if not targetPlayer then 
			...
			if exports.titan_freecam:isEnabled (localPlayer) then
				exports.titan_freecam:toggleFreecam("dropme")
			end

			if toggleRecon(true, targetPlayer) then
			
			if exports.titan_integration:isPlayerDeveloper(targetPlayer) then
				if exports.titan_integration:isPlayerDeveloper(localPlayer) then
				toggleRecon(true, targetPlayer)
				else
				outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#ffffff Bu adamı izleyecek yüreğin yok.", 255, 0, 0, true)
...
function displayReconInfo()
    ...
    dxDrawText("Target: "..exports.titan_global:getPlayerFullIdentity(reconTarget,3,true), 517+xo, 406+yo, 887+xo, 423+yo, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, true, false, false)
    ...
    dxDrawText("Money: $"..exports.titan_global:formatMoney(getElementData(reconTarget, "money")), 517+xo, 457+yo, 706+xo, 474+yo, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, true, false, false)
    dxDrawText("Bank: $"..exports.titan_global:formatMoney(getElementData(reconTarget, "bankmoney")), 706+xo, 457+yo, 887+xo, 474+yo, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, true, false, false)
    ...
    if vehicle then
    	veh = "Vehicle: " .. exports.titan_global:getVehicleName( vehicle ) .. " (" ..getVehicleName( vehicle ).." - ID #"..getElementData( vehicle, "dbid" ) .. " - HP: "..math.floor( getElementHealth( vehicle ))..")"
    end
...
local function getTarget(order)
	...
	if order == "arrow_r" then
		...
		if not reconTargets[pointer] then
			pointer = 1
			local target = exports.titan_global:findPlayerByPartialNick(localPlayer, reconTargets[pointer])
			if not target then
