...
reportTypes = {
	...
	--{"Mapping Issue", {44, 28}, "MAP", 0, 0, 0 }, Hanz IF YOU EVER WANT TO BRING THIS BACK, UNCOMMENT
	--{"Scripting Question", {32}, "ScrT", 148, 126, 12, "Use this type if you with to contact the Scripting Team." },
}

adminTeams = exports.titan_integration:getAdminStaffNumbers()
auxiliaryTeams = exports.titan_integration:getAuxiliaryStaffNumbers()
SUPPORTER = exports.titan_integration:getSupporterNumber()
	...
	for k,v in ipairs(staff) do
		if v == 39 or v == 43 then
			...
			for key, player in ipairs(players) do
				if exports.titan_integration:isPlayerVCTMember(player) or exports.titan_integration:isPlayerVehicleConsultant(player) then
					vcount = vcount + 1
					save = player
				end
			end
			...
			if vcount==0 then
				return false, "Aktif araç yetkilisi mevcut değil. UCP'den şikayet açınız. https:/www."..exports["titan_pool"]:getServerName().."roleplay.com/ucp"
		...
		elseif v == 32 then
			...
			if scount==0 then
				return false, "Aktif yazılımcı mevcut değil. UCP'den şikayet açınız. https:/www."..exports["titan_pool"]:getServerName().."roleplay.com/ucp"
...
function showExternalReportBox(thePlayer)
	...
	return (exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer)) and (getElementData(thePlayer, "report_panel_mod") == "2" or getElementData(thePlayer, "report_panel_mod") == "3")
end

function showTopRightReportBox(thePlayer)
	if not thePlayer then return false end
	return (exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer)) and (getElementData(thePlayer, "report_panel_mod") == "1" or getElementData(thePlayer, "report_panel_mod") == "3")
end