...
function removeOOC(text)
	if not text then
		outputChatBox("Error code 0001 occurs, please report to "..exports["titan_pool"]:getServerName().." Roleplay MTA Scripting Team at https:/www."..exports["titan_pool"]:getServerName().."roleplay.com/ucp")
		return ""
	end
	...
	if itemID then
		...
		elseif (itemID==3) then -- car key
			...
			if veh and getElementData(veh, "dbid") == itemValue then
				...
				-- unlock nearby cars
				local value = exports.titan_pool:getElement( "vehicle", itemValue )
				if value then
		...
		elseif (itemID==4) or (itemID==5) then -- house key or business key
			...
			if found then
				local dbid, entrance, exit, interiorType, interiorElement = exports['titan_interiors']:findProperty( source, found )
				...
				if locked == 0 then
					...
					if not (exports.titan_global:hasItem(source, 4, found)) and not (exports.titan_global:hasItem(source, 5, found)) then
						exports.titan_logs:logMessage("[INTERIOR-UNLOCK] Interior #" .. found .. " was unlocked by " .. getPlayerName(source), 21)
					end
				else
					...
					if not (exports.titan_global:hasItem(source, 4, found)) and not (exports.titan_global:hasItem(source, 5, found)) then
						exports.titan_logs:logMessage("[INTERIOR-LOCK] Interior #" .. found .. " was locked by " .. getPlayerName(source), 21)
					end
				end
		...
		exports["titan_donate"]:silahKasa(source)
		elseif (itemID==583) then
		exports["titan_donate"]:premiumKasa(source)		

		elseif (itemID==8) then -- sandwich
			giveHunger(source, 70)
			takeThirst(source, 7)
			
			exports.titan_global:applyAnimation(source, "food", "eat_burger", 4000, false, true, true)
		...
		elseif (itemID==9) then -- sprunk
			...
			exports.titan_global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			toggleAllControls(source, true, true, true)
			triggerClientEvent(source, "onClientPlayerWeaponCheck", source)
		...
		elseif (itemID==10) then -- red dice
			...
			exports.titan_global:sendLocalText(source, " ✪ " .. getPlayerName(source):gsub("_", " ") .. " bir " .. (max == 6 and "" or (max .. "-yüzlü ")) .. " zar atar ve '" .. math.random( 1, max ) .."' sayısı gelir.", 255, 51, 102)
		...
		elseif (itemID==16) then -- clothes
			...
			--if fittingskins[gender] and fittingskins[gender][race] and fittingskins[gender][race][skin] then
				...
				exports.titan_anticheat:changeProtectedElementDataEx(source, "clothing:id", clothingid, true)
		...
		elseif (itemID==17) then -- watch
			...
			exports.titan_global:applyAnimation(source, "COP_AMBIENT", "Coplook_watch", 4000, false, true, true)
		...
		elseif (itemID==28) then -- GLOWSTICK
			...
			exports.titan_pool:allocateElement(marker)
			...
			--if (getElementData(source, "duty") > 0) and (getElementModel(source) == 285) then
				...
				if not (found) then
					...
					elseif (interiorStatus[3]) then
						...
						exports.titan_anticheat:changeProtectedElementDataEx(found, "status", interiorStatus, true)
						...
						exports.titan_global:sendMessageToAdmins("Player "..getPlayerName(source).." opened interior "..dbid.." with a doorram")
						exports.titan_logs:dbLog(source, 31, { source, found }, "UNLOCK WITH DOORRAM")
					else
                ...
                if sucess then
                    if (tonumber(itemvalue) >= 1) then
                        if (hasSpaceForItem(source, 182, 1)) then
                            ...
                            exports.titan_global:sendLocalMeAction(source, "grabs a marijuana bud out of the bag, begining to roll the joint afterwards.")
		...
		elseif (itemID==57) then -- FUEL CAN
			local nearbyVehicles = exports.titan_global:getNearbyElements(source, "vehicle")
		...
		elseif (itemID==60) then
			...
			local dimension = getElementDimension(source)
			local retval = call(getResourceFromName("titan_interiors"), "addSafeAtPosition", source, x,y,z, rz) --0 no error, 1 safe already exists, 2 player does not own interior
			if (retval == 0) then
		...
		elseif (itemID==69) then -- Dictionary
			local learned = call(getResourceFromName("titan_languages"), "learnLanguage", source, itemValue, true)
			local lang = call(getResourceFromName("titan_languages"), "getLanguageName", itemValue)
		...
		elseif (itemID==75) then
			...
			for k, v in pairs( exports.titan_global:getNearbyElements(source, "vehicle", 200) ) do
			...
			for k, v in pairs( exports.titan_global:getNearbyElements(source, "player", 200) ) do
				if hasItem( v, 74, itemValue ) then
					local x, y, z = getElementPosition( v )
					createExplosion( x, y, z, 10, source )
					createExplosion( x, y, z, 10, source )
		...
		elseif (itemID==77) then -- Card Deck
			...
			exports.titan_global:sendLocalText(source, " ✪ " .. getPlayerName(source):gsub("_", " ") .. " bir kart çeker ve kart ".. sign[snumber] .. ( number == 1 and "n" or "" ) .. " " .. cards[number] .." gelmiştir.", 255, 51, 102)
		elseif (itemID==79) then -- Porn tape
			exports.titan_global:applyAnimation( source, "PAULNMAC", "wank_loop", -1, true, false, false)
		...
		elseif (itemID==104) then -- portable TV
			...
			if isTvUsed == nil or isTvUsed == false then
				...
				exports.titan_anticheat:changeProtectedElementDataEx(source, "isTvUsed", true, false)
			else
				exports.titan_anticheat:changeProtectedElementDataEx(source, "isTvUsed", false, false)
			end
		...
		elseif itemID == 134 then
			outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Cüzdanında toplam " .. exports.titan_global:formatMoney(itemValue) .. " TL paran var.", source, 255, 0, 255, true)
		...
		elseif masks[itemID] then
			...
			if (itemID == 56 and tonumber(getElementData(source, "vipver")) <= 0) then
				outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffffBu öğeyi sadece VIP'ler veya İEM üyeleri kullanabilir. VIP satın almak için ( /market )", source, 255, 0, 0, true)
				return false
			end
			local mask = getElementData(source, data[1])
			if (itemID == 26 and tonumber(getElementData(source, "vipver")) <= 0) then
				outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffffBu öğeyi sadece VIP'ler veya İEM üyeleri kullanabilir. VIP satın almak için ( /market )", source, 255, 0, 0, true)
				return false
			end
			...
			if mask then
				...
				exports.titan_anticheat:changeProtectedElementDataEx(source, data[1], false, true)
			...
			elseif getPedOccupiedVehicle(source) and getElementData(getPedOccupiedVehicle(source), "job") > 0 and getVehicleOccupant(getPedOccupiedVehicle(source)) == source then
				...
				exports.titan_anticheat:changeProtectedElementDataEx(source, data[1], true, true)

				if itemID == 90 then -- Helmet
					if getElementData(source, "bikerhelmet") then
						exports.titan_anticheat:changeProtectedElementDataEx(source, "bikerhelmet", false, true)
						triggerEvent("artifacts:remove", source, source, "bikerhelmet")
					end
					if getElementData(source, "fullfacehelmet") then
						exports.titan_anticheat:changeProtectedElementDataEx(source, "fullfacehelmet", false, true)
						triggerEvent("artifacts:remove", source, source, "fullfacehelmet")
					end
				...
				elseif itemID == 171 then -- Biker Helmet
					if getElementData(source, "helmet") then
						exports.titan_anticheat:changeProtectedElementDataEx(source, "helmet", false, true)
						triggerEvent("artifacts:remove", source, source, "helmet")
					end
					if getElementData(source, "fullfacehelmet") then
						exports.titan_anticheat:changeProtectedElementDataEx(source, "fullfacehelmet", false, true)
		...
		elseif itemID == 580 then
			outputChatBox(""..exports["titan_pool"]:getServerName()..":#f9f9f9 Tamir kiti geçici olarak devre dışı bırakılmıştır.", source, 255, 0, 0, true)
		--[[
			local itemValue = tonumber(itemValue)
			local veh = getPedOccupiedVehicle(source)
			...
			if source.vehicle then
				...
				if itemValue >= 1 then
					...
					--outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#ffffff Aracınızı başarıyla tamir ettirdiniz.", source, 0, 255, 0, true)
				else
					takeItemFromSlot(source, itemSlot)
				end
			end
    	...
    	elseif (itemID == 182) then
            if hasItem( source, 107 ) then -- If da player has a lighter, how else would he smoke it dawg
                exports.titan_global:sendLocalMeAction(source, "lights up a rolled joint.")
                ...
                exports.titan_global:sendLocalMeAction(source, "shows everyone a rolled joint.")
		...
		elseif (itemID==210) then -- Coca-Cola Christmas Edition
			...
			exports.titan_bone_attach:attachElementToBone(bottleObj,source,11,0,0,0,0,0,0)
			thePlayer = source
			setTimer(function()
				exports.titan_global:applyAnimation(thePlayer, "VENDING", "VEND_Drink_P", 4000, false, true, true)
				toggleAllControls(thePlayer, true, true, true)
				triggerClientEvent(thePlayer, "onClientPlayerWeaponCheck", thePlayer)
				exports.titan_global:sendLocalMeAction(thePlayer, "drinks a coke.")
		...
		elseif (itemID==213) then --pinnekjott
			...
			takeItemFromSlot(source, itemSlot)
			--local thisAnimFunc = exports.titan_global:applyAnimation
			setTimer(function()
				exports.titan_global:applyAnimation(source, "BSKTBALL", "BBALL_Jump_Shot", 4000, false, true, true)
			end, 5000, 1)
			if getResourceState(getResourceFromName("titan_xmas")) == "running" then
		...
		elseif (itemID>=452) and (itemID<=503) then
			exports["titan_wearable-system"]:triggerWearableSystem(source, itemID, itemSlot)
			...
			if not (getElementData(source, "Atatürk Bayrağı")) then
				...
				if texture then
					for k,v in ipairs(texture) do
						exports["titan_item-texture"]:addTexture(bayrak, v[2], v[1])
					end
				end
				--exports.titan_bone_attach:attachElementToBone(bayrak, source, 11, 0, 0, 0, 0, 180, 100)
				exports.titan_bone_attach:attachElementToBone(bayrak, source, 12, 0, 0, 0, 0, 200, 30)
				...
				exports.titan_bone_attach:detachElementFromBone(bayrak)
		...
		if(hasItem) then
			...
			if badges[itemID] then
				if ( getElementData( source, badges[itemID][1] ) ) then
					exports.titan_anticheat:changeProtectedElementDataEx(source, badges[itemID][1], false, true)
					if pedname then
						--exports.titan_global:sendLocalMeAction(source, "removes " .. badges[itemID][2] .. ".")
						exports.titan_global:sendLocalText(source, "* "..tostring(pedname)..", "..badges[itemID][2].." isimli şeyi çıkartır.", 255, 51, 102, 5)
					end
				else
					for key, badge in pairs ( badges ) do
						if key ~= itemID then
							if ( getElementData ( source, badge[1] ) ) then
								exports.titan_anticheat:changeProtectedElementDataEx( source, badge[1], false, true )
								if pedname then
									--exports.titan_global:sendLocalMeAction( source, "removes " .. badge[2] .. "." )
									exports.titan_global:sendLocalText(source, "* "..tostring(pedname)..", "..badge[2].." isimli şeyi çıkartır.", 255, 51, 102, 5)
								end
							end
						end
					end

					exports.titan_anticheat:changeProtectedElementDataEx( source, badges[itemID][1], removeOOC(tostring(itemValue)), true )
					if pedname then
						--exports.titan_global:sendLocalMeAction( source, "puts on " .. badges[itemID][2] .. "." )
						exports.titan_global:sendLocalText(source, "* "..tostring(pedname).." puts on "..badges[itemID][2]..".", 255, 51, 102, 5)
					end
					return true
				end
			...
			elseif masks[itemID] then
				...
				if mask then
					--exports.titan_global:sendLocalMeAction(source, data[3] .. ".")
					exports.titan_anticheat:changeProtectedElementDataEx(source, data[1], false, true)
					if pedname then
						exports.titan_global:sendLocalText(source, "* "..tostring(pedname).." "..data[3]..".", 255, 51, 102, 5)
					end
					return false
				elseif getPedOccupiedVehicle(source) and getElementData(getPedOccupiedVehicle(source), "job") > 0 and getVehicleOccupant(getPedOccupiedVehicle(source)) == source then
					--outputChatBox("You can't use this in a Civilian vehicle.", source, 255, 0, 0)
					...
					--exports.titan_global:sendLocalMeAction(source, data[2] ..".")
					exports.titan_anticheat:changeProtectedElementDataEx(source, data[1], true, true)
					if pedname then
						exports.titan_global:sendLocalText(source, "* "..tostring(pedname).." "..data[2]..".", 255, 51, 102, 5)
					end
					return true
				end
			else
...
function explodeFlash(obj)
	local players = exports.titan_global:getNearbyElements(obj, "player")
...
function destroyItem(itemID)
	...
	if itemID and itemID > 0 then
		...
		if itemID == -100 then
			...
			exports.titan_global:takeWeapon(source, tonumber(-itemID))
			itemName = getWeaponNameFromID( -itemID )
		end
	end
	outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 " .. itemName .. " isimli nesneyi çöp kutusuna bıraktın.", source, 255, 194, 14, true)
...
function isWatchingTV( player )
	return exports['titan_freecam']:isPlayerFreecamEnabled( player )
end
...
function toggleBadge(source, badges, itemID , itemValue)
	if ( getElementData( source, badges[itemID][1] ) ) then
		...
		if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 or itemID == 158 or itemID == 168 then
		exports.titan_anticheat:changeProtectedElementDataEx( source, badges[itemID][1], true )
		triggerEvent('sendAme',  source, "" .. badges[itemID][1] .. " kafasına giydirir." )
		else
			if getElementData(source, "spamblock") then outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 İşlem yaparken biraz yavaş olmalısın. (15 sn)", source, 255, 0, 0, true) return end
			exports.titan_anticheat:changeProtectedElementDataEx( source, badges[itemID][1], removeOOC(itemValue), true )
...
function christmasCoke(thePlayer)
	...
	if(#santaCars > 0) then
		...
		for k,v in ipairs(santaCars) do
			...
			if math.random(100) >= thisChance then --if no car won
				...
				exports.titan_global:sendLocalMeAction(thePlayer, "finds a car key in "..tostring(gendertext).." christmas coca-cola bottle.")
	...
	if math.random(100) >= 40 then
		...
		exports.titan_global:sendLocalMeAction(thePlayer, "finds a lottery ticket in "..tostring(gendertext).." christmas coca-cola bottle.")
	end
end

function christmasLotteryUseTicket(santa)
	local thePlayer = client
	local hasItem, itemSlot, itemValue = exports.titan_global:hasItem(thePlayer, santaLotteryTicketID)
	if hasItem then
		...
		if prizes[prize] then
			...
			exports.titan_global:sendLocalMeAction(santa, "sağ elinde bulunan "..tostring(itemName).." isimli öğeyi "..tostring(playerName).." kişisine verir.")
		else
...
function christmasGetCokeFromSanta(santa)
	...
	local playerName = getPlayerName(thePlayer):gsub("_", " ")
	exports.titan_global:sendLocalMeAction(santa, "gives "..tostring(playerName).." a coke.")
end
...
function istiridye(player)
	if exports.titan_global:takeItem(player, 555) then
		local itemName = ""
		local itemID
		local randomNumber = math.random(0,9)
		exports.titan_global:sendLocalMeAction(player, "yavaşça sağ eliyle istiridyeyi açar.")
		
		setTimer(function()
			if ((randomNumber==1) or (randomNumber==4) or (randomNumber == 5)) then
				exports.titan_global:sendLocalDoAction(player, "Elimdeki istiridyemin içerisinden hiç bir şey bulamadım.")
			elseif ((randomNumber==2) or (randomNumber==8)) then
				itemName = "Yeşil İnci"
				itemID = 558
				exports.titan_global:sendLocalDoAction(player, "Elimdeki istiridyenin içerisinde "..itemName.." bulunmaktadır.")
				exports.titan_global:giveItem(player, itemID, 1)	
			...
			elseif (randomNumber==8) then
				...
				outputChatBox(""..exports["titan_pool"]:getServerName()..":#f9f9f9 İstiridyenin içerisi boş çıktı.", player, 75, 75, 75, true)
			end
		end, 1000, 1)
	else
		outputChatBox(""..exports["titan_pool"]:getServerName()..": #f9f9f9Bu komutu kullanabilmek için bir adet istiridyeye ihtiyacın var.", player, 75, 75, 75, true)
	end
end
