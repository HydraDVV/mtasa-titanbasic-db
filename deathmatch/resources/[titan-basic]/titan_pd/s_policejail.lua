...
function destroyJailTimer ( ) -- 0001290: PD /release bug
	...
	if (isTimer(theMagicTimer)) then
		...
		exports['titan_anticheat']:changeProtectedElementDataEx(source, "pd.jailserved", 0, false)
		exports['titan_anticheat']:changeProtectedElementDataEx(source, "pd.jailtime", 0, false)
		exports['titan_anticheat']:changeProtectedElementDataEx(source, "pd.jailtimer", false, false)
		exports['titan_anticheat']:changeProtectedElementDataEx(source, "pd.jailstation", false, false)
	end
end
	...
	if (logged==1) then
		...
		if (factionType==2) and playerCol then
			if not (targetPlayerNick) or not (fine) or not (jailtime) or not (...) or (jailtime<1) or (jailtime>1441) then
				outputChatBox("KULLANIM: /arrest [Player Partial Nick / ID] [Fine] [Jail Time (Minutes 1->1440)] [Crimes Committed]", thePlayer, 255, 194, 14)
			else
				local targetPlayer, targetPlayerNick = exports.titan_global:findPlayerByPartialNick(thePlayer, targetPlayerNick)
				...
				if targetPlayer then
					...
					elseif targetCol ~= playerCol then
						...
						if (jailTimer) then
							...
							local targetPlayerhasmoney = exports.titan_global:getMoney(targetPlayer, true)
							local amount = tonumber(fine)
							if not exports.titan_global:takeMoney(targetPlayer, amount) then
								finebank = true
								exports.titan_global:takeMoney(targetPlayer, targetPlayerhasmoney)
								local fineleft = amount - targetPlayerhasmoney
								local bankmoney = getElementData(targetPlayer, "bankmoney")
								exports['titan_anticheat']:changeProtectedElementDataEx(targetPlayer, "bankmoney", bankmoney-fineleft, false)
							end
						
							exports['titan_anticheat']:changeProtectedElementDataEx(targetPlayer, "pd.jailserved", 0, false)
							exports['titan_anticheat']:changeProtectedElementDataEx(targetPlayer, "pd.jailtime", jailtime+1, false)
							...
							if restrainedObj then
								...
								exports.titan_global:giveItem(thePlayer, restrainedObj, 1)
							...
							exports['titan_anticheat']:changeProtectedElementDataEx(targetPlayer, "pd.jailstation", targetCol, false)
							...
							exports.titan_logs:logMessage("[PD/ARREST] ".. factionRankTitle .. " " .. username .. " jailed " .. targetPlayerNick .. " for " .. jailtime .. " minute(s). Reason: "..reason , 30)
						end
					end
				end
			end
...
function timerPDUnjailPlayer(jailedPlayer)
	if(isElement(jailedPlayer)) then
		...
		if ( timeServed ) then
			exports['titan_anticheat']:changeProtectedElementDataEx(jailedPlayer, "pd.jailserved", tonumber(timeServed)+1, false)
			local timeLeft = timeLeft - 1
			exports['titan_anticheat']:changeProtectedElementDataEx(jailedPlayer, "pd.jailtime", timeLeft, false)
			...
			if (timeLeft<=0) then
				...
				exports['titan_anticheat']:changeProtectedElementDataEx(jailedPlayer, "pd.jailserved", 0, false)
				exports['titan_anticheat']:changeProtectedElementDataEx(jailedPlayer, "pd.jailtime", 0, false)
				exports['titan_anticheat']:changeProtectedElementDataEx(jailedPlayer, "pd.jailtimer", false, false)
				exports['titan_anticheat']:changeProtectedElementDataEx(jailedPlayer, "pd.jailstation", false, false)
			...
			elseif (timeLeft>0) then
				...
				exports['titan_anticheat']:changeProtectedElementDataEx(jailedPlayer, "pd.jailtimer", theTimer, false)
			end
		end
	end
end
...
function jailRelease(thePlayer, commandName, targetPlayerNick)
	...
	if (logged==1) then
		...
		if factionType == 2 and isInArrestColshape(thePlayer) or exports.titan_global:isPlayerAdmin(thePlayer) and getElementData(thePlayer, "adminduty") == 1 then
			if not (targetPlayerNick) then
				...
				if targetPlayer then
					...
					if (jailTimer) then
						exports['titan_anticheat']:changeProtectedElementDataEx(targetPlayer, "pd.jailtime", 1, false)
						timerPDUnjailPlayer(targetPlayer)
						exports.titan_logs:logMessage("[PD/RELEASE]" .. username .. " released "..targetPlayerNick , 30)
					else
