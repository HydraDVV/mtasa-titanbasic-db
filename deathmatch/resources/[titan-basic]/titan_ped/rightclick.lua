...
function clickPed(button, state, absX, absY, wx, wy, wz, element)
	...
	if (element) and (getElementType(element)=="ped") and (button=="right") and (state=="down") and (sent==false) and (element~=getLocalPlayer()) then
		...
		if (interact and interact ~= "false") then
			...
			if (getDistanceBetweenPoints3D(x, y, z, wx, wy, wz)<=3) then
				...
				if(interact == "ch.reception") then
					rcMenu = exports.titan_rightclick:create(getElementData(element, "rpp.npc.name"))
					row.talk = exports.titan_rightclick:addrow("Talk")
				...
				elseif(interact == "election") then --Election ped
					rcMenu = exports.titan_rightclick:create(getElementData(element, "rpp.npc.name"))
					row.talk = exports.titan_rightclick:addrow("Vote")
					addEventHandler("onClientGUIClick", row.talk,  function (button, state)
				...
				elseif(interact == "mission.clarice") then
					...
					row.util = exports.titan_rightclick:addrow("WooHoo_clarice")
				...
				elseif(interact == "santa") then
					rcMenu = exports.titan_rightclick:create(getElementData(element, "rpp.npc.name"))
					row.get = exports.titan_rightclick:addrow("Get Coke")
					...
					if exports.titan_global:hasItem(getLocalPlayer(), 211) then --has christmas lottery ticket
						row.claim = exports.titan_rightclick:addrow("Claim Prize")
				...
				elseif(interact == "pilotmission") then
					rcMenu = exports.titan_rightclick:create(getElementData(element, "rpp.npc.name"))
					row.getlisting = exports.titan_rightclick:addrow("Get listing")
					addEventHandler("onClientGUIClick", row.getlisting, function ()
							triggerEvent("pilotmission:startGUI", getResourceRootElement( getResourceFromName("titan_sfia") ))
						end, false)
					local temp_table = getElementData(getResourceRootElement( getResourceFromName("titan_sfia") ), "sfia_pilots:table")
					local name = string.gsub(getPlayerName(getLocalPlayer()), "_", " ")
					for k, v in pairs(temp_table) do
						if v["charactername"] == name then
							row.gettask = exports.titan_rightclick:addrow("Get task")
							addEventHandler("onClientGUIClick", row.gettask, function ()
									triggerEvent("pilotmission:domission", getResourceRootElement( getResourceFromName("titan_sfia") ))
								end, false)
							break
						end
					end
				...
				if stretcherElement then
					...
					if stretcherPlayer and stretcherPlayer == player then
						...
						row.stretcher = exports.titan_rightclick:addrow("Take from stretcher")
					...
					if not stretcherPlayer then
						...
						row.stretcher = exports.titan_rightclick:addrow("Lay on stretcher")
						addEventHandler("onClientGUIClick", row.stretcher, function (button, state)
				...
				if(getElementHealth(element) < 50) then
					...
					if(factionType == 4) then --if player is in ES faction and has a first aid kit
						...
						row.heal = exports.titan_rightclick:addrow("Heal")
		...
		if (getElementData(element, "rpp.npc.dbid")) then
			if exports.titan_integration:isPlayerScripter(localPlayer) then
				...
				--if(getElementData(element, "rpp.npc.spawnpos")) then
					if not rcMenu then
						rcMenu = exports.titan_rightclick:create(getElementData(element, "rpp.npc.name") or "NPC")
					end
					--if getElementData(element, "dbid") then
						row.admEdit = exports.titan_rightclick:addrow("ADM: Edit")
						addEventHandler("onClientGUIClick", row.admEdit, function (button, state)
							adminEditPedGui(element)
						end, false)
					--end

					row.respawn = exports.titan_rightclick:addrow("ADM: Respawn")
			...
			--[[if (exports.titan_integration:isPlayerSeniorAdmin(localPlayer) and exports.titan_global:isStaffOnDuty(localPlayer)) or exports.titan_integration:isPlayerScripter(localPlayer) then
				local name = getElementData(element, "rpp.npc.name")
				if not rcMenu then
					rcMenu = exports.titan_rightclick:create(name or "NPC")
				end
				row.delete = exports.titan_rightclick:addrow("ADM: Temporary delete ped")
