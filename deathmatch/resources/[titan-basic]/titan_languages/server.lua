	...
	if (hasLanguage) then
		...
		if currSkill < 100 then
			...
			if chance == 1 then
				...
				exports.titan_anticheat:changeProtectedElementDataEx(player, "languages.lang" .. slot .. "skill", currSkill+1, false)
...
function removeLanguage(player, language)
	...
	if (hasLanguage) then
		...
		exports.titan_anticheat:changeProtectedElementDataEx(player, "languages.lang" .. slot, 0, false)
		exports.titan_anticheat:changeProtectedElementDataEx(player, "languages.lang" .. slot .. "skill", 0, false)
...
function learnLanguage(player, lang, showmessages, skill)
	...
	if (hasLanguage) then
		...
		if skill then
			exports.titan_anticheat:changeProtectedElementDataEx(player, "languages.lang" .. slot .. "skill", skill, false)
		...
		if (freeslot==0) then
			...
			exports.titan_anticheat:changeProtectedElementDataEx(player, "languages.lang" .. freeslot, lang, false)
			exports.titan_anticheat:changeProtectedElementDataEx(player, "languages.lang" .. freeslot .. "skill", skill or 0, false)
...
function useLanguage(lang)
	...
	if (hasLanguage) then
		...
		exports.titan_anticheat:changeProtectedElementDataEx(source, "languages.current", slot, false)
...
function unlearnLanguage(lang)
	...
	if (hasLanguage) then
		...
		exports.titan_anticheat:changeProtectedElementDataEx(source, "languages.current", nextSlot, false)
...
for key, value in pairs( languages ) do
	function speakLang( thePlayer, commandName, ... )
		if doesPlayerHaveLanguage( thePlayer, key ) then
			call( getResourceFromName( "titan_chat" ), "localIC", thePlayer, table.concat({...}, " "), key )
		else
			outputChatBox( "#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Sen " .. value .. " dilini konuşmayı bilmiyorsun.", thePlayer, 255, 0, 0, true )
		end
	end
