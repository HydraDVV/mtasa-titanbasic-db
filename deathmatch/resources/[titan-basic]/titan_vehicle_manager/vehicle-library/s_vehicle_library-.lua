...
function refreshCarShop()
	local theResource = getResourceFromName("titan_vehicle_shop")
	if theResource then
		...
		if getResourceState(theResource) == "running" then
			...
			if hiddenAdmin == 0 then
				exports.titan_global:sendMessageToAdmins("[VEHICLE MANAGER]" .. getPlayerName(client) .. " restarted the carshops.")
			else
				exports.titan_global:sendMessageToAdmins("[VEHICLE MANAGER] A hidden admin restarted the carshops.")
			end
			--exports.titan_logs:logMessage("[STEVIE] " .. getElementData(client, "account:username") .. "/".. getPlayerName(client) .." restarted the gatekeepers." , 25)
			exports.titan_logs:dbLog(client, 4, client, "RESETCARSHOP")
		elseif getResourceState(theResource) == "loaded" then
			...
			if hiddenAdmin == 0 then
				exports.titan_global:sendMessageToAdmins("[VEHICLE MANAGER] " .. getPlayerName(client) .. " started the carshops.")
			else
				exports.titan_global:sendMessageToAdmins("[VEHICLE MANAGER] A hidden admin started the carshops.")
			end
			exports.titan_logs:dbLog(client, 4, client, "RESETCARSHOP")
...
function openVehlib(thePlayer)
	if exports.titan_integration:isPlayerVCTMember(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer) or exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerScripter(thePlayer) or exports.titan_integration:isPlayerVehicleConsultant(thePlayer) then
...
function createVehicleRecord(data)
	...
	if not data.update then
		...
		exports.titan_logs:dbLog(client, 6, { client }, " Created new vehicle in library.")
		exports.titan_global:sendMessageToAdmins("[vehicle_manager]: "..getElementData(client, "account:username").." has created new vehicle in library.")
		return true
	else
		if data.copy then
			dbExec(mysql:getConnection(), "INSERT INTO vehicles_shop SET vehmtamodel='"..toSQL(data["mtaModel"]).."', vehbrand='"..toSQL(data["brand"]).."', vehmodel='"..toSQL(data["model"]).."', vehyear='"..toSQL(data["year"]).."', vehprice='"..toSQL(data["price"]).."', vehtax='"..toSQL(data["tax"]).."', createdby='"..toSQL(getElementData(client, "account:id")).."', notes='"..toSQL(data["note"]).."' , enabled='"..toSQL(enabled).."', `spawnto`='"..toSQL(data["spawnto"]).."', `doortype` = " .. data.doortype)
			...
			exports.titan_logs:dbLog(client, 6, { client }, " Updated vehicle #"..data.id.." from library.")
			exports.titan_global:sendMessageToAdmins("[vehicle_manager]: "..getElementData(client, "account:username").." has updated vehicle #"..data.id.." in library.")
			return true
		end
	end
end
	...
	if customHandlings and type(customHandlings) == "string" then
		...
		if h then
			for i = 1, #handlings do 
				if i ~= 29 then -- I don't know why this isn't working in 1.4. Temporarily disable this stat / Hanz
...
function giveTempVctAccess(thePlayer, commandName, targetPlayer)
	if exports.titan_integration:isPlayerSeniorAdmin(thePlayer) or exports.titan_integration:isPlayerVehicleConsultant(thePlayer) then
		if not targetPlayer then
			...
			if not exports.titan_integration:isPlayerVCTMember(targetPlayer) and not exports.titan_integration:isPlayerSeniorAdmin(thePlayer) then
			...
			local thePlayerIdentity = exports.titan_global:getPlayerFullIdentity(thePlayer)
			local targetPlayerIdentity = exports.titan_global:getPlayerFullIdentity(targetPlayer)
			
			if not hasVctAdmin then
				if not (exports.titan_integration:isPlayerSeniorAdmin(thePlayer) or exports.titan_integration:isPlayerVehicleConsultant(thePlayer)) then
				...
				if setElementData(targetPlayer, "hasVctAdmin", true) then
					...
					exports.titan_global:sendMessageToAdmins("[VCT] "..thePlayerIdentity.." has given " ..targetPlayerIdentity.. " temporary VCT admin.")
					exports.titan_logs:dbLog(thePlayer, 4, targetPlayer, commandName)
				end
			else
				if setElementData(targetPlayer, "hasVctAdmin", false) then
					...
					exports.titan_global:sendMessageToAdmins("[VCT] "..thePlayerIdentity.." has revoked from " .. targetPlayerIdentity .. " temporary VCT admin.")
				end
			end
		end
	end
