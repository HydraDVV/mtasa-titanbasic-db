...
function applyPickupClientConfigSettings()
    local streamerDistance = tonumber( exports.titan_account:loadSavedData("streamer-pickup", "25") )
...
function interiorShowPickups(interiorElement)
    ...
    local outsidePickup = createPickup( entrance[INTERIOR_X], entrance[INTERIOR_Y], entrance[INTERIOR_Z], 3, int[INTERIOR_DISABLED] and 1314 or ( getElementType(interiorElement) == "elevator" and tpObjectModel or ( int[INTERIOR_TYPE] == 2 and tpObjectModel  or ( int[INTERIOR_OWNER] < 1 and int[INTERIOR_FACTION] < 1 and ( int[INTERIOR_TYPE] == 1 and 1272 or 1274 ) or tpObjectModel  ) ) ) )
    ...
    if useFakePickups then
        if not isPickupStreamable(outsidePickup) then
            local fakeHelper = createObject(int[INTERIOR_DISABLED] and 1314 or ( getElementType(interiorElement) == "elevator" and tpObjectModel or ( int[INTERIOR_TYPE] == 2 and tpObjectModel  or ( int[INTERIOR_OWNER] < 1 and ( int[INTERIOR_TYPE] == 1 and 1272 or 1274 ) or tpObjectModel  ) ) ), entrance[INTERIOR_X], entrance[INTERIOR_Y], entrance[INTERIOR_Z])
            ...
            local fakeScale = 0.5
            if(fakeModel == 1272 or fakeModel == 1274) then
                fakeScale = 1.0
            end
...
local robotoFont = exports.titan_fonts:getFont("Roboto", 10)
local BizNoteFont = dxCreateFont( ":titan_resources/fonts/BizNote.ttf", 21 ) or "default-bold"
...
function renderInteriorName()
    ...
    if theInterior and isElement(theInterior) and isElementWithinColShape ( localPlayer, theInterior ) then
        ...
        dxDrawText ( intName or "Bilinmeyen", intName_left+textShadowDistance , intName_top+textShadowDistance , intName_right+textShadowDistance, intName_bottom+textShadowDistance, tocolor(0,0,0,255),
                    1.0, robotoFont, "center", "center", false, true )
        dxDrawText ( intName or "Bilinmeyen", intName_left , intName_top , intName_right, intName_bottom, textColor,
                    1.0, robotoFont, "center", "center", false, true )
        ...
        if isLastSourceInterior then
            ...
            if canPlayerKnowInteriorOwner(theInterior) then
                ...
                if intStatus[INTERIOR_OWNER] > 0 then
                    local ownerName = exports.titan_cache:getCharacterNameFromID(intStatus[INTERIOR_OWNER])
                    if intType == 3 then
                ...
                elseif intStatus[INTERIOR_FACTION] > 0 then
                    local ownerName = exports.titan_cache:getFactionNameFromId(intStatus[INTERIOR_FACTION])
                    if intType ~= 2 then
                        intOwner = "Mülk Sahibi: "..(ownerName or "..Loading..")
                        intInst = "Girmek için F tuşuna basın"
                    end
                    ...
                    elseif intType == 3 then
                        local intPrice = exports.titan_global:formatMoney(intStatus[INTERIOR_COST])
		...
		if tonumber(intFee) > 0 then
			...
			dxDrawText ( intFee , intFee_left , intName_top , intFee_right, intFee_bottom, textColor,
					1, robotoFont, "center", "center", false, true )
...
function canPlayerKnowInteriorOwner(theInterior)
    return  (getElementData(theInterior, "status")[INTERIOR_OWNER] == 0) -- unown.
        or  (exports.titan_integration:isPlayerTrialAdmin(localPlayer) and (getElementData(localPlayer, "duty_admin") == 1))
...
function canPlayerSeeInteriorID(theInterior)
    return  getElementData(localPlayer, "faction") == 1 -- LSPD
        ...
        or  (exports.titan_integration:isPlayerTrialAdmin(localPlayer) and (getElementData(localPlayer, "duty_admin") == 1))
        or  (exports.titan_integration:isPlayerSupporter(localPlayer) and (getElementData(localPlayer, "duty_supporter") == 1))
end
...
addEventHandler("onClientResourceStop", getResourceRootElement(getResourceFromName("titan_elevators")),
    function(stoppedRes)
        clearElevators()
    end
);
...
function forcePickupSpawn()
    if exports.titan_integration:isPlayerScripter(getLocalPlayer()) then
        initializeSoFar()
    end
end
