...
function beginSave()
	outputDebugString("Oyuncularin istatistikleri veritabanina kaydedildi.") -- !Hanz
...
function syncTIS()
	for key, value in ipairs(getElementsByType("player")) do
		...
		if (tis) and (getPlayerIdleTime(value) < 600000)  then
			exports.titan_anticheat:changeProtectedElementDataEx(value, "timeinserver", tonumber(tis)+1, false)
		end
	end
end
...
addEventHandler("onResourceStart", root,
    function(startedRes)
        if getResourceName(startedRes) == "titan_mysql" then
            connection = exports.titan_mysql:getConnection()
...
function savePlayer(reason, player)
	...
	if isElement(player) then
		...
		if (logged==1 or reason=="Change Character") then
			...
			-- Fix for freecam-tv
			if exports['titan_freecam']:isPlayerFreecamEnabled(player) then 
			...
			-- LAST AREA
			local zone = exports.titan_global:getElementZoneName(player)
			if not zone or #zone == 0 then
				zone = "Unknown"
			end
			local level = getElementData(player, "level") or 1
			local exp = getElementData(player, "exp") or 1
			...
			local update = dbExec(connection, "UPDATE characters SET thirst='"..(thirst) .. "', hoursplayed='"..(hoursplayed) .. "', bankmoney='"..(bankmoney).."', hunger='" .. (hunger) .. "', level='" .. (level) .. "', x='" .. (x) .. "', y='" .. (y) .. "', z='" .. (z) .. "', rotation='" .. (rot) .. "', health='" .. (health) .. "', armor='" .. (armor) .. "', dimension_id='" .. (dimension) .. "', interior_id='" .. (interior) .. "', " ..(money) .. (businessprofit) .. "lastlogin=NOW(), lastarea='" .. (zone) .. "', timeinserver='" .. (timeinserver) .. "' WHERE id=" .. (getElementData(player, "dbid")))
			if not (update) then
				outputDebugString( "Saveplayer Update:" )
			end
			local bakiye = getElementData(player, "bakiyeMiktar") or 0
			local update3 = dbExec(mysql:getConnection(), "UPDATE accounts SET bakiyeMiktari='"..bakiye.."' WHERE id = " .. (getElementData(player,"account:id")))
			if not (update3) then
...
addCommandHandler("saveall", function(p) if exports.titan_integration:isPlayerSeniorAdmin(p) then beginSave() outputChatBox("Done.", p) end end)
