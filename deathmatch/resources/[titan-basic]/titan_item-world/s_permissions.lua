...
function getItemPropertiesData(object)
	...
	if creator > 0 then
		creatorName = exports.titan_global:getCharacterNameFromID(creator)
	end
...
function saveItemProperties(object, use, useData, move, moveData, pickup, pickupData)
	...
	if getElementParent(getElementParent(object)) == getResourceRootElement(getResourceFromName("titan_item-world")) then
		...
		if id then
			...
			local itemName = exports['titan_items']:getItemName(tonumber(getElementData(object, "itemID")), getElementData(object, "itemValue"))
			...
			exports.titan_anticheat:changeProtectedElementDataEx(object, "worlditem.permissions", permissions)
