...
function vehicleBlown()
	exports.titan_anticheat:changeProtectedElementDataEx(source, "lspd:flashers", nil, true)
end
...
local function getFactionType(vehicle)
	...
	local vehicleFactionElement = exports.titan_pool:getElement("team", vehicleFactionID)
...
local function canUseStrobes(vehicle, data)
	if data then
		if data[2] and exports.titan_global:hasItem(vehicle, data[2]) then
			return data[1]
		end
		for i = 3, #data do
...
function toggleFlasherState()
	...
	if (theVehicle) then
		...
		if currentFlasherState ~= 0 then
			exports.titan_anticheat:changeProtectedElementDataEx(theVehicle, "lspd:flashers", nil, true)
		else
			...
			if color then
				exports.titan_anticheat:changeProtectedElementDataEx(theVehicle, "lspd:flashers", color, true)
			else
