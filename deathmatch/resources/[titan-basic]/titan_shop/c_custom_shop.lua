...
function addItemToShop(source, item, slot, worldItem, npcID, shopElement )
	...
	if tonumber(item[1]) == 16 then --clothes
		local value = exports.titan_global:explode(":",tostring(item[2]))
		local skinid = tonumber(value[1]) or 1
		itemURL = ":titan_account/img/" .. ("%03d"):format( skinid or 1 ) .. ".png"
	elseif tonumber(item[1]) == 115 then -- weapons
		itemURL = ":titan_items/images/-"..string.match(tostring(item[2]), "(%d+)")..".png"
	else -- other
		itemURL = ":titan_items/images/"..tostring(item[1])..".png"
	end
	
	--outputDebugString(item[1].." - "..item[2]) 
	
	local itemName = exports["titan_items"]:getItemName( tonumber(item[1]), tostring(item[2]) ) or ""
	local itemValue = ""
	if not exports["titan_items"]:getItemHideItemValue(tonumber(item[1])) then
		itemValue = exports["titan_items"]:getItemValue( tonumber(item[1]), tostring(item[2]) ) or ""
	end
	...
	local mDesc = guiCreateMemo(147,99,280,56,exports["titan_items"]:getItemDescription( tonumber(item[1]), tostring(item[2]) ) or "",false,wAddingItemsToShop)
...
function customShopBuy(products, proID, shopElement)
	...
	local ePrice = guiCreateLabel(189,62,238,20,exports.titan_global:formatMoney(itemPrice),false,wCustomShopBuy)
