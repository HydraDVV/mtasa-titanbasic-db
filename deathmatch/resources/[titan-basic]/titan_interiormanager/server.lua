							...
							if rows > 0 then
								...
								dbQuery(
									function(qh, thePlayer)
										...
										triggerClientEvent(thePlayer, "createCheckIntWindow", thePlayer, result, exports.titan_global:getPlayerAdminTitle(thePlayer), result2, notes)
									end,
	...
	if exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer) then
		...
		for _, interior in ipairs(possibleInteriors) do
			if tonumber(intID) == getElementData(interior, "dbid") then
				if not exports.titan_integration:isPlayerAdmin(thePlayer) then
					local success, msg1, msg2 = exports["titan_jobs"]:remoteOrderSupplies(thePlayer, intID, amount, true)
					...
					if hiddenAdmin == 0 then
						exports.titan_global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(thePlayer):gsub("_", " ").. " ("..adminUsername..") has restocked "..getElementData(interior, "name").." (ID#"..intID..") with "..amount.." of supplies.")
					else
						exports.titan_global:sendMessageToAdmins("[INTERIOR]: A hidden admin has has restocked "..getElementData(interior, "name").." (ID#"..intID..") with "..amount.." of supplies.")
					end
...
function setInteriorFaction(thePlayer, cmd, ...)
	if exports.titan_integration:isPlayerAdmin(thePlayer) then
		...
		if tonumber(clue) then
			theFaction = exports.titan_pool:getElement("team", tonumber(clue))
		else
			theFaction = exports.titan_factions:getFactionFromName(clue)
		end
		...
		local dbid, entrance, exit, interiorType, interiorElement = exports['titan_interiors']:findProperty( thePlayer )
		if not isElement(interiorElement) then
			outputChatBox("No interior found here.", thePlayer, 255, 0, 0 )
			return
		end

		local can , reason = exports.titan_global:canFactionBuyInterior(theFaction)
		if not can then
		...
		call( getResourceFromName( "titan_items" ), "deleteAll", interiorType == 1 and 5 or 4, dbid )
		exports.titan_global:giveItem(thePlayer, interiorType == 1 and 5 or 4, dbid)

		exports.titan_logs:dbLog(thePlayer, 37, { "in"..tostring(dbid) } , "SETINTFACTION INTERIOR ID#"..dbid.." TO FACTION '"..factionName.."'")
		exports['titan_interiors']:realReloadInterior(tonumber(dbid))
		triggerClientEvent(thePlayer, "createBlipAtXY", thePlayer, entrance[INTERIOR_TYPE], entrance[INTERIOR_X], entrance[INTERIOR_Y])
		exports.titan_global:sendMessageToAdmins("[INTERIOR] "..exports.titan_global:getPlayerFullIdentity(thePlayer).." transferred the ownership of interior '"..intName.."' ID #"..dbid.." to faction '"..factionName.."'.")
		return true
	end
end
...
function setInteriorToMyFaction(thePlayer, cmd)
	...
	local theFaction = exports.titan_pool:getElement("team", tonumber(factionId))
	if not theFaction then
		outputChatBox("No faction found.", thePlayer, 255, 0, 0 )
		return
	end
	...
	local can , reason = exports.titan_global:canPlayerFactionBuyInterior(thePlayer)
	if not can then
		outputChatBox(reason, thePlayer, 255, 0, 0 )
		return 
	end
	...
	exports.titan_logs:dbLog(thePlayer, 37, { "in"..tostring(dbid) } , "SETINTTOMYFACTION INTERIOR ID#"..dbid.." TO FACTION '"..factionName.."'")
	exports['titan_interiors']:realReloadInterior(tonumber(dbid))
	triggerClientEvent(thePlayer, "createBlipAtXY", thePlayer, entrance[INTERIOR_TYPE], entrance[INTERIOR_X], entrance[INTERIOR_Y])
	exports.titan_global:sendMessageToAdmins("[INTERIOR] "..exports.titan_global:getPlayerFullIdentity(thePlayer).." transferred the ownership of interior '"..intName.."' ID #"..dbid.." to his faction '"..factionName.."'.")
	return true
end
