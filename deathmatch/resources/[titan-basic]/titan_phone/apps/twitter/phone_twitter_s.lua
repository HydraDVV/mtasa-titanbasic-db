    ...
    function(player, name, location, text)
        ...
        for k,v in ipairs(advertisementMessages) do
            ...
            if (found) or (found2) or (string.lower(text)==tostring(v)) then
                exports.titan_global:sendMessageToAdmins("AdmWrn: "..getPlayerName(player).." adlı oyuncu yasaklı tweet attı, mesajı:" .. tostring(text))
                outputChatBox("#575757"..exports["titan_pool"]:getServerName()..":#ffffff Tweet yazısında yasaklı kelimeler bulundu.", player, 255, 0, 0, true)
                return
            end
        end
        if not exports.titan_integration:isPlayerDeveloper(player) then
