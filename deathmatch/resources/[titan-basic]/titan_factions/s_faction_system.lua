...
function getFactionFromName(factionName)
	for k,v in ipairs(exports.titan_pool:getPoolElementsByType("team")) do
...
function getTeamFromFactionID(factionID)
	...
	return exports.titan_pool:getElement("team", factionID)
end
		...
		function(qh)
			...
			Async:foreach(res,
				function(row)
					...
					if theTeam then
						exports.titan_pool:allocateElement(theTeam, id)
						exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "type", factionType, true)
						exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "money", money, true)
						exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "id", id, true)
						...
						local motd = row.motd
						exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "ranks", factionRanks, true)
						exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "wages", factionWages, false)
						exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "motd", motd, false)
						exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "note", row.note == mysql_null() and "" or row.note, false)
						exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "fnote", row.fnote == mysql_null() and "" or row.fnote, false)
						exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "phone", row.phone ~= mysql_null() and row.phone or nil, false)
						exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "max_interiors", tonumber(row.max_interiors), false, true)
					--	setElementData(theTeam, "birlik_level", tonumber(row.level)) --Don't sync at all / Hanz
					--	setElementData(theTeam, "birlik_onay", tonumber(row.onay)) --Don't sync at all / Hanz
				...
				function()
					...
					exports.titan_pool:allocateElement(citteam, -1)
					
					setElementData(getResourceRootElement(getResourceFromName("titan_factions")), "factionDuty", custom)
					setElementData(getResourceRootElement(getResourceFromName("titan_factions")), "factionLocations", locations)
	
					local players = exports.titan_pool:getPoolElementsByType("player")
					for k, thePlayer in ipairs(players) do
						if getElementData(thePlayer, "loggedin") == 1 then
							setPlayerTeam(thePlayer, exports.titan_pool:getElement("team", (getElementData(thePlayer, "faction") or -1)) or citteam)
						end
					end
				end
			)
...
function getPlayerFaction(playerName)
	...
	if (not thePlayerElement or override) then  -- Player is offline
		local accounts, characters = exports.titan_account:getTableInformations()
...
function showFactionMenuEx(source, factionID, fromShowF)
	...
	if (logged==1) then
		...
		if (menuVisible==0) then
			...
			if (factionID~=-1) then
				local theTeam = exports.titan_pool:getElement("team", factionID)
				if theTeam then
							...
							if rows > 0 then
								...
								for index, value in ipairs(getElementsByType("vehicle")) do
									if value and isElement(value) and tonumber(value:getData("faction")) == tonumber(factionID) then
										...
										local veh = exports.titan_pool:getElement("vehicle", value:getData("dbid"))
										vehicleModels[j] = exports.titan_global:getVehicleName(veh)
										if exports.titan_global:hasItem(veh, 139) and getElementDimension(veh) == 0 and getElementInterior(veh) == 0 then
											vehicleLocations[j] = exports.titan_global:getElementZoneName(veh) 
										else
				...
				outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #f4f4f4Herhangi bir birlikte bulunmuyorsunuz.", source, 255, 0, 0, true)
			end
		else
...
function callbackUpdateRanks(ranks, wages)
	...
	if (wages) then
		...
		exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "wages", wages, false)
	end
	...
	exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "ranks", ranks, false)
...
function callbackRespawnVehicles()
	...
	if not (factionCooldown) then
		...
		exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "cooldown", true, false)
	else
...
function resetFactionCooldown(theTeam)
	exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "cooldown")
end

function callbackRespawnOneVehicle(vehicleID)
	local theTeam = getPlayerTeam(source)
	local theTeamID = getElementData(theTeam, "id")
	local theVehicle = exports.titan_pool:getElement("vehicle", tonumber(vehicleID))
	...
	if theVehicle then
		...
		if (theTeamID == theVehicleID and not getVehicleOccupant(theVehicle, 0) and not getVehicleOccupant(theVehicle, 1) and not getVehicleOccupant(theVehicle, 2) and not getVehicleOccupant(theVehicle, 3) and not getVehicleTowingVehicle(theVehicle)) then
			...
			exports.titan_logs:dbLog(source, 6, theVehicle, "FACTIONRESPAWN")
...
function callbackUpdateNote(note)
	...
	if (factionID~=-1) then
		if dbExec(mysql:getConnection(), "UPDATE factions SET note='" .. tostring((note)) .. "' WHERE id='" .. factionID .. "'") then
			...
			exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "note", note, false)
		else
...
function callbackUpdateFNote(fnote)
	...
	if (factionID~=-1) then
		if dbExec(mysql:getConnection(), "UPDATE factions SET fnote='" .. tostring((fnote)) .. "' WHERE id='" .. factionID .. "'") then
			...
			exports.titan_anticheat:changeProtectedElementDataEx(theTeam, "fnote", fnote, false)
		else
...
function callbackRemovePlayer(removedPlayerName)
	...
	if dbExec(mysql:getConnection(), "UPDATE characters SET faction_id='-1', faction_leader='0', faction_rank='1', duty = 0 WHERE charactername='" .. (removedPlayerName) .. "'") then
		...
		if (removedPlayer) then -- Player is online
			...
			exports.titan_anticheat:changeProtectedElementDataEx(removedPlayer, "faction", -1, false)
			exports.titan_anticheat:changeProtectedElementDataEx(removedPlayer, "factionleader", 0, false)
			exports.titan_anticheat:changeProtectedElementDataEx(targetPlayer, "factionleader", 0, false)
...
function callbackToggleLeader(playerName, isLeader)
	...
	if (isLeader) then -- Make player a leader
		...
		if dbExec(mysql:getConnection(), "UPDATE characters SET faction_leader='1' WHERE charactername='" .. (playerName) .. "'") then
			...
			-- Send message to everyone in the faction
			--exports.titan_factions:sendNotiToAllFactionMembers(factionID, username:gsub("_", " ") .. " promoted " .. playerName:gsub("_", " ") .. " to leader of your faction '"..getTeamName(theTeam).."'.")
			
			local thePlayer = getPlayerFromName(playerName)
			if(thePlayer) then -- Player is online, tell them
				exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "factionleader", 1, true)
			end
		else
		...
		if dbExec(mysql:getConnection(), "UPDATE characters SET faction_leader='0' WHERE charactername='" .. (playerName) .. "'") then
			...
			if(thePlayer) then -- Player is online, tell them
				...
				exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "factionleader", 0, true)
			end
			
			-- Send message to everyone in the faction
		--exports.titan_factions:sendNotiToAllFactionMembers(factionID, username:gsub("_", " ") .. " demoted " .. playerName:gsub("_", " ") .. " from leader to member of your faction '"..getTeamName(theTeam).."'.")
		else
...
function callbackPromotePlayer(playerName, rankNum, oldRank, newRank)
	...
	if dbExec(mysql:getConnection(), "UPDATE characters SET faction_rank='" .. rankNum .. "' WHERE charactername='" .. (playerName) .. "'") then
		...
		if(thePlayer) then -- Player is online, set his rank
			exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "factionrank", rankNum, false)
		end
		
		-- Send message to everyone in the faction
		--exports.titan_factions:sendNotiToAllFactionMembers(factionID, playerName:gsub("_", " ") .. " was promoted from '" .. oldRank .. "' to '" .. newRank .. "' by "..username:gsub("_", " ").." of '"..getTeamName(theTeam).."'")
	else
...
function callbackDemotePlayer(playerName, rankNum, oldRank, newRank)
	...
	if dbExec(mysql:getConnection(), "UPDATE characters SET faction_rank='" .. rankNum .. "' WHERE charactername='" .. safename .. "'") then
		...
		-- Send message to everyone in the faction
		--exports.titan_factions:sendNotiToAllFactionMembers(factionID, playerName:gsub("_", " ") .. " was demoted from '" .. oldRank .. "' to '" .. newRank .. "' by "..username:gsub("_", " ").." of '"..getTeamName(theTeam).."'")
	else
...
function callbackQuitFaction()
	...
	elseif dbExec(mysql:getConnection(), "UPDATE characters SET faction_id='-1', faction_leader='0', duty = 0, faction_perks='{}' WHERE charactername='" .. safename .. "'") then
		exports["titan_infobox"]:addBox(client, "success","'" .. theTeamName .. "' isimli birlikten çıkış yaptın.")
		exports.titan_global:takeItem(client, 87)
		exports.titan_global:takeItem(client, 87)
		exports.titan_global:takeItem(client, 87)
		exports.titan_global:takeItem(client, 86)
		exports.titan_global:takeItem(client, 86)
		exports.titan_global:takeItem(client, 86)
		exports.titan_global:takeItem(client, 64)
		exports.titan_global:takeItem(client, 64)
		exports.titan_global:takeItem(client, 64)
		local newTeam = getTeamFromName("Citizen")
		setPlayerTeam(client, newTeam)
		exports.titan_anticheat:changeProtectedElementDataEx(client, "faction", -1, false)
		exports.titan_anticheat:changeProtectedElementDataEx(client, "factionrank", 1, false)
		exports.titan_anticheat:changeProtectedElementDataEx(client, "factionleader", 0, false)
		exports.titan_anticheat:changeProtectedElementDataEx(client, "factionphone", nil, false)
		exports.titan_anticheat:changeProtectedElementDataEx(client, "factionPackages", {}, false)
		--triggerClientEvent(client, "updateFactionInfo", client, -1, 1)
		triggerEvent("duty:offduty", client)
		
		-- Send message to everyone in the faction
		local factionID = getElementData(theTeam, "id")
		--exports.titan_factions:sendNotiToAllFactionMembers(factionID, username:gsub("_", " ") .. " left your faction '" .. theTeamName .. "'.")
	else
...
function callbackInvitePlayer(invitedPlayer)
	...
	if dbExec(mysql:getConnection(), "UPDATE characters SET faction_leader = 0, faction_id = " .. factionID .. ", faction_rank = 1 WHERE charactername='" .. safename .. "'") then
		...
		if (targetTeam~=nil) and (getTeamName(targetTeam)~="Citizen") then
			...
			exports.titan_anticheat:changeProtectedElementDataEx(invitedPlayer, "faction", factionID, false)
			outputChatBox("Player " .. invitedPlayerNick:gsub("_", " ") .. " is now a member of faction '" .. tostring(theTeamName) .. "'.", client, 0, 255, 0)
			--exports.titan_factions:sendNotiToAllFactionMembers(factionID, invitedPlayerNick:gsub("_", " ") .. " joined as a new member of your faction '" .. tostring(theTeamName) .. "'.")				
			if	(invitedPlayer) then
				triggerEvent("onPlayerJoinFaction", invitedPlayer, theTeam)
				exports.titan_anticheat:changeProtectedElementDataEx(invitedPlayer, "factionrank", 1, false)
				exports.titan_anticheat:changeProtectedElementDataEx(client, "factionphone", nil, false)
...
function hideFactionMenu()
	exports.titan_anticheat:changeProtectedElementDataEx(client, "factionMenu", 0, false)
end
	...
	if hasPlayerAccessOverFaction(client, factionID) then
		...
		dbQuery(
			function(qh, client)
				...
				if rows > 0 then
					for index, row in ipairs(res) do
						...
						elseif tonumber(row["from"]) then
							...
							if num < 0 then
								from = getTeamName(exports.titan_pool:getElement("team", -num)) or "-"
						...
						elseif tonumber(row["to"]) and tonumber(row["to"]) < 0 then
							to = getTeamName(exports.titan_pool:getElement("team", -tonumber(row["to"])))
						end
				...
				local bankmoney = exports.titan_global:getMoney(faction)
...
addEventHandler('factionmenu:setphone', root,
	function(playerName, number)
		...
		if dbExec(mysql:getConnection(), "UPDATE characters SET faction_phone=" .. (tonumber(number) or "NULL") .. " WHERE charactername='" .. safename .. "'") then
			local thePlayer = getPlayerFromName(playerName)
			if(thePlayer) then -- Player is online, tell them
				exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "factionphone", tonumber(number) or nil, false)
			end
		end
	end)
...
function fromDatetime(string)
	local split1 = exports.titan_global:split(string, " ")
	local date = split1[1]
	local time = split1[2]

	local datesplit = exports.titan_global:split(date, "-")
	local year = tonumber(datesplit[1])
	local month = tonumber(datesplit[2])
	local day = tonumber(datesplit[3])

	local timesplit = exports.titan_global:split(date, ":")
...
-- Hanz'nın kıymetli elleri tarafından tamamiyle düzenlendi.
...
function refreshClient(message, factionID, dontSendToClient)
	...
	local resource = getResourceRootElement(getResourceFromName("titan_factions"))
	if resource then
...
function addDuty(dutyItems, finalLocations, dutyNewSkins, name, factionID, dutyID)
	...
	if dutyID == 0 then
		...
		exports.titan_logs:dbLog(client, 35, "fa"..tostring(factionID), "Added duty "..name.." Database ID #"..index)
	else
		...
		exports.titan_logs:dbLog(client, 35, "fa"..tostring(factionID), "Revised duty "..name.." Database ID #"..dutyID)
	end
end
...
function addLocation(x, y, z, r, i, d, name, factionID, index)
	local interiorElement = exports.titan_pool:getElement("interior", d) or d == 0
	if interiorElement then
		...
		if tonumber(interiorF) == tonumber(factionID) or d == 0 then
			if not index then -- Index is used if the event is from a edit
				...
				exports.titan_logs:dbLog(client, 35, "fa"..tostring(factionID), "Added location, Name:"..name.." Database ID:"..newIndex.." x:"..x.." y:"..y.." z:"..z.." radius:"..r.." interior:"..i.." dimension:"..d)
			else
				...
				exports.titan_logs:dbLog(client, 35, "fa"..tostring(factionID), "Revised location ID #"..index.." x:"..x.." y:"..y.." z:"..z.." radius:"..r.." interior:"..i.." dimension:"..d)
			end
		else
...
function addVehicle(vehicleID, factionID)
	local element = exports.titan_pool:getElement("vehicle", vehicleID)
	if element then
		if getElementData(element, "faction") == factionID then
			...
			exports.titan_logs:dbLog(client, 35, "fa"..tostring(factionID), "Added Vehicle #"..vehicleID.." Database ID:"..newIndex)
...
function removeLocation(removeID, factionID)
	...
	exports.titan_logs:dbLog(client, 35, "fa"..tostring(factionID), "Removed Location #"..removeID)
...
function removeDuty(removeID, factionID)
	...
	exports.titan_logs:dbLog(client, 35, "fa"..tostring(factionID), "Removed duty #"..removeID)
...
function factionActive(thePlayer, commandName)
			...
			if takim and takim > 0 then
				local takimveri = exports.titan_pool:getElement("team", takim)
				local takimisim = getTeamName(takimveri)
				local takimoyunculari = getPlayersInTeam(takimveri)
				
				if #takimoyunculari == 0 then
					outputChatBox("#822828 "..exports["titan_pool"]:getServerName()..":#b9c9bf".. takimisim .."' maalesef oluşumunda hiç çevrimiçi oyuncu bulamadım.", thePlayer, 255, 255, 255, true)
				else
					...
					for k, takimoyuncular in ipairs(takimoyunculari) do
						...
						outputChatBox("#822828 "..exports["titan_pool"]:getServerName()..": #b9c9bf  Karakter : #FFFFFF".. getPlayerName(takimoyuncular) .." #b9c9bf Rütbe : #FFFFFF".. oyuncurutbe .. " #b9c9bf Telefon : - #FFFFFF ", thePlayer, 255, 255, 255, true)
					end
				end
			else
				outputChatBox("#822828 "..exports["titan_pool"]:getServerName()..": #b9c9bf Herhangi bir oluşumda değilsiniz, lütfen oluşuma girdikten sonra tekrar deneyiniz.", thePlayer, 255, 255, 255, true)
			end
	end
