...
addEventHandler("onResourceStart", getResourceRootElement(getThisResource()), function() 
	for i, player in pairs(getElementsByType("player")) do
		exports.titan_anticheat:changeProtectedElementDataEx(player, "usingThisShop", false, false, true)
	end
end)
...
function setShopCurrentUser(shop, player)
	...
	if shop and getElementType(shop) == "ped" and getElementType(player) == "player" then
		exports.titan_anticheat:changeProtectedElementDataEx(shop, "usingThisShop", player, false, true)
		exports.titan_anticheat:changeProtectedElementDataEx(player, "usingThisShop", shop, false, true)
...
function removeMeFromCurrentShopUser(player)
	...
	exports.titan_anticheat:changeProtectedElementDataEx(source, "usingThisShop", nil, false, true)
	if not shop or not isElement(shop) or not (getElementType(shop) == "ped") then
		outputDebugString("Can't get shop from client")
		return false
	end
	exports.titan_anticheat:changeProtectedElementDataEx(shop, "usingThisShop", nil, false, true)
...
function canIAccessThisShop(shop, player)
	...
	elseif not whoIsUsingIt then
		...
		if whoIsUsingIt and isElement(whoIsUsingIt) and getElementType(whoIsUsingIt) == "player" then
			return false , exports.titan_global:getPlayerName(whoIsUsingIt)
		else
			return true
		end
	end
