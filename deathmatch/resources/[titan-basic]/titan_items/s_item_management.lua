...
local function getID(element)
	...
	elseif getElementType(element) == "vehicle" then -- Vehicle
		return getElementData(element, "dbid")
	elseif getElementParent(getElementParent(element)) == getResourceRootElement(getResourceFromName("titan_item-world")) then -- World Item
							...
							if query then
								...
								if itemID == 115 then -- guns
									...
									if isThisGunDuplicated(itemValue, target) then
										...
										outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffffFazla uyanık olma Query was here :P.", target, 255, 0, 0, true)
								...
								if ( (itemID >= 31) and (itemID <= 43) ) or itemBannedByAltAltChecker[itemID] then
									...
									if not exports.titan_global:isStaffOnDuty(to) and not exports.titan_global:isStaffOnDuty(from) then
										if hoursPlayedFrom < 10 then
											outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffff"..getItemName( itemID ).." isimli eşyayı "..getName(to).."'a transfer edebilmek için 10 saat oynaman olması gerekmektedir.", from, 255, 0, 0, true)
											return false, "Item transferi engellendi, < 10 saat"
										end
										
										if hoursPlayedTo < 10 then
											outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffff"..getItemName( itemID ).." isimli eşyayı "..getName(from).." size transfer edebilmesi için 10 saat oynaması olması gerekmektedir.", to, 255, 0, 0, true)
											return false, "Item transferi engellendi, < 10 saat"
										end
									end
								end
								...
								if itemID == 134 then -- MONEY
									if takeItemFromSlot(from, slot, true) then
										if exports.titan_global:giveMoney(to, tonumber(itemValue)) then
											return true
										end
									end
								else
...
function getMaxWeight(element)
	...
	elseif getElementType( element ) == "vehicle" then
		...
		elseif isTruck( element ) then
			return tonumber(exports["titan_jobs"]:getTruckCapacity(element)) or 120
		else
			return 20
		end
...
function deleteAll( itemID, itemValue )
	if itemID then
		...
		if saveditems then
			for value in pairs( saveditems ) do
				if isElement( value ) then
					while exports.titan_global:hasItem( value, itemID, itemValue ) do
						exports.titan_global:takeItem( value, itemID, itemValue )
					end
				end
			end
		end
		...
		-- remove world items
		local objects = getElementsByType('object', getResourceRootElement(getResourceFromName('titan_item-world')))
		if objects then
				...
				function(qh)
					...
					if rows > 0 then
						for index, row in ipairs(res) do
							Async:foreach(getElementsByType("object", getResourceRootElement(getResourceFromName("titan_item-world"))), function(value)
								if isElement( value ) then
	...
	function(thePlayer, commandName, targetName)
		if exports.titan_integration:isPlayerTrialAdmin(thePlayer) then
			if not (targetName) then
				return outputChatBox("|| "..exports["titan_pool"]:getServerName().." Roleplay || /" .. commandName .. " [Oyuncu İsmi/ID]", thePlayer, 255, 194, 14)
			end
			local targetPlayer, targetPlayerName = exports.titan_global:findPlayerByPartialNick(thePlayer, targetName)
			if not targetPlayer then
				return outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffffKişi bulunamadı.", thePlayer, 255, 0, 0, true)
			end
			loadItems(targetPlayer, true)
			outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #ffffffKişinin envanteri başarıyla yüklendi.", thePlayer, 0, 255, 0, true)
		end
	end
)
