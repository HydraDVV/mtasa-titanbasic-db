...
function openStaffManager(staffInfo1, staffTeams1, globalChangelogs1) --/ Hanz 8 mayıs 05:46
	if canPlayerAccessStaffManager(localPlayer) then
		...
		if GUIEditor.window[1] and isElement(GUIEditor.window[1]) then
	        ...
	        GUIEditor.window[1] = guiCreateWindow(538, 144, 700, 461+yExtend, exports["titan_pool"]:getServerName().." Roleplay - Yetkili Yönetim Arayüzü © Hanz", false)
	        guiWindowSetSizable(GUIEditor.window[1], false)
	        exports.titan_global:centerWindow(GUIEditor.window[1])
	        	...
	        	if source == GUIEditor.edit[1] then
	        		...
	        		if string.len(text) >= 3 then
		        		...
		        		timer.username = setTimer(function() 
		        			local username = exports.titan_cache:getUsername(text)
		        			if username then
	        ...
	        for i, rank in ipairs(staffTitles[1]) do
	       	...
	       	exports.titan_global:guiComboBoxAdjustHeight(GUIEditor.combobox.admin, count)
	        ...
	        for i, rank in ipairs(staffTitles[2]) do
	       	...
	       	exports.titan_global:guiComboBoxAdjustHeight(GUIEditor.combobox.sup, count)
	        ...
	        for i, rank in ipairs(staffTitles[3]) do
	       	...
	       	exports.titan_global:guiComboBoxAdjustHeight(GUIEditor.combobox.vct, count)
	        ...
	        for i, rank in ipairs(staffTitles[4]) do
	       	...
	       	exports.titan_global:guiComboBoxAdjustHeight(GUIEditor.combobox.scripter, count)
	        ...
	        for i, rank in ipairs(staffTitles[5]) do
	       	...
	       	exports.titan_global:guiComboBoxAdjustHeight(GUIEditor.combobox.mapper, count)
...
function formatRating(rating)
	if not rating or not tonumber(rating) then
		...
		return exports.titan_global:round(rating, 2)
	end
end
...
function guiStaffChange()
	...
	if source == GUIEditor.combobox.admin then
		...
		if (not exports.titan_integration:isPlayerSeniorAdmin(localPlayer) or myLevel < admin or myLevel < currentPos[1]) and not exports.titan_integration:isPlayerLeadScripter(localPlayer) then
	...
	elseif source == GUIEditor.combobox.sup then
		...
		if not exports.titan_integration:isPlayerSeniorAdmin(localPlayer) and not exports.titan_integration:isPlayerSupportManager(localPlayer) then
	...
	elseif source == GUIEditor.combobox.vct then
		...
		if not exports.titan_integration:isPlayerSeniorAdmin(localPlayer) and (myLevel <= vct or myLevel <= currentPos[3]) then
	...
	elseif source == GUIEditor.combobox.scripter then
		...
		if not exports.titan_integration:isPlayerLeadScripter(localPlayer) then
	...
	elseif source == GUIEditor.combobox.mapper then
		...
		if not exports.titan_integration:isPlayerLeadScripter(localPlayer) and (myLevel <= mapper or myLevel <= currentPos[5]) then
...
function showConfirmEditStaff()
    ...
    exports.titan_global:centerWindow(GUIEditor.window[2])
