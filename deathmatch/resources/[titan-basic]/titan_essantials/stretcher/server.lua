		...
		if vehicle and isElement(vehicle) then
			...
			if(isElement(patient)) then --and getElementAttachedTo(stretcherArray[playerElement]) == patient
				...
				if not warpResult then
					outputChatBox("The "..tostring(exports.titan_global:getVehicleName(vehicle)).." does not have space for any more patients.", playerElement, 255, 0, 0)
				else
...
function createStretcher( playerElement, vehicle )
	...
	if hasPlayerStretcherSpawned( playerElement ) then
		if vehicle then
			exports.titan_global:sendLocalMeAction(playerElement, "puts the stretcher inside the "..tostring(exports.titan_global:getVehicleName(vehicle))..".")
		else
			exports.titan_global:sendLocalMeAction(playerElement, "puts the stretcher inside.")
		end
		destroyStretcher( playerElement, vehicle )
	else
		if vehicle then
			exports.titan_global:sendLocalMeAction(playerElement, "takes out a stretcher from the "..tostring(exports.titan_global:getVehicleName(vehicle))..".")
		else
			exports.titan_global:sendLocalMeAction(playerElement, "takes out a stretcher.")
		end
...
function movePedOntoStretcher(	targetElement, playerElement )
	...
	exports.titan_global:applyAnimation( targetElement, "CRACK", "crckdeth2", -1, true )
...
function takePedFromStretcher(	targetElement, playerElement )
	...
	exports.titan_global:removeAnimation(targetElement)
