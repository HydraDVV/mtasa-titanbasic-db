...
local Johnson = createPed(211, 1490.73046875, 1308.9599609375, 1093.2890625)
setPedRotation(Johnson, 180)
setElementDimension(Johnson, 144)
	...
	if isInSlot(x, y, w, 25) then
		...
		if getKeyState("mouse1") and lastClick+200 <= getTickCount() then
			...
			if selectedLicense ~= 0 then
				...
				if exports.titan_global:hasMoney(localPlayer, licensecost) then
					if (license == 1) then
						if getElementData(localPlayer, "license.car") < 0 then
							--exports.titan_hud:sendBottomNotification(localPlayer, "Department of Motor Vehicles", "You need to wait another " .. -getElementData(localPlayer, "license.car") .. " hours before being able to obtain a " .. licensetext .. "." )
					...
					elseif (license == 2) then
						if getElementData(localPlayer, "license.bike") < 0 then
							--exports.titan_hud:sendBottomNotification(localPlayer, "Department of Motor Vehicles", "You need to wait another " .. -getElementData(localPlayer, "license.bike") .. " hours before being able to obtain a " .. licensetext .. "." )
					...
					elseif (license == 3) then
						if getElementData(localPlayer, "license.boat") < 0 then
							--exports.titan_hud:sendBottomNotification(localPlayer, "Department of Motor Vehicles", "You need to wait another " .. -getElementData(localPlayer, "license.boat") .. " hours before being able to obtain a " .. licensetext .. "." )
					...
					exports.titan_hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Ehliyeti almak için yeterli paranız yok. Fiyat: $"..licensecost)
				end
			else
				exports.titan_hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Lütfen listeden ehliyet tipini seçiniz." )
			end
		end
	end
...
function showRecoverLicenseWindow()
	...
	addEventHandler("onClientGUIClick", gui.bRecover, function()
		...
		if (row==-1) or (col==-1) then
			exports.titan_hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Lütfen, ehliyet tipini seçiniz!" )
			return false
		end
		...
		if (licensetext=="Car License") then
			if vehiclelicense~= 1 then
				exports.titan_hud:sendBottomNotification(localPlayer, getElementData(dominick, "name"), "Sorry, we are unable to locate a license for you in our records. Please meet agent Zehra Yildiz in the other room." )
				return false
			end
...
function acceptLicense(button, state)
	if (button=="left") then
		if (source==bAcceptLicense) or (source==bBuyLicense)then
			...
			if (row==-1) or (col==-1) then
				exports.titan_hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Lütfen, listeden ehliyet tipiniz seçiniz!" )
				return false
			end
			...
			if (source==bAcceptLicense) then
				if not exports.titan_global:hasMoney( localPlayer, licensecost ) then
					exports.titan_hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Ehliyeti almak için yeterli paranız yok. Fiyat: $"..licensecost)
					return false
				end
			end
			...
			if source==bAcceptLicense then
				...
				elseif (license == 5) then
					if getElementData(localPlayer, "license.fish") < 0 then
						exports.titan_hud:sendBottomNotification(localPlayer, "Department of Motor Vehicles", "You need to wait another " .. -getElementData(localPlayer, "license.fish") .. " hours before being able to obtain a " .. licensetext .. "." )
		...
		if veh and getVehicleOccupant( veh ) == getLocalPlayer( ) then
			if isElementFrozen( veh ) and getVehicleEngineState( veh ) then
				if getVehicleType(veh) == 'veh' or getVehicleType(veh) == 'Bike' then
					exports.titan_hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Motorun ayaklığını indirmeniz gerekmektedir. /kickstand yazarak indirebilirsiniz." )
				elseif getVehicleType(veh) == 'Boat' then
					exports.titan_hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Çapanızı geri çekmeniz gerekmektedir. /anchor yazarak geri çekebilirsiniz." )
				else
					exports.titan_hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Aracınızın el freni kaldırılmış. /handbrake yazarak indirebilirsiniz." )
				end
			elseif not getVehicleEngineState( veh ) then
				exports.titan_hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Aracınızın motoru çalışmamaktadır. 'J' tuşuna basarak motoru çalıştırabilirsiniz." )
			end
		end
	end
)