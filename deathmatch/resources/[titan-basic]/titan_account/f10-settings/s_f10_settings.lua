...
function loadASettingsCallback(queryHandler, player, id)
	...
	if rows > 0 then
		for index, row in pairs(result) do
			if (row["name"] == 'duty_admin' and not exports.titan_integration:isPlayerTrialAdmin(player)) or (row["name"] == 'duty_supporter' and not exports.titan_integration:isPlayerSupporter(player)) then
				row["value"] = 0
			end
