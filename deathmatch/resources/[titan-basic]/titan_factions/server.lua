	...
	if foundPackage and canPlayerUseDutyPackage(thePlayer, foundPackage[1]) then
		...
		for itemIndexID, itemTableContent in ipairs(itemTable) do
			if itemTableContent[2] > 0 then -- its a real item
				exports.titan_global:giveItem(thePlayer, itemTableContent[2], itemTableContent[3]) 
			else -- Its a weapon :O!
				if itemTableContent[2] == -100 then
					...
					local weaponSerial = exports.titan_global:createWeaponSerial(2, characterDatabaseID)
					exports.titan_global:giveItem(thePlayer, 115, gtaWeaponID ..":".. weaponSerial ..":" .. getWeaponNameFromID ( gtaWeaponID ) .. " (D)"  ) 
					
					local gtaweaponcap = exports.titan_weaponcap:getGTACap(gtaWeaponID)
					...
					if packsToGive > 0 and hasAmmo then
						for i=1,packsToGive do 
							exports.titan_global:giveItem(thePlayer, 116, gtaWeaponID ..":".. gtaweaponcap ..":Ammopack for " .. getWeaponNameFromID ( gtaWeaponID ) .. " (D)"  ) 
						end
					end
		...
		exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "duty", grantID, true)
	...
	if tonumber(grantID) > 0 then
		...
		for itemSlot, itemCheck in ipairs(items) do
			if (itemCheck[1] == 115) then -- Weapon
				...
				local serialNumberCheck = exports.titan_global:retrieveWeaponDetails(itemCheckExplode[2])
				if (tonumber(serialNumberCheck[2]) == 2) then -- /duty spawned
					exports['titan_items']:takeItemFromSlot(thePlayer, itemSlot - correction, false)
		...
		if foundPackage then
			for itemIndexID, itemTableContent in ipairs(foundPackage[5]) do
				if itemTableContent[2] > 0 then -- its a real item
					exports.titan_global:takeItem(thePlayer, itemTableContent[1], itemTableContent[2]) 
				end
			end
		end
		...
		if savedSkin then
			...
			exports['titan_items']:doItemGiveawayChecks(thePlayer, 16)

			dbExec(mysql:getConnection(), "UPDATE characters SET duty = '0' WHERE id = '" .. (getElementData( thePlayer, "dbid" )).."'" )	
		end
		
		exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "duty", 0, true)
