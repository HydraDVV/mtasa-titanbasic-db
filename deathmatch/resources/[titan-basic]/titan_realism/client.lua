...
local font = exports.titan_fonts:getFont("BoldFont", 15)
...
function drawSpeedo()
	...
	if (vehicle) then
		speed = exports.titan_global:getVehicleVelocity(vehicle, getLocalPlayer())
		local x = width
		local y = height
		local ax, ay = x - 180, y/2
				
		dxDrawShadowText("hız:", ax, ay-50, 5, 5, tocolor(230, 230, 230, 255), 1, "diploma", "left", "top", false, false, false, true, false)
		dxDrawShadowText(tostring(math.floor(speed)).." km/h", ax+50, ay-50, 5, 5, tocolor(230, 230, 230, 180), 1, "diploma", "left", "top", false, false, true, true, false)
		speed = speed - 100
...
function drawFuel()
	...
	if (vehicle) then
		...
		local FuelPer = (fuel/exports["titan_vehicle_fuel"]:getMaxFuel(vehicle))*100

		if FuelPer > 100 then
			FuelPer = fuel 
		end
		local ax, ay = x - 180, y/2
		dxDrawShadowText("yakıt:", ax, ay-20, 5, 5, tocolor(230, 230, 230, 255), 1, "diploma", "left", "top", false, false, false, true, false)
		dxDrawShadowText(tostring(math.floor(getElementData(vehicle, "fuel") or 0)).." lt", ax+75, ay-20, 5, 5, tocolor(230, 230, 230, 180), 1, "diploma", "left", "top", false, false, true, true, false)
	end
end
...
function deactivateCruiseControl()
    ...
    setPedControlState("accelerate",false)
    ccEnabled = false
    exports.titan_hud:sendBottomNotification(localPlayer, "Hız Sabitleyici", "Hız sabitleyiciyi kapattınız.")
end
				...
				if (ccEnabled) then
					...
					if targetSpeed > 10 then
						if (getVehicleType(theVehicle) == "Automobile" or getVehicleType(theVehicle) == "Bike" or getVehicleType(theVehicle) =="Boat" or getVehicleType(theVehicle) == "Train" or getVehicleType(theVehicle) == "Plane" or getVehicleType(theVehicle) == "Helicopter") then
							exports.titan_hud:sendBottomNotification(localPlayer, "Hız Sabitleme", "Hızı düşürmek için '-', arttırmak için '+'ya basın.")
							activateCruiseControl()
						end
					else
                        exports.titan_hud:sendBottomNotification(localPlayer, "Hız Sabitleyici", "Hız sabitleyici, hızını korumak için kullanılabilir.")
					end
				end
            else
                exports.titan_hud:sendBottomNotification(localPlayer, "Hız Sabitleyici", "Motor açık değil!")
			end
		end
	end
end
...
function restrictBikes(manual) 
    ...
    if (tV) then
        local maxSpeed = limitSpeed[getElementModel(tV)]
        if maxSpeed then 
            tS = exports.titan_global:getVehicleVelocity(tV) 
            if tS > maxSpeed then 
...
function doCarAlarm(vehicle)
	if isElement(vehicle) then
		...
		if exports['titan_global']:hasItem(vehicle, 130) == true then
...
function updateCig()
	...
	for thePlayer, theObject in pairs(l_cigar) do
		if (isElement(thePlayer)) then
			...
			local ratio = r/360
		    exports.titan_bone_attach:attachElementToBone(theObject,thePlayer,11,0.03,0.02,0.13,0,180,0)
		end
	end
	...
	for thePlayer, theObject in pairs(r_cigar) do
		if (isElement(thePlayer)) then
			...
			exports.titan_bone_attach:attachElementToBone(theObject,thePlayer,12,0,0.02,0.13,0,180,0)
...
local newfont = dxCreateFont ( ":titan_fonts/files/Roboto.ttf" , 12)
...
function playSoundHandbrake(state)
	if state == "off" then
		local sound = playSound(":titan_resources/hb_off.mp3")
		if sound then
			setSoundVolume(sound , 1)
		end
	else
		local sound = playSound(":titan_resources/hb_on.mp3")
		if sound then
		...
		if isElementStreamedIn(value) and (isElementOnScreen(value)) and (tV~=value) then
			...
			if (isLineOfSightClear(px, py, pz, x, y, z, true, false, false, true, false, false, true, tV)) then
				...
				if (tx) then
					...
					local realName = exports.titan_global:getVehicleName(value)
