...
itemworld = exports['titan_item-world']
item = exports['titan_items']
	...
	if (element) and (getElementType(element)=="object") and (button=="right") and (state=="down") then
		...
		if (getDistanceBetweenPoints3D(x, y, z, wx, wy, wz)<=maxDistance) then
			...
			if(model == 2517) then --SHOWERS
				rcMenu = exports.titan_rightclick:create("Shower")
				if showering[1] then
					row.a = exports.titan_rightclick:addrow("Stop showering")
					addEventHandler("onClientGUIClick", row.a,  function (button, state)
						takeShower(element)
					end, false)
				else
					row.a = exports.titan_rightclick:addrow("Take a shower")
					addEventHandler("onClientGUIClick", row.a,  function (button, state)
						takeShower(element)
					end, false)
				end
			--[[elseif(model == 2964) then --Pool table / Billiard
				rcMenu = exports.titan_rightclick:create("Pool Table")
				row.a = exports.titan_rightclick:addrow("New Game")
				addEventHandler("onClientGUIClick", row.a,  function (button, state)
					outputDebugString("object-interaction: triggering billiard")
					--exports['titan_minigame-billiard'].startNewGame(element, getLocalPlayer())
			...
			elseif(model == 2146) then --Stretcher (ES)
				rcMenu = exports.titan_rightclick:create("Stretcher")
				row.a = exports.titan_rightclick:addrow("Take Stretcher")				
				addEventHandler("onClientGUIClick", row.a,  function (button, state)
			...
			elseif(model == 962) then --Airport gate control box
				...
				if airGateID then
					rcMenu = exports.titan_rightclick:create("Control Box")
					row.a = exports.titan_rightclick:addrow("Control Gate")				
					addEventHandler("onClientGUIClick", row.a,  function (button, state)
