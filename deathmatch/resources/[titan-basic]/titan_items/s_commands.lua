...
function breathTest(thePlayer, commandName, targetPlayer)
	if (exports.titan_global:hasItem(thePlayer, 53)) then
			...
			if targetPlayer then -- is the player online?
				...
				if (logged==0) then -- Are they logged in?
					...
					if (getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)>4) then -- Are they standing next to each other?
						...
						exports.titan_global:sendLocalMeAction(thePlayer, "issues "..targetPlayerName.." " .. badge[2] .. ", reading " .. badgeNumber .. ".")
						exports.titan_global:giveItem(targetPlayer, itemID, prefix .. badgeNumber)
					end
				end
			end
		end
...
function writeNote(thePlayer, commandName, ...)
	...
	elseif getElementData(thePlayer, 'note-timeout') and math.abs(getElementData(thePlayer, 'note-timeout') - tick) < 5000 then
		...
		exports.titan_global:sendLocalMeAction(thePlayer, "writes a note on a piece of paper.")
		setElementData(thePlayer, 'note-timeout', tick, false)
	end
	else
		outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Bu komutu kullanabilmek için A5+ yetkiliye ihtiyacın var.", thePlayer, 255, 0, 0, true)
	end
end
...
function changeLock(thePlayer)
		...
		if theVehicle then
			...
			if dbid > 0 then
				exports.titan_logs:logMessage( "[VEHICLE] " .. getPlayerName( thePlayer ) .. " changed the lock for Vehicle #" .. dbid .. " (" .. exports.titan_global:getVehicleName(theVehicle).." - "..getVehicleName( theVehicle ) .. ")", 16)

				exports['titan_vehicle_manager']:addVehicleLogs(dbid, 'changelock', thePlayer)
			...
			local dbid, entrance, exit, interiortype = exports['titan_interiors']:findProperty( thePlayer )
			if dbid > 0 then
				if interiortype == 2 then
					...
					exports.titan_logs:logMessage( "[HOUSE] " .. getPlayerName( thePlayer ) .. " changed the lock for House #" .. dbid .. ")", 16) 

					exports['titan_interiormanager']:addInteriorLogs(dbid, 'changelock', thePlayer)
...
function DeleteMoneyItem(thePlayer)
    if exports.titan_global:hasItem(thePlayer, 134) then
	    exports.titan_global:takeItem(thePlayer, 134)
	end
end
