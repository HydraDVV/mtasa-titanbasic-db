...
function giveMoney(thePlayer, amount)
	...
	elseif thePlayer and isElement(thePlayer) and amount > 0 then
		...
		if getElementType(thePlayer) == "player" then
			...
			if not exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "money", getMoney( thePlayer ) + amount, true ) then
				return false
			end
		...
		elseif getElementType(thePlayer) == "team" then
			return dbExec(mysql:getConnection(), "UPDATE factions SET bankbalance = bankbalance + " .. amount .. " WHERE id = " .. getElementData( thePlayer, "id" ) ) and exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "money", getMoney( thePlayer ) + amount, true ) 
		end
	end
	return false
end
	...
	elseif thePlayer and isElement(thePlayer) and amount > 0 then
		...
		elseif hasMoney(thePlayer, amount) then
			if getElementType(thePlayer) == "player" then
				...
				if not exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "money", money - amount, true )	then
					return false
				end
			...
			--	triggerClientEvent(thePlayer, "moneyUpdateFX", thePlayer, false, amount)
				return true, amount
			elseif getElementType(thePlayer) == "team" then
				return dbExec(mysql:getConnection(), "UPDATE factions SET bankbalance = bankbalance - " .. amount .. " WHERE id = " .. getElementData( thePlayer, "id" ) ) and exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "money", money - amount, true )	
			end
			return false, 0
		end
	end
...
function setMoney(thePlayer, amount, onSpawn)
	...
	if thePlayer and isElement(thePlayer) and (amount >= 0 or onSpawn) then
		...
		if getElementType(thePlayer) == "player" then
			...
			while exports['titan_items']:takeItem(thePlayer, 134) do
			...
			if not exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "money", amount, true ) then
				return false
			end

			
			if amount > 0 then
				exports.titan_global:giveItem(thePlayer, 134, amount)
			end
			...
			return true
		elseif getElementType(thePlayer) == "team" then
			return dbExec(mysql:getConnection(), "UPDATE factions SET bankbalance = " .. amount .. " WHERE id = " .. getElementData( thePlayer, "id" ) ) and exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "money", amount, true)
		end
	end
end
