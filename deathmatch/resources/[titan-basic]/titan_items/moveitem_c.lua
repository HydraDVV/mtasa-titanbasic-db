	...
	if getElementType(element)=="interior" then
		...
		if not hasKey(getElementData(element, "dbid"), element) and interiorType~=2 then
			...
			if interiorType==2 then
				if (not exports.titan_factions:isInFactionType(getLocalPlayer(), 2) and not exports.titan_factions:isInFactionType(getLocalPlayer(), 3) and getElementData(getLocalPlayer(), "duty_admin") ~= 1) then
					local row = guiGridListAddRow(gElementItems)
...
function hasKey( key, element )
	...
	if exports.titan_global:hasItem(getLocalPlayer(), 4, key) or exports.titan_global:hasItem(getLocalPlayer(), 5,key) then
		return true, false
	elseif getElementData(getLocalPlayer(), "duty_admin") == 1 then
		...
		if interiorType==2 then
			if (not exports.titan_factions:isInFactionType(getLocalPlayer(), 2) and not exports.titan_factions:isInFactionType(getLocalPlayer(), 3)) then
				return false, false
			else
				return true, true
			end
...
local function moveFromElement( button )
	...
	if button == "left" and col ~= -1 and row ~= -1 then
		...
		if slot < 0 --[[and exports.titan_integration:isPlayerTrialAdmin(localPlayer)]] then
			...
			if item then
				...
				if (hasKey(getElementData(element, "dbid"), element) and getElementType(element)=="interior") or getElementType(element)~="interior" then
					if itemID < 0 and itemID ~= -100 then -- weapon
						local free, totalfree = exports.titan_weaponcap:getFreeAmmo( -itemID )
						local cap = exports.titan_weaponcap:getAmmoCap( -itemID )
						if totalfree == 0 then
...
local function adminCheck( button )
	...
	if button == "right" and col ~= -1 and row ~= -1 then
		...
		if exports.titan_global:isStaffOnDuty(getLocalPlayer()) then
...
local function openElementInventory( ax, ay )
	if not getElementData(localPlayer, "exclusiveGUI") then
		...
		if not elementtype then
			if getElementParent(getElementParent(source)) == getResourceRootElement(getResourceFromName("titan_item-world")) then
		...
		wInventory = guiCreateStaticImage(ax-w/2, ay-h/2, w, h, ":titan_resources/images/window_body.png",false)
...
function collisionFix()
	for key, value in pairs( getElementsByType( "object", getResourceRootElement( getResourceFromName("titan_item-world") ) ) ) do
