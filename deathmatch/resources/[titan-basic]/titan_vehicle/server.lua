...
local threads = { }
local syntaxTable = {
	["s"] = "#00a8ff["..exports["titan_pool"]:getServerName().." RPG]#ffffff ",
	["e"] = "#e84118["..exports["titan_pool"]:getServerName().." RPG]#ffffff ",
	["w"] = "#fbc531["..exports["titan_pool"]:getServerName().." RPG]#ffffff ",
}
...
function createPermVehicle(thePlayer, commandName, ...)
	if exports.titan_integration:isPlayerHeadAdmin(thePlayer) then
		...
		if (#args < 7) then
			...
			local vehShopData = exports["titan_vehicle_manager"]:getInfoFromVehShopID(tonumber(args[1]))
			...
			local targetPlayer, username = exports.titan_global:findPlayerByPartialNick(thePlayer, userName)
			local loadstate = 0
			if targetPlayer then
				...
				if (factionVehicle==1) then
					...
					to = theTeam
					loadstate = 1
					if not exports.titan_global:takeMoney(theTeam, cost) then
					...
					factionVehicle = -1
					to = targetPlayer
					if not exports.titan_global:takeMoney(targetPlayer, cost) then
						...
						return
					elseif not exports.titan_global:canPlayerBuyVehicle(targetPlayer) then
						...
						exports.titan_global:giveMoney(targetPlayer, cost)
						return
					end
				end
				...
				if not (veh) then
					outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Hatalı Araç ID'si.", thePlayer, 255, 128, 128, true)
					exports.titan_global:giveMoney(to, cost)
				else
					...
					local var1, var2 = exports['titan_vehicle']:getRandomVariant(id)
					...
					if (insertid) then
						if (factionVehicle==-1) then
							exports.titan_global:giveItem(targetPlayer, 3, tonumber(insertid))
						end
						...
						if (hiddenAdmin==0) then
							exports.titan_global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " " .. getPlayerName(thePlayer) .. " ("..adminUsername..") has spawned a "..vehicleName .. " (ID #" .. insertid .. ") to "..owner.." for $"..cost..".")
							...
							exports.titan_global:sendMessageToAdmins("AdmCmd: A Hidden Admin has spawned a "..vehicleName .. " (ID #" .. insertid .. ") to "..owner.." for $"..cost..".")
...
function loadOneVehicle(id, hasCoroutine, loadDeletedOne)
	...
	if veh then
		...
		exports["titan_items"]:loadItems(veh)
		exports.titan_pool:allocateElement(veh, row.id)
		...
		if #customTextures > 0 then
			for somenumber, texture in ipairs(customTextures) do
				exports["titan_item-texture"]:addTexture(veh, texture[1], texture[2])
			end
		end
		...
		local hasInterior, interior = exports['titan_vehicle_interiors']:add( veh )
		...
		if getResourceFromName("titan_vehicle_manager") then
			exports["titan_vehicle_manager"]:loadCustomVehProperties(tonumber(row.id), veh) -- Hanz / LOAD CUSTOM VEHICLE PROPERTIES AND HANDLING
		end
	end
end
...
function vehicleRespawn(exploded)
	...
	setElementData(source, "fuel", exports["titan_vehicle_fuel"]:getMaxFuel(vehid))
	...
	if veh and inVehicle == 1 then
		...
		if (seat == 0) then
			...
			if not (enginelessVehicle[model]) then
				...
				local vehKey = exports['titan_global']:hasItem(source, 3, vehID)
				if engine == 0 then
					...
					--if (vehKey) or (owner < 0) and (faction == -1) or (playerFaction == faction) and (faction ~= -1) or ((source:getData("duty_admin") or 0) == 1) then
						...
						elseif exports.titan_global:hasItem(veh, 74) then
							while exports.titan_global:hasItem(veh, 74) do
								exports.titan_global:takeItem(veh, 74)
							end
							...
							local vehicleRunnerTimer = setTimer(
								function()
									if randomVehicleEngine ~= 1 then
										...
										setElementData(veh, "lastused", exports.titan_datetime:now(), true)
...
function toggleLock(source, key, keystate)
	...
	elseif not veh then
		if getElementDimension(source) >= 19000 then
			local vehicle = exports.titan_pool:getElement("vehicle", getElementDimension(source) - 20000)
			if vehicle and exports['titan_vehicle_interiors']:isNearExit(source, vehicle) then
		...
		local interiorFound, interiorDistance = exports['titan_interiors']:lockUnlockHouseEvent(source, true)

		local x, y, z = getElementPosition(source)
		local nearbyVehicles = exports.titan_global:getNearbyElements(source, "vehicle", 30)
		...
		for i, veh in ipairs(nearbyVehicles) do
			...
			if shortest > distanceToVehicle and ( exports.titan_global:isStaffOnDuty(source) or exports.titan_global:hasItem(source, 3, dbid) or (source:getData("faction") > 0 and source:getData("faction") == veh:getData("faction")) ) then
...
function sellVehicle(thePlayer, commandName, targetPlayerName)
	if isPedInVehicle(thePlayer) then
		if not targetPlayerName then
			exports["titan_infobox"]:addBox(thePlayer, "info", "/sell [Karakter Adı & ID]")
		else
			...
			if targetPlayer and getElementData(targetPlayer, "dbid") then
				...
				if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) < 20 then
					...
					if theVehicle then
						...
						if getElementData(theVehicle, "owner") == getElementData(thePlayer, "dbid") or exports.titan_integration:isPlayerTrialAdmin(thePlayer) then
							if getElementData(targetPlayer, "dbid") ~= getElementData(theVehicle, "owner") then
								if exports.titan_global:hasSpaceForItem(targetPlayer, 3, vehicleID) then
									if exports.titan_global:canPlayerBuyVehicle(targetPlayer) then
											...
											if query then
												exports.titan_anticheat:changeProtectedElementDataEx(theVehicle, "owner", getElementData(targetPlayer, "dbid"), true)
												exports.titan_anticheat:changeProtectedElementDataEx(theVehicle, "owner_last_login", exports.titan_datetime:now(), true)
												exports.titan_anticheat:changeProtectedElementDataEx(theVehicle, "lastused", exports.titan_datetime:now(), true)

												exports.titan_global:takeItem(thePlayer, 3, vehicleID)

												if not exports.titan_global:hasItem(targetPlayer, 3, vehicleID) then
													exports.titan_global:giveItem(targetPlayer, 3, vehicleID)
												end

												exports.titan_logs:logMessage("[SELL] car #" .. vehicleID .. " was sold from " .. getPlayerName(thePlayer):gsub("_", " ") .. " to " .. targetPlayerName, 9)
												exports["titan_infobox"]:addBox(thePlayer, "success", "Başarıyla " .. getVehicleName(theVehicle) .. " isimli aracınızı " .. targetPlayerName .. " kişisine devrettiniz.")
												exports["titan_infobox"]:addBox(targetPlayer, "success", ""..getPlayerName(thePlayer) .. " isimli oyuncu size " .. getVehicleName(theVehicle) .. " model aracını devretti.")
												...
												exports.titan_logs:dbLog(thePlayer, 6, { theVehicle, thePlayer, targetPlayer }, "SELL '".. getVehicleName(theVehicle).."' '".. (getPlayerName(thePlayer):gsub("_", " ")) .."' => '".. targetPlayerName .."'")
											end
									else
								...
								outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Kendi aracınızı kendinize satamazsınız.", thePlayer, 255, 0, 0, true)
							end
						else
							outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Bu araç sana ait değil.", thePlayer, 255, 0, 0, true)
						end
					else
						outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Araçta olmak zorundasınız.", thePlayer, 255, 0, 0, true)
					end
				else
					outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 " .. targetPlayerName .. " isimli kişiye çok uzaksınız.", thePlayer, 255, 0, 0, true)
				end
			end
		end
	end
...
function AdminVehicleSale(thePlayer, commandName, args)
	if isPedInVehicle(thePlayer) and exports.titan_integration:isPlayerSeniorAdmin(thePlayer) then
		local vehType = getVehicleType(getPedOccupiedVehicle(thePlayer))
		if ( vehType == ("Plane" or "Helicopter" or "Boat") or (getElementData(thePlayer, "temporarySell") == true ) or (exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer)) ) and not args then
			exports["titan_infobox"]:addBox(thePlayer, "info", "/sell [Karakter Adı & ID]")
		elseif ( vehType == ("Plane" or "Helicopter" or "Boat") or (getElementData(thePlayer, "temporarySell") == true ) or (exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer)) ) and args then
...
function setRealInVehicle(thePlayer)
	if isVehicleLocked(source) then
		...
		local birlika = 'Birlik Yok'
							if birlika == 0 then
								birlika = exports.titan_cache:getFactionNameFromId(birlika)
							end
		...
		elseif (faction==-1) and (owner>0) then
			local ownerName = exports['titan_cache']:getCharacterName(owner)
...
function removeFromFactionVehicle(thePlayer)
	...
	if (vfaction~=-1) then
		...
		for key, value in ipairs(exports.titan_pool:getPoolElementsByType("team")) do
	...
	if (vjob>0) and (seat==0) then
		for key, value in pairs(exports['titan_items']:getMasks()) do
			if getElementData(thePlayer, value[1]) then
				exports.titan_global:sendLocalMeAction(thePlayer, value[3] .. ".")
...
function doBreakdown()
	if exports.titan_global:hasItem(source, 74) then
		while exports.titan_global:hasItem(source, 74) do
			exports.titan_global:takeItem(source, 74)
		end
...
function lockUnlockInside(vehicle)
	...
	--if (owner ~= -1) then
		if ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
			if not locklessVehicle[model] or exports.titan_global:hasItem( source, 3, dbid ) then
...
function lockUnlockOutside(vehicle)
	if (not source or exports.titan_integration:isPlayerTrialAdmin(source)) or ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
		local dbid = getElementData(vehicle, "dbid")
...
function fillFuelTank(veh, fuel)
	...
	local engine = veh:getData("engine")
	local max = exports["titan_vehicle_fuel"]:getMaxFuel(getElementModel(veh))
	...
	elseif (engine==1) then
		...
		exports.titan_global:takeItem(source, 57, fuel)
		exports.titan_global:giveItem(source, 57, math.ceil(fuel-fuelAdded))
...
function removeNOS(theVehicle)
	...
	exports['titan_savevehicle']:saveVehicleMods(theVehicle)
	exports.titan_logs:dbLog(source, 4, {  theVehicle }, "MODDING REMOVENOS")
end
	...
	if (requires) then
		if (requires==1) then
			...
			if (id==dbid) then
				...
				call( getResourceFromName( "titan_items" ), "clearItems", veh )
				call( getResourceFromName( "titan_items" ), "deleteAll", 3, id )
			end
		end
	end
end
...
function setVehiclePosition(thePlayer, commandName)
	...
	if veh then
		if isElementWithinColShape( thePlayer, HospitalCol ) and getElementData( thePlayer, "faction" ) ~= 2 and not exports.titan_integration:isPlayerTrialAdmin(thePlayer) and not exports.titan_integration:isPlayerSupporter(thePlayer) then
			outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Sadece Los Santos Medical Department üyeleri bu bölgeye araç parkında bulunabilir.", thePlayer, 255, 128, 128, true)
		elseif isElementWithinColShape( thePlayer, PershingSquareCol ) and getElementData( thePlayer, "faction" ) ~= 1  and not exports.titan_integration:isPlayerTrialAdmin(thePlayer) and not exports.titan_integration:isPlayerSupporter(thePlayer) then
			outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Sadece Los Santos Police Department üyeleri bu bölgeye araç parkında bulunabilir.", thePlayer, 255, 128, 128, true)
		else
			...
			if (owner==playerid) or (exports.titan_global:hasItem(thePlayer, 3, dbid)) or (exports.titan_integration:isPlayerSupporter(thePlayer) and  exports.titan_logs:logMessage("[AVEHPOS] " .. getPlayerName( thePlayer ) .. " parked car #" .. dbid .. " at " .. x .. ", " .. y .. ", " .. z, 9)) or (exports.titan_integration:isPlayerTrialAdmin(thePlayer) and exports.titan_logs:logMessage("[AVEHPOS] " .. getPlayerName( thePlayer ) .. " parked car #" .. dbid .. " at " .. x .. ", " .. y .. ", " .. z, 9)) then
				if (dbid<0) then
					outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Bu araç kalıcı olarak park edilemez.", thePlayer, 255, 128, 128, true)
				else
					...
					exports["titan_infobox"]:addBox(thePlayer, "info", "Başarıyla bulunduğunuz konuma aracınızı parkladınız.")
					exports.titan_logs:dbLog(thePlayer, 4, {  veh }, "PARK")
					...
					if ( veh:getData("Impounded") or 0 ) > 0 then
						local owner = getPlayerFromName( exports['titan_cache']:getCharacterName( getElementData( veh, "owner" ) ) )
						if isElement( owner ) and exports.titan_global:hasItem( owner, 2 ) then
	...
	if thePlayer and seat == 0 then
		if getElementData(thePlayer, "autopark") == "1" then
			...
			if source:getData("owner") > -1 and dbid > 0 then
				if exports.titan_global:hasItem(thePlayer, 3, dbid) or exports.titan_global:hasItem(source, 3, dbid) then
					local x, y, z = getElementPosition(source)
...
function setVehiclePosition2(thePlayer, commandName, vehicleID)
	if exports.titan_integration:isPlayerTrialAdmin( thePlayer ) then
		...
		if not vehicleID or vehicleID < 0 then
			...
			local veh = exports.titan_pool:getElement("vehicle", vehicleID)
			if veh then
				...
				exports.titan_logs:logMessage("[AVEHPOS] " .. getPlayerName( thePlayer ) .. " parked car #" .. vehicleID .. " at " .. x .. ", " .. y .. ", " .. z, 9)
			else
...
function setVehiclePosition3(veh)
	if isElementWithinColShape( source, HospitalCol ) and getElementData( source, "faction" ) ~= 2 and not exports.titan_integration:isPlayerTrialAdmin(source) then
	...
	elseif isElementWithinColShape( source, PershingSquareCol ) and getElementData( source, "faction" ) ~= 1  and not exports.titan_integration:isPlayerTrialAdmin(source) then
		...
		local dbid = veh:getData("dbid")
		local x, y, z = getElementPosition(veh)
		if (owner==playerid) or (exports.titan_global:hasItem(source, 3, dbid)) or (exports.titan_integration:isPlayerTrialAdmin(source) and exports.titan_logs:logMessage("[AVEHPOS] " .. getPlayerName( source ) .. " parked car #" .. dbid .. " at " .. x .. ", " .. y .. ", " .. z, 9)) then
			if (dbid<0) then
	...
	if (quitReason == "Timed out") then -- if timed out
		if (isPedInVehicle(source)) then -- if in vehicle
			...
			if (vehicleSeat == 0) then	-- is in driver seat?
				...
				if exports.titan_global:hasItem(theVehicle, 3, dbid) then
					exports.titan_global:takeItem(theVehicle, 3, dbid)
					exports.titan_global:giveItem(source, 3, dbid)
				end
				--------------------------------------------
				...
				if not (passenger1) and not (passenger2) and not (passenger3) then
					...
					local playerFaction = tonumber(source:getData("faction"))
					if exports.titan_global:hasItem(source, 3, dbid) or ((playerFaction == vehicleFaction) and (vehicleFaction ~= -1)) then
...
function controlRamp(theVehicle)
	...
	if not (exports['titan_items']:hasItem(theVehicle, 117)) then
	...
	if not (rampObject) or not (isElement(rampObject)) then
		if (getElementModel(theVehicle) == 578) then
			...
			exports.titan_anticheat:changeProtectedElementDataEx(theVehicle, "vehicle:ramp:object", rampObject, false)
		end
	else
		destroyElement(rampObject)
		exports.titan_anticheat:changeProtectedElementDataEx(theVehicle, "vehicle:ramp:object", nil, false)
	end
end
...
function handleOdoMeterRequest(totalDistance, syncDistance)
	if not totalDistance then
		...
		if theVehicle == source then
			...
			if theSeat == 0 then
				local totDistance = getElementData(theVehicle,"odometer") or 0
				exports.titan_anticheat:changeProtectedElementDataEx(theVehicle, 'odometer', totDistance + syncDistance, false )
			end
		end
	end
end
...
function getVehicleOwner(vehicle)
	...
	if faction > 0 then
		return getTeamName(exports.titan_pool:getElement('team', faction))
	else
		return call(getResourceFromName("titan_cache"), "getCharacterName", getElementData(vehicle, "owner")) or "N/A"
	end
end
		...
		if hasVehicleWindows(theVehicle) then
			if (getVehicleOccupant(theVehicle) == thePlayer) or (getVehicleOccupant(theVehicle, 1) == thePlayer) then
				if not (isVehicleWindowUp(theVehicle)) then
					exports.titan_anticheat:changeProtectedElementDataEx(theVehicle, "vehicle:windowstat", 0, true)
					exports.titan_global:sendLocalMeAction(thePlayer, "panelden aracın camını kapatır.")
					...
					exports.titan_anticheat:changeProtectedElementDataEx(theVehicle, "vehicle:windowstat", 1, true)
					exports.titan_global:sendLocalMeAction(thePlayer, "panelden aracın camını açar.")
					for i = 0, getVehicleMaxPassengers(theVehicle) do
						local player = getVehicleOccupant(theVehicle, i)
						if (player) then
							triggerClientEvent(player, "updateWindow", theVehicle)
