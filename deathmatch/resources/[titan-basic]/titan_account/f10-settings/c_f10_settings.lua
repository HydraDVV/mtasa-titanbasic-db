...
function showSettingsWindow(tab)
	...
	if getResourceFromName("titan_vehicle_radio") then
	...
	if getResourceFromName("titan_logs") then
	...
	if getResourceFromName("titan_logs") and getResourceFromName("titan_phone") then
	...
	if getResourceFromName("titan_chat") then
	...
	if getResourceFromName("titan_chat") and guiCheckBoxGetSelected(GUIEditor_Checkbox.graphic_chatbub) then
	...
	if getResourceFromName("titan_hud") then
	...
	if getResourceFromName("titan_shader_radar") then
	...
	if getResourceFromName("titan_shader_water") then
	...
	if getResourceFromName("titan_shader_car_paint") then
	...
	if getResourceFromName('titan_vehicle') then
	...
	if getResourceFromName("titan_realism") then
	...
	posY = posY + lineH
	local dyn_light = exports.titan_global:isResourceRunning('dynamic_lighting') and true or false
...
--[[
	local groundsnow = exports.titan_global:isResourceRunning('shader_snow_ground') and true or false
	...
	local snowfall = exports.titan_global:isResourceRunning('snow') and true or false
	if snowfall then
		posY = posY + lineH
	...
	local xmas = exports.titan_global:isResourceRunning('xmas') and true or false
	if xmas then
		posY = posY + lineH
	...
	if exports.titan_integration:isPlayerSupporter(getLocalPlayer()) or exports.titan_integration:isPlayerTrialAdmin(getLocalPlayer()) then
		posY = posY + lineH
	...
	--if getResourceFromName("titan_description") then
...
function applyGameSettings(name, value)
	if name and value then
		...
		elseif name == "hide_hud" then
			...
			elseif exports.titan_global:hasItem(localPlayer, 111) then
