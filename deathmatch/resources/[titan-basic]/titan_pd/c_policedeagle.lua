...
function isPDDD2()
	return getElementData(getLocalPlayer(), "faction") == 1 or getElementData(getLocalPlayer(), "faction") == 23
end
...
function switchMode()
	if isPDDD2() and (getPedWeapon(localPlayer)==24) and (getPedTotalAmmo(localPlayer)>0) then -- has an un-empty deagle
		...
		if mode == 0 then -- tazer mode
			...
			outputChatBox( "[-]#f9f9f9 Çok amaçlı tabancanızda öldürücü modunu aktifleştirdiniz.",81, 149, 232, true )
		elseif mode == 1 and ( getElementData(team, "type") == 2 or getElementData(team, "type") == 3 ) then -- lethal mode
			outputChatBox( "[-]#f9f9f9 Çok amaçlı tabancanızda radar modunu aktifleştirdiniz.", 81, 149, 232, true )
		...
		elseif mode == 2 or mode == 1 and ( getElementData(team, "type") == 2 or getElementData(team, "type") == 3 ) then -- radar gun mode
			outputChatBox( "[-]#f9f9f9 Çok amaçlı tabancanızda tazer modunu aktifleştirdiniz.", 81, 149, 232, true )
		...
		if (getElementType(target)=="vehicle") then
			if (getPedWeapon(localPlayer)==24) then
				...
				if (mode==2 and getPedOccupedVehicle( localPlayer ) ~= target) then
					actualspeed = exports.titan_global:getVehicleVelocity(target)
					outputChatBox(exports.titan_global:getVehicleName(target) .. " markali aracin hizi " .. math.floor(actualspeed) .. " km/h.", 255, 194, 14)
				end
			end
		end
	end
