...
function showItemList()
	if (exports.titan_integration:isPlayerSeniorAdmin(localPlayer) or exports.titan_integration:isPlayerTrial(localPlayer) or exports.titan_integration:isPlayerScripter(localPlayer)) then
		if not (wItemsList) then
			local width, height = 800,600
			local scrWidth, scrHeight = guiGetScreenSize()
			local x = scrWidth/2 - (width/2)
			local y = scrHeight/2 - (height/2)
			wItemsList = guiCreateWindow(x, y, width, height,"Envanter Eşya Yönetim Arayüzü | © "..exports["titan_pool"]:getServerName().." Roleplay",false)
