function unsitInHelicopter(vehicle)
	...
	if (isElement(vehicle)) then
		if not (getElementType(vehicle)=="vehicle") then
			...
			for key, value in ipairs(helicopters) do
				...
				if (players) then
					...
					if (removed) then
						exports.titan_anticheat:changeProtectedElementDataEx(value, "players", players, true)
					end
				end
			end
			
			exports.titan_anticheat:changeProtectedElementDataEx(source, "seat")
			detachElements(source, vehicle)
			exports.titan_global:removeAnimation(source)
		elseif (seat) and (seat>0) then
			...
			exports.titan_anticheat:changeProtectedElementDataEx(vehicle, "players", players, true)
			exports.titan_anticheat:changeProtectedElementDataEx(source, "seat")
			detachElements(source, vehicle)
			exports.titan_global:removeAnimation(source)
		end
...
function sitInHelicopter(vehicle)
	...
	--[[for key, value in ipairs(players) do
		...
		elseif not isElementAttached ( value ) then
			exports.titan_anticheat:changeProtectedElementDataEx(value, "seat")
			table.remove(players, key)
	...
	if (not players) or (#players<3) then
		...
		if not (s1) then
			...
			exports.titan_global:applyAnimation(source, "FOOD", "FF_Sit_Look", 999999, true, true, false)
		...
		players[seat] = source
		exports.titan_anticheat:changeProtectedElementDataEx(source, "seat", seat)
