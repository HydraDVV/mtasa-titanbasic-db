...
function doCheck(sourcePlayer, command, ...)
	if (exports.titan_integration:isPlayerTrialAdmin(sourcePlayer) or exports.titan_integration:isPlayerSupporter(sourcePlayer)) then
		if not (...) then
			...
			local noob = exports.titan_global:findPlayerByPartialNick(sourcePlayer, table.concat({...},"_"))
			if (noob) then
									...
									function(qh)
										...
										dbQuery(
											function(qh)
												...
												local adminlevel = exports.titan_global:getPlayerAdminTitle(noob)
				...
				exports.titan_logs:dbLog(thePlayer, 4, ..., "CHECK")
			end
		end
	end
end
...
function showAdminHistory( target )
	...
	if not (exports.titan_integration:isPlayerTrialAdmin( client ) or exports.titan_integration:isPlayerSupporter( client )) then
				...
				if (tonumber(sqlQuery["action"]) == 4) then -- Warning
					...
					for i, player in pairs(getElementsByType("player")) do
						if getElementData(player, "account:id") == tonumber(accountNumber) then
							...
							exports.titan_anticheat:changeProtectedElementDataEx(player, "warns", warns, false)
							break
						end
					end
				end
...
addCommandHandler( "history", 
	function( thePlayer, commandName, ... )
		if not (exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer)) then
			if (...) then
		...
		if (...) then
			targetPlayer = exports.titan_global:findPlayerByPartialNick(thePlayer, table.concat({...},"_"))
		end
		...
		if targetPlayer then
			...
			local accounts, characters = exports.titan_auth:getTableInformations()
			local __index = 0
