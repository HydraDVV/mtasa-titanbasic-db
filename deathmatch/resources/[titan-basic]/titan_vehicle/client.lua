...
function openVehicleDoorGUI( vehicleElement )
	if vControlGUI["titan_pool"] then
		closeVehicleGUI()
		return
	end
	...
	local options = 0
	local guiPos = 30
	vControlGUI["titan_pool"] = guiCreateWindow(700,236,272,288,"Vehicle Control",false)	
	for index, doorEntry in ipairs(doors) do
		vControlGUI["scroll"..index] = guiCreateScrollBar(24,guiPos + 17,225,17,true,false,vControlGUI["titan_pool"])
		vControlGUI["label"..index] = guiCreateLabel(30,guiPos,135,15,doorEntry[1],false,vControlGUI["titan_pool"])
	...
	guiSetSize(vControlGUI["titan_pool"],272,guiPos+40, false)
	vControlGUI["close"] = guiCreateButton(23,guiPos,230,14,"Close",false, vControlGUI["titan_pool"])
...
function closeVehicleGUI()
	if vControlGUI["titan_pool"] then
		destroyElement(vControlGUI["titan_pool"] )
		vControlGUI = { }
...
function updateServerSide(theScrollBar, state)
	if vControlGUI["titan_pool"] then -- and state == "up" then
...
function lockUnlock(button, state)
    if (button=="left") then
        ...
        elseif exports.titan_global:hasItem(localPlayer, 3, getElementData(vehicle, "dbid")) or (exports["titan_factions"]:isPlayerInFaction(localPlayer, getElementData(vehicle, "faction"))) then
    ...
    if(source == localPlayer) then
        ...
        for k,v in ipairs(vehicles) do
            ...
            if(model == 525) then --towtruck
                ...
                return true
            --stage 2: Check items
            elseif(exports.titan_global:hasItem(v, 144)) then --single yellow strobe (airport, etc.)
...
addEventHandler("showVehiclesPanel", root,
	function(table)
		...
		window = guiCreateWindow(0, 0, 389, 366, "İnaktif Araçlarım - "..exports["titan_pool"]:getServerName().." Roleplay", false)
        	...
        	function(b)
        		if (source == ok) then
        			...
        			if row ~= -1 and col ~= -1 then
        				...
        				outputChatBox(exports.titan_pool:getServerSyntax(false, "s").."Araç başarıyla aktif edildi.", 255, 255, 255, true)
        			else
        				outputChatBox(exports.titan_pool:getServerSyntax(false, "e").."Aktif etmek istediğiniz araç ID'yi girin.", 255, 255, 255, true)
        			end
        		end
        	end
        )
