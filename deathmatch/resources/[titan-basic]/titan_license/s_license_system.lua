...
function recoveryLicense(licensetext, cost, itemID, npcName)
	if not exports.titan_global:takeMoney(source, cost) then
		exports.titan_hud:sendBottomNotification(source, npcName, "Can I have $"..exports.titan_global:formatMoney(cost).." fee for recovering the "..licensetext.." please?")
		return false
	end

	if exports.titan_global:giveItem(source, itemID, getPlayerName(source):gsub("_", " ")) then
		exports.titan_hud:sendBottomNotification(source, npcName, "You have paid $"..exports.titan_global:formatMoney(cost).." fee for recovering the "..licensetext..".")
	end
end
...
function onLicenseServer()
	...
	if (gender == 0) then
		exports.titan_global:sendLocalText(source, "Emma Brewles diyor ki: Merhaba beyefendi, ehliyet için mi başvuracaktınız?", 255, 255, 255, 10)
	else
		exports.titan_global:sendLocalText(source, "Emma Brewles diyor ki: Merhaba hanfendi, ehliyet için mi başvuracaktınız?", 255, 255, 255, 10)
	end
end
...
function payFee(amount, reason)
	if exports.titan_global:takeMoney(source, amount) then
		if not reason then
			reason = "a license"
		end
		exports.titan_hud:sendBottomNotification(source, "Motorlu Taşıtlar Departmanı", reason.." için $"..exports.titan_global:formatMoney(amount).." ödediniz.")
	end
end
