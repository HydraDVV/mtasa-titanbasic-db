					...
					if exports.titan_integration:isPlayerTrialAdmin( thePlayer ) or interiorStatus[INTERIOR_OWNER] == getElementData( thePlayer, "dbid" ) then
						...
						if canHazFee then
							...
							if query then
								...
								exports["titan_infobox"]:addBox(thePlayer, "buy", interiorName.." isimli interiorun için giriş ücretini $"..exports.titan_global:formatMoney(theFee).." olarak ayarladın.")
						...
						exports["titan_infobox"]:addBox(thePlayer, "warning", "Bu iş yeri sana ait değil.")
					end
				else
					exports["titan_infobox"]:addBox(thePlayer, "error", "Bu interior bir iş yeri değil.")
				end
			else
				exports["titan_infobox"]:addBox(thePlayer, "warning", "Giriş Ücreti $0 ile $100 arasında olmalıdır.")
			end
		else
			exports["titan_infobox"]:addBox(thePlayer, "error", "Bir interiorda değilsiniz.")
		end
	end
end
...
function reloadInterior(thePlayer, commandName, interiorID)
	if (exports.titan_integration:isPlayerAdminIII(thePlayer)) then
		if not interiorID then
			...
			if dbid ~= 0 then			
				realReloadInterior(dbid)
				outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Interior veritabanında yeniden yükleniyor.. #" .. dbid, thePlayer, 0, 255, 0, true)
			else
				if reloadOneInterior(tonumber(interiorID), false) then
					outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 #" .. tonumber(interiorID).." ID'li interior veritabanında başarıyla yüklendi.", thePlayer, 0, 255, 0)
				end
			end
		end
	end
			...
			if not houseID then
				...
				if dbid ~= 0 then
					...
					if (query) then
						...
						outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Sildiğin interioru geri almak için '/restoreint " .. dbid .. "' yazmalısın.", thePlayer, 255, 194, 14,true )
...
addCommandHandler("moveinterior",
	function(player, cmd, interiorID)
		if (exports.titan_integration:isPlayerHeadAdmin(player) or exports.titan_integration:isPlayerScripter(player)) then
			...
			outputChatBox(exports.titan_pool:getServerSyntax(player, false,"s").."Mülkün giriş pozisyonu başarıyla ayarlandı.", player, 255, 255, 255, true)
...
function createInterior(thePlayer, commandName, interiorId, inttype, cost, ...)
	if exports.titan_integration:isPlayerAdminIII( thePlayer ) then
		...
		elseif not exports.titan_global:takeMoney(getTeamFromName("Los Santos Government"), cost) then
			exports["titan_infobox"]:addBox(thePlayer, "error", "Bu mülkiyeti satın alamazsınız. (Devlet'e ait.)")
		else
			...
			if interior then
				...
				if (query) then
					...
					outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 #" .. id .. " ID'li interior başarıyla oluşturuldu.", thePlayer, 255, 194, 14, true)
					...
					if hiddenAdmin == 0 then
						exports.titan_global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(thePlayer):gsub("_", " ").. " ("..adminUsername..") isimli yetkili bir interior oluşturdu. (#"..id.." İsim: '"..name.."', Tip: "..inttype..", Fiyat: "..cost.." TL)")
					else
						exports.titan_global:sendMessageToAdmins("[INTERIOR]: Gizli bir yetkili interior oluşturdu. (#"..id.." İsim: '"..name.."', Tip: "..inttype..", Fiyat: "..cost.." TL)")
					end
...
function updateInteriorExit(thePlayer, commandName)
	if exports.titan_integration:isPlayerSeniorAdmin( thePlayer )  then
		...
		if (dimension==0) then
			...
			exports["titan_infobox"]:addBox(thePlayer, "success", "Interior'un çıkış yerini güncellediniz.")
...
function changeInteriorName( thePlayer, commandName, ...)
	if (exports.titan_integration:isPlayerAdminIII(thePlayer)) then 
		...
		if not (...) then 
			exports["titan_infobox"]:addBox(thePlayer, "info", "Kullanım: /"..commandName.." [Yeni Mülkiyet İsmi]")
		elseif (dimension==0) then
			exports["titan_infobox"]:addBox(thePlayer, "error", "Bir mülk içerisinde değilsiniz.")
		else
			name = table.concat({...}, " ")
		
			dbExec(mysql:getConnection(),"UPDATE interiors SET name='" .. ( name) .. "' WHERE id='" .. id .. "'")
			exports["titan_infobox"]:addBox(thePlayer, "info", "Mülkiyet adı "..name.." olarak değiştirildi.")
			...
			if hiddenAdmin == 0 then
				...
				exports.titan_global:sendMessageToAdmins("[INTERIOR]: Gizli bir yetkili #"..id.." ID'li interiorun ismini '"..name.."' olarak değiştirdi.")
			end
...
function forceSellProperty(thePlayer, commandName, intID)
	if exports.titan_integration:isPlayerAdminIII(thePlayer)  then
		if not intID or not tonumber(intID) or (tonumber(intID)%1 ~= 0) or (tonumber(intID) <= 0) then
			exports["titan_infobox"]:addBox(thePlayer, "info", "Mülkiyeti satılığa çıkarmanızı sağlar. /forcesell [Interior ID]")
			return
		end
		local possibleInteriors = exports.titan_pool:getPoolElementsByType('interior')
