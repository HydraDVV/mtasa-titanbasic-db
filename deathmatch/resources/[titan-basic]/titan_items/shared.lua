...
local function findVehicleName( value )
	...
	for _, theVehicle in pairs( getElementsByType( "vehicle" ) ) do
		if getElementData( theVehicle, "dbid" ) == value then
			_vehiclecache[value] = exports.titan_global:getVehicleName( theVehicle )
			return _vehiclecache[value]
		end
	end
	return "?"
...
function getItemName(id, value)
	...
	elseif ( id == 4 or id == 5 ) and value then
		local pickup = exports['titan_interiors']:findParent( nil, value )
	...
	elseif(id == 165) then --video disc
		...
		if disc > 1 then
			local discData = exports.titan_fakevideo:getFakevideoData(disc)
			if discData then
	...
	if i then
		...
		elseif id == 150 then--ATM card
			...
			if tonumber( itemExploded[2] ) > 0 then
				return "Card Number: '"..itemExploded[1].."', Owner: '"..tostring(exports.titan_cache:getCharacterNameFromID(itemExploded[2])):gsub("_", " ").."'"
			else
				return "Card Number: '"..itemExploded[1].."', Owner: '"..tostring(exports.titan_cache:getBusinessNameFromID(math.abs(itemExploded[2]))).."'"
			end
