...
function makeInformationIcon(thePlayer, commandName, ...)
	...
	if ... then
		...
		if query then
			...
			exports["titan_infobox"]:addBox(thePlayer, "success", "Başarıyla "..id.." ID'li infoyu oluşturdunuz.")
		else
			outputChatBox("MySQL hatası oluştu, Hanz'a bildiriniz.", thePlayer, 255, 0, 0)
		end
		else
			outputChatBox("Kullanım:#f9f9f9 /infoekle [İçerik]", thePlayer, 255, 194, 14, true)
		end
	end
end
...
function getNearbyInformationIcons(thePlayer, commandName)
	if (exports.titan_integration:isPlayerTrialAdmin(thePlayer)) then
		...
		outputChatBox("Çevrende Bulunan Yazılar:", thePlayer, 255, 126, 0)
		local count = 0
		for key, value in ipairs (informationicons) do
			...
			if dbid then
				...
				if distance <= 10 and getElementDimension(informationicons[key]) == getElementDimension(thePlayer) and getElementInterior(informationicons[key]) == getElementInterior(thePlayer) then
					local createdby = getElementData(informationicons[key], "informationicon:createdby")
					local information = getElementData(informationicons[key], "informationicon:information")
					outputChatBox("   #" .. dbid .. " ID | " .. createdby .. " tarafından | İçerik: " .. information, thePlayer, 255, 126, 0)
					count = count + 1
				end
			end
		end		
		...
		if tonumber(ID) then
			...
			if informationicons[ID] then
				...
				if query then
					...
					--exports["titan_infobox"]:addBox(thePlayer, "success", "Başarıyla "..id.." ID'li infoyu sildiniz.")					
				else
					outputChatBox("MySQL hatası oluştu, Hanz'ya bildiriniz.", thePlayer, 255, 0, 0)
				end
			else
				outputChatBox("Böyle bir info ID'si bulunamadı.", thePlayer, 255, 0, 0)
			end
		else
			outputChatBox("Kullanım:#f9f9f9 /infosil [ID]", thePlayer, 255, 194, 14, true)
		end
	end
end
