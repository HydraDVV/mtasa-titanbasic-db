...
function sendStatus( thePlayer, commandName, ... )
	...
	if string.len(message) > 90 then
		outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Verdiğiniz /status durumu 50 karakteri geçiyor, lütfen daha kısa bir şeyler bulun.", thePlayer, 255, 0, 0, true)
		outputChatBox("[-]#f9f9f9 Sizin girmiş olduğunuz karakter sayısı: "..string.len(message), thePlayer, 50, 255, 255, true)
		return
	end	
	
	exports["titan_infobox"]:addBox(thePlayer, "success", "Başarılı bir şekilde /status durumunu etkinleştirdiniz.")
...
function sendToNearByClientsStatus(root, message)
	...
	for k, v in pairs(exports.titan_pool:getPoolElementsByType("player")) do
