...
function showLibrary(vehs, thePed)
	...
	GUIEditor_Window[1] = guiCreateWindow((sx-w)/2,(sy-h)/2,w,h,"Araç Kütüphanesi - "..exports["titan_pool"]:getServerName().." Development Team / Hanz]",false)
	...
	for i = 1, #vehs do
		...
		guiGridListSetItemText(grid, row, col.price, "$"..exports.titan_global:formatMoney(vehs[i].vehprice), false, false) 
		guiGridListSetItemText(grid, row, col.tax, "$"..exports.titan_global:formatMoney(vehs[i].vehtax), false, false) 
	...
	if thePed and isElement(thePed) and getElementData(thePed, "carshop") then
			...
			function( button )
				if button == "left" then
					...
					if row ~= -1 and col ~= -1 then
						...
						if createdby ~= getElementData(localPlayer, "account:username") and not exports.titan_integration:isPlayerVehicleConsultant(localPlayer)  then
		...
		if exports.titan_integration:isPlayerVehicleConsultant(localPlayer) or exports.titan_integration:isPlayerSeniorAdmin(localPlayer) then
		...
		elseif exports.titan_integration:isPlayerTrialAdmin(localPlayer) or exports.titan_integration:isPlayerVCTMember(localPlayer) then
			guiSetEnabled(GUIEditor_Button[7], true) -- RESTART CARSHOP
			guiSetEnabled(GUIEditor_Button[3], true) -- HANDLING
		end
	end
...
function validateCreateVehicle(data)
	if guiGetText(GUIEditor_Button[9]) == "Create" or guiGetText(GUIEditor_Button[9]) == "Update" then
		...
		elseif vehModel and tonumber(vehModel) then
			guiSetText(GUIEditor_Label[1], "MTA Vehicle Model (OK!):")
			guiLabelSetColor(GUIEditor_Label[1], 0, 255,0)
		elseif exports.titan_integration:isPlayerScripter(getLocalPlayer()) then
			guiSetText(GUIEditor_Label[1], "MTA Vehicle Model (OK!):")
			guiLabelSetColor(GUIEditor_Label[1], 0, 255,0)			
		else
