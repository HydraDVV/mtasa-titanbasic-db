...
function createElevator(thePlayer, commandName, oneway)
	if (exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerScripter(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer)) then
		...
		if id then
			...
			if (query) then
				...
				exports.titan_global:giveItem(thePlayer, 73, id)
...
function createElevatorWithFriend(thePlayer, commandName, targetPlayer)
	if (exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer)) then
		if not (targetPlayer) then
			...
			local targetPlayer, tragetPlayerName = exports.titan_global:findPlayerByPartialNick(thePlayer, targetPlayer)
			local x, y, z = getElementPosition(thePlayer)
			local interiorwithin = getElementInterior(thePlayer)
			local dimensionwithin = getElementDimension(thePlayer)
	...
	thread:query("SELECT rot, tprot, id, x, y, z, tpx, tpy, tpz, dimensionwithin, interiorwithin, dimension, interior, car, disabled, oneway FROM `elevators` WHERE id = " .. elevatorID,
		function(res, rows, err)
			for index, row in ipairs(res) do
				...
				exports.titan_pool:allocateElement(elevatorElement, tonumber(row.id), true)
			end
		end
	)
	return true
...
function enterElevator(goingin)
	...
	if ( ( vehicle and elevatorStatus[INTERIOR_TYPE]  ~= 0 and getVehicleOccupant( vehicle ) == player ) or not vehicle ) then
		...
		local dbid, entrance, exit, interiorType, interiorElement  = call( getResourceFromName( "titan_interiors" ), "findProperty", player, otherCP[INTERIOR_DIM] )
		if dbid > 0 then

		else
			dbid, entrance, exit, interiorType, interiorElement  = call( getResourceFromName( "titan_interiors" ), "findProperty", player, currentCP[INTERIOR_DIM] )
		end
		...
		if vehicle then
			...
			if interiorElement and isElement(interiorElement) and getElementType(interiorElement) == "interior" then
				exports.titan_anticheat:changeProtectedElementDataEx(interiorElement, "lastused", exports.titan_datetime:now(), true)
				dbExec(mysql:getConnection(),"UPDATE interiors SET lastused=NOW() WHERE id="..dbid)
				exports.titan_logs:dbLog("SYSTEM", 31, { interiorElement, player, vehicle } , "ENTERED/EXITED")
				exports["titan_interiormanager"]:addInteriorLogs(dbid, "ENTERED/EXITED", player)
			end
		elseif isElement(player) then
			if movingInSameInt then
				...
				exports["titan_interiors"]:setPlayerInsideInterior(interiorElement, player, otherCP,movingInSameInt)
			end
			return true
		else
...
function warpVehicleIntoInteriorfunction(vehicle, interior, dimension, offset, x,y,z,pickup,other, interiorElement,movingInSameInt)
	if isElement(vehicle) then
		...
		exports.titan_anticheat:changeProtectedElementDataEx(vehicle, "health", getElementHealth(vehicle), false)
		...
		setTimer(function ()
			...
			exports.titan_anticheat:changeProtectedElementDataEx(vehicle, "health")
...
function deleteElevator(thePlayer, commandName, id)
	if (exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_integration:isPlayerSupporter(thePlayer) or exports.titan_integration:isPlayerScripter(thePlayer)) or commandName == "PROPERTYCLEANUP" then
...
function delElevatorsFromInterior(thePlayer, commandName, intID)
	if (exports.titan_integration:isPlayerTrialAdmin(thePlayer)) or commandName == "PROPERTYCLEANUP" then
		if (not tonumber(intID) or tonumber(intID)%1~= 0 ) and commandName ~= "PROPERTYCLEANUP" then
			...
			if tonumber(intID) == 0 and not exports.titan_integration:isPlayerSeniorAdmin(thePlayer) then
...
addEventHandler( "toggleCarTeleportMode", getRootElement(),
	function( player )
		...
		if query then
			...
			exports.titan_anticheat:changeProtectedElementDataEx( source, "status", elevatorStatus, false )
			if mode == 0 then
