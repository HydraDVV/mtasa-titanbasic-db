			...
			function( button )
				...
				if row ~= -1 and col ~= -1 then
					...
					elseif button == "right" then
						triggerServerEvent("faction:admin:showf3", getLocalPlayer(), gridID, exports.titan_integration:isPlayerAdmin(getLocalPlayer()) )
					end
				else
...
function createAdminDuty(factionT, dutyT)
    ...
    exports.titan_global:guiComboBoxAdjustHeight(DutyAllow.combobox[1], #factionT)
    ...
    exports.titan_global:guiComboBoxAdjustHeight(DutyAllow.combobox[2], 2)
	...
	if selection == 0 then -- Items
		for k,v in pairs(factionTable[key][3]) do
			if tonumber(v[2]) > 0 then
				...
				guiGridListSetItemText(DutyAllow.gridlist[1], row, 2, exports["titan_items"]:getItemName(v[2]), false, false)
				guiGridListSetItemText(DutyAllow.gridlist[1], row, 3, exports["titan_items"]:getItemDescription(v[2], v[3]), false, false)
...
function allowItem()
	...
	if not exports['titan_items']:isItem(itemID) and selection == 0 then
