function checkLSCSD(thePlayer, seat, jacked)
	...
	if (thePlayer == getLocalPlayer()) and (seat == 0) and (vehicleFaction == 23) and not (playerFaction == 23) then
		cancelEvent()
		outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #f0f0f0Bu aracı yalnızca Los Santos County Sheriff Department üyeleri kullanabilir.", 255, 0, 0, true)
	end
end
...
function checkLSPD(thePlayer, seat, jacked)
	...
	if (thePlayer == getLocalPlayer()) and (seat == 0) and (vehicleFaction == 1) and not (playerFaction == 1) then
		cancelEvent()
		outputChatBox("#822828"..exports["titan_pool"]:getServerName()..": #f0f0f0Bu aracı yalnızca Los Santos Police Department üyeleri kullanabilir.", 255, 0, 0, true)
	end
end
