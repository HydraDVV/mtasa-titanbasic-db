--CUSTOM SHOP / Hanz
version = exports["titan_global"]:getScriptVersion()
...
BizNoteFont = guiCreateFont( ":titan_resources/fonts/BizNote.ttf", 30 )
BizNoteFont18 = guiCreateFont( ":titan_resources/fonts/BizNote.ttf", 18 )
...
function showGeneralshopUI(shop_type, race, gender, discount, products1)
	...
	if not wCustomShop and not wAddingItemsToShop and not wGeneralshop and not getElementData(getLocalPlayer(), "shop:NoAccess" ) then
		...
		if shop_type==17 then
			...
			local shopTabPanel = guiCreateTabPanel(9,26,700,350,false,wCustomShop)
			...
			for _, record in ipairs(products) do
				...
				local itemName = exports["titan_items"]:getItemName( tonumber(record[2]), tostring(record[3]) ) 
				local itemValue = ""
				if not exports["titan_items"]:getItemHideItemValue(tonumber(record[2])) then
					itemValue = exports["titan_items"]:getItemValue( tonumber(record[2]), tostring(record[3]) )
				end
				local itemPrice = exports.titan_global:formatMoney(math.ceil(tonumber(record[5] or 0))).. " $" or false
			...
			if isClientBizOwner then
				...
				if sContactInfo then
					...
					sFormatedPhone = ""
					if sPhone ~= "" then
						sFormatedPhone = "(+555) "..exports.titan_global:formatMoney(sPhone)
					end
				...
				exports["titan_dxlib"]:convertUI(tGoodBye)
				...
				guiSetText(lWelcomeText,'"Hoşgeldin patron! Naber?" || '..currentCap..'/'..sCapacity..' Ürünler , Toplam Gelir: TL'..exports.titan_global:formatMoney(sIncome)..'.')
				...
				GUIEditor_Label[3] = guiCreateLabel(11,95,716,20,"- Gelir: $"..exports.titan_global:formatMoney(sIncome),false,tBizManagement)
				GUIEditor_Label[4] = guiCreateLabel(11,115,716,20,"- Personel Ödemesi: $"..exports.titan_global:formatMoney(sPendingWage).." ($"..exports.titan_global:formatMoney(sCapacity/wageRate).."/hour)",false,tBizManagement)
				GUIEditor_Label[5] = guiCreateLabel(11,135,716,20,"- kâr: $"..exports.titan_global:formatMoney(sProfit),false,tBizManagement)
				...
				if sProfit < 0 then
					...
					if sProfit < (0 - warningDebtAmount) then 
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.titan_global:formatMoney(sProfit).." (Uyarı: Eğer borcunuz varsa $"..exports.titan_global:formatMoney(limitDebtAmount)..", Çalışanlarınız işten ayrılacaktır)." )
						guiLabelSetColor ( GUIEditor_Label[5], 255, 0, 0)
				...
				elseif sProfit == 0 then
					...
					if sProfit < 500 then
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.titan_global:formatMoney(sProfit).." (Ortalama).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 255, 0)
					elseif sProfit >= 500 and sProfit < 1000 then
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.titan_global:formatMoney(sProfit).." (İyi!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 245, 0)
					elseif sProfit >= 1000 and sProfit < 2000 then
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.titan_global:formatMoney(sProfit).." (Çok iyi!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 235, 0)
					elseif sProfit >= 2000 and sProfit < 4000 then
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.titan_global:formatMoney(sProfit).." (Mükemmel!!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 225, 0)
					elseif sProfit >= 4000 and sProfit < 8000 then
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.titan_global:formatMoney(sProfit).." (Beklenmedik!!!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 215, 0)
					elseif sProfit >= 8000 and sProfit < 20000 then
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.titan_global:formatMoney(sProfit).." (İnanılmaz!!!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 205, 0)
					elseif sProfit >= 20000 then
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.titan_global:formatMoney(sProfit).." (İşin kralıyız!!!!).")
				...
				addEventHandler( "onClientGUIClick", bExpand, function ()
					...
					guiSetText(GUIEditor_Label[4] , "- Personel Ödemesi: $"..exports.titan_global:formatMoney(sPendingWage).." ($"..exports.titan_global:formatMoney(getElementData(ped, "sCapacity")/wageRate).."/Saatlik)")
			...
			--setSoundVolume(playSound(":titan_resources/sounds/inv_open.mp3"), 0.3)
		...
		elseif shop_type==19 then --Faction Drop NPC - Weapons
			...
			for _, record in ipairs(products) do
				...
				local itemName = exports["titan_items"]:getItemName( tonumber(record["pItemID"]), tostring(record["pItemValue"]) ) 
				local itemValue = exports["titan_items"]:getItemValue( tonumber(record["pItemID"]), tostring(record["pItemValue"]) ) 
				local description = exports["titan_items"]:getItemDescription( tonumber(record["pItemID"]), itemValue ) 
				local itemPrice = exports.titan_global:formatMoney(math.floor(tonumber(record["pPrice"] or 0))).. " $"
				...
				guiGridListSetItemText(fdgw.gProducts, row, fdgw.colQuantity, exports.titan_global:formatMoney(record["pQuantity"]), false, false)
				...
				if products then 
					...
					if (row==-1) or (col==-1) then
						...
						if tonumber(quan) <= 0 then
							exports.titan_global:playSoundError()
							return false
						end
			...
			if canPlayerAdminShop(localPlayer) then
				...
				exports.titan_global:guiComboBoxAdjustHeight(cFaction, counter)
				...
				exports.titan_global:guiComboBoxAdjustHeight(cMember, 3)
			...
			if shop_type == 7 then
				if not exports.titan_global:hasItem(localPlayer, 183) then -- Viozy Membership Card
			...
			tabpanel = guiCreateTabPanel(9,26,610,570,false,wGeneralshop)
			...
			for _, category in ipairs( shop ) do
				...
				for _, item in ipairs( category ) do
					...
					if item.minimum_age and getElementData(localPlayer, "age") < item.minimum_age then
						...
						guiGridListSetItemText( grid, row, cPrice, tostring(exports.titan_global:formatMoney(math.ceil(discount * item.price))).." $", false, false )
					end
				...
				if hasSkins then -- event handler for skin preview
					addEventHandler( "onClientGUIClick", grid, function( button, state )
						if button == "left" then
							...
							if item.itemID == 16 then
								iClothesPreview = guiCreateStaticImage( 620, 23, 100, 100, ":titan_account/img/" .. ("%03d"):format( item.itemValue or 1 ) .. ".png", false, source)
							end
						end
					end, false )
				end
			...
			if isClientBizOwner then
				...
				if shop_type ~= 14 then -- Lazy fix for non-profitable carpart shop,  maxime
					...
					GUIEditor_Label[3] = guiCreateLabel(11,95,716,20,"- Gelir: $"..exports.titan_global:formatMoney(sIncome),false,tBizManagement)
					GUIEditor_Label[4] = guiCreateLabel(11,115,716,20,"- Personel ödemesi: $"..exports.titan_global:formatMoney(sPendingWage).." (Already bound to interior taxes)",false,tBizManagement)
					GUIEditor_Label[5] = guiCreateLabel(11,135,716,20,"- kâr: $"..exports.titan_global:formatMoney(sProfit),false,tBizManagement)
					GUIEditor_Label[6] = guiCreateLabel(11,155,57,19,"- Satışlar: ",false,tBizManagement)
					GUIEditor_Memo[1] = guiCreateMemo(11,179,498,184,sSales,false,tBizManagement)
					guiMemoSetReadOnly(GUIEditor_Memo[1], true)
					...
					elseif sProfit == 0 then
						...
						elseif sProfit >= 4000 and sProfit < 8000 then
							...
							guiLabelSetColor ( GUIEditor_Label[5], 0, 215, 0)
						elseif sProfit >= 8000 and sProfit < 20000 then
							guiSetText(GUIEditor_Label[5] , "- Profit: $"..exports.titan_global:formatMoney(sProfit).." (İnanılmaz!!!).")
							guiLabelSetColor ( GUIEditor_Label[5], 0, 205, 0)
						elseif sProfit >= 20000 then
							guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.titan_global:formatMoney(sProfit).." (İşin kralıyız!!!!).")
							guiLabelSetColor ( GUIEditor_Label[5], 0, 195, 0)
...
function hideGeneralshopUI()
	...
	if wGeneralshop then
		...
		wGeneralshop = nil
		--setSoundVolume(playSound(":titan_resources/sounds/inv_close.mp3"), 0.3)
	end
	if wCustomShop then
...
function factionDropUpdateWeaponList(newItems)
	...
	if fdgw.gProducts and isElement(fdgw.gProducts) then
		...
		for _, record in ipairs(products) do
			...
			local itemName = exports["titan_items"]:getItemName( tonumber(record["pItemID"]), tostring(record["pItemValue"]) ) 
			local itemValue = ""
			if not exports["titan_items"]:getItemHideItemValue(tonumber(record["pItemID"])) then
				itemValue = exports["titan_items"]:getItemValue( tonumber(record["pItemID"]), tostring(record["pItemValue"]) )
			end
			local description = exports["titan_items"]:getItemDescription( tonumber(record["pItemID"]), itemValue ) 
			local itemPrice = exports.titan_global:formatMoney(math.floor(tonumber(record["pPrice"] or 0))).. " $"
			guiGridListSetItemText(fdgw.gProducts, row, fdgw.colName, itemName or "Unknown", false, false)
