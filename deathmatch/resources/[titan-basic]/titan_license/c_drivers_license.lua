...
function createLicenseQuestionWindow(number)
	...
	if(number < NoQuestionToAnswer) then
		...
		exports["titan_dxlib"]:convertUI(guiQuestionWindow)
...
function createTestFinishWindow()
	...
	exports["titan_dxlib"]:convertUI(guiFinishWindow)
end
...
function initiateDrivingTest()
	...
	outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Artık araç sürüş testine girmeye hazırsınız, dışarıdaki sürücü araçlarından birisini alın.", 255, 255, 0, true)
	else
				outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Zaten bir ehliyetin var.", 255, 0, 0, true)
	end
end
...
function startDrivingTest(element)
	if element == getLocalPlayer() then
		...
		if not (testVehicle[id]) then
			outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Sadece ehliyet araçları ile bu bölgeden geçebilirsiniz.", 255, 0, 0, true ) -- Wrong car type.
		else
			destroyElement(blip)
			...
			outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Test aracına zarar vermeden sürüşü tamamlamanız gerekiyor, bol şans!", 255, 194, 14, true)
		end
	end
end
	...
	if (element == localPlayer) then
		...
		if not (testVehicle[id]) then
			...
			if (tonumber(max_number-1) == tonumber(m_number)) then -- if the next checkpoint is the final checkpoint.
				outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Testi tamamlamak için son varış noktasına park edin.", 255, 15, 255, true)
	...
	if (element == localPlayer) then
		...
		if not (testVehicle[id]) then
			...
			if (vehicleHealth >= 800) then
				...
				-- PASS --
				----------
				outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Aracı inceledik ve bir hasar göremedik.", 255, 194, 14, true)
				triggerServerEvent("acceptCarLicense", getLocalPlayer())
			else
				----------
				-- Fail --
				----------
				outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Araçta ufak tefek çizikler var, iyi bir şoför olman için iyi bir sürücü olman gerekiyor. Yeniden başvurabilirsin.", 255, 0, 0, true)
			end
