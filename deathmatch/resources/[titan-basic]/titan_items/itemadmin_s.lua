...
function getNearbyItems(thePlayer, commandName)
	if exports.titan_integration:isPlayerTrialAdmin(thePlayer) or exports.titan_global:isStaffOnDuty(thePlayer) or exports.titan_global:isPlayerScripter(thePlayer) then
		...
		for k, theObject in ipairs(getElementsByType("object", getResourceRootElement(getResourceFromName("titan_item-world")))) do
			...
			if dbid then
				...
				if distance <= 10 and getElementDimension(theObject) == getElementDimension(thePlayer) and getElementInterior(theObject) == getElementInterior(thePlayer) and getElementData(theObject, "itemID") ~= 169 then
					outputChatBox("   #" .. dbid .. (getElementData(theObject, "protected") and ("(" .. getElementData(theObject, "protected").. ")") or "") .. " by " .. ( exports['titan_cache']:getCharacterName( getElementData(theObject, "creator"), true ) or "?" ) .. " - " .. ( getItemName( getElementData(theObject, "itemID") ) or "?" ) .. "(" .. getElementData(theObject, "itemID") .. "): " .. tostring( getElementData(theObject, "itemValue") or 1 ), thePlayer, 255, 126, 0)
					count = count + 1
				end
			end
		end
...
function delItem(thePlayer, commandName, targetID)
	if (exports.titan_integration:isPlayerTrialAdmin(thePlayer)) then
		if not (targetID) then
			...
			for key, value in ipairs(getElementsByType("object", getResourceRootElement(getResourceFromName("titan_item-world")))) do
