...
function add( vehicle )
	if getElementModel( vehicle ) == 519 then -- Shamal
		...
		--add cockpit door		
		local gate = exports["titan_gates"]:createGate(2924,1,34.5,1199.8000488281,0,0,180,-0.30000001192093,34.5,1199.8000488281,0,0,180,1,dim,0,30,9,tostring(dbid),2)
...
function enterCockpitByKey(thePlayer, key, keyState)
	...
	if vehicle then
		...
		if(model == 577 or model == 519) then --AT-400 & Shamal
			if(getElementData(thePlayer, "duty_admin") == 1 or exports.titan_global:hasItem(thePlayer, 3, tonumber(dbid)) or (getElementData(thePlayer, "faction") > 0 and getElementData(thePlayer, "faction") == getElementData(vehicle, "faction"))) then
				allowed = true
			end
		end
		if allowed then
...
function teleportTo( player, x, y, z, dimension, interior, freeze )
	...
	setTimer(
		function( player )
			...
			if freeze then 
				triggerClientEvent( player, "usedElevator", player ) -- DISABLED because the event was buggged for an unknown reason on client side / Hanz
	...
	function( vehicle )
		...
		if vehicles[ vehicle ] then
			if isVehicleLocked( vehicle ) then
				outputChatBox( "Bu aracın kapısı kilitli.", source, 255, 0, 0 )
			else
				local model = getElementModel(vehicle)
		...
		if getElementData( value, "dbid" ) == dim then
			if isVehicleLocked( value ) then
				...
				if(getElementData(value, "airport.gate.connected")) then
					...
					local teleportArr = exports["titan_sfia"]:getDataForExitingConnectedPlane(gateID, value)
					if teleportArr then
		...
		if (tpd > 20000) then
			...
			for key, value in ipairs( getElementsByType( "vehicle" ) ) do
				if getElementData( value, "dbid" ) == vid then
					exports.titan_global:sendLocalText(player, " *" .. getPlayerName(player):gsub("_"," ") .. " begins to knock on the vehicle.", 255, 51, 102)
					exports.titan_global:sendLocalText(value, " * Knocks can be heard coming from inside the vehicle. *      ((" .. getPlayerName(player):gsub("_"," ") .. "))", 255, 51, 102)
				end
			end
		else
			if vehicles[veh] then
				...
				if (exit) then
					exports.titan_global:sendLocalText(player, " *" .. getPlayerName(player):gsub("_"," ") .. " begins to knock on the vehicle.", 255, 51, 102)
					exports.titan_global:sendLocalText(exit, " * Knocks can be heard coming from the outside. *      ((" .. getPlayerName(player):gsub("_"," ") .. "))", 255, 51, 102)
				end
			end
		end
	end
...
function seeThroughWindows(thePlayer, nx, ny, nz) -- This is for /windows
	...
	if (getElementData(thePlayer, "isInWindow") == false or not getElementData(thePlayer, "isInWindow")) and dim > 20000 then
		...
		local id = dim - 20000
		local vehicle = exports.titan_pool:getElement("vehicle", tonumber(id))
		local x, y, z = getElementPosition(vehicle)
