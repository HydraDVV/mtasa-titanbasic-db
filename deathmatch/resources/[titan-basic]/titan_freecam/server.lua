...
function asyncActivateFreecam ()
	if not isEnabled(source) then
		...
		if not exports.titan_integration:isPlayerTrialAdmin(source) then
			exports.titan_global:sendMessageToAdmins("[FREECAM] "..exports.titan_global:getAdminTitle1(source).." has activated temporary /freecam.")
		end
		setElementData(source, "freecam:state", true, false)
		exports.titan_logs:dbLog(source, 4, {source}, "FREECAM")
	end
end
...
function tv(player)
	local hasTV = exports.titan_global:hasItem(player, 104, 1)
	...
	function(player)
		if getElementData(player, "faction") == 20 then
			if not isTVRunning() then
				...
				exports.titan_logs:dbLog(player, 23, player, "TV START")
				watching = 0
				earnings = 0
		...
		if getElementData(player, "faction") == 20 then
			if isTVRunning() then
				...
				for k, v in ipairs( getElementsByType( "player" ) ) do
					...
					if getElementData(v, "faction") == 20 then
						outputChatBox("[TRT] Max. İzleyiciler: " .. watching .. ", Bağışlar: $" .. exports.titan_global:formatMoney(earnings), v, 200, 100, 200)
					end
				end
				
				exports.titan_logs:dbLog(player, 23, player, "TRT STOP - VIEWERS: " .. watching .. " EARNINGS: " .. earnings)
			else
	...
	function(player)
		if getElementData(player, "faction") == 20 then
			if isTVRunning() then
				...
				outputChatBox("[TRT] Max. İzleyiciler: " .. watching .. ", Kazanç: $" .. exports.titan_global:formatMoney(earnings), player, 200, 100, 200)
			else
...
function add( affectedPlayers )
	...
	if isTVRunning() and shownto > 0 then
		...
		exports.titan_global:giveMoney(getTeamFromName"TRT Haber Bülteni", LSN_MONEY * shownto)
		--exports.titan_logs:dbLog(player, 23, player, "TV $" .. ( 10 * shownto ) .. " " .. message)
	else
		--exports.titan_logs:dbLog(player, 23, player, "TV OFF " .. message)
	end
end