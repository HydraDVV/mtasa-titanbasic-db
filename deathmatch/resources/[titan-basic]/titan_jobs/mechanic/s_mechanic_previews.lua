function headlightPreview( veh, color1, color2, color3 )
	if veh then
		if not getElementData( veh, "oldheadcolors" ) then
			exports.titan_anticheat:changeProtectedElementDataEx( veh, "oldheadcolors", { getVehicleHeadLightColor( veh ) }, false ) 
		end
		...
		if setVehicleHeadLightColor ( veh, color1, color2, color3) then
			...
			exports.titan_anticheat:changeProtectedElementDataEx( veh, "job:hpreviewtimer", newTimer, false )
		end
	end
end
...
function headlightEndPreview( veh )
	if veh then
		...
		if colors then
			...
			exports.titan_anticheat:changeProtectedElementDataEx( veh, "oldheadcolors" )
		end
	end
end
...
function previewColors( veh, color1, color2, color3, color4 )
	if veh then
		if not getElementData( veh, "oldcolors" ) then
			exports.titan_anticheat:changeProtectedElementDataEx( veh, "oldcolors", { getVehicleColor( veh, true ) }, false )
		end
		...
		if setVehicleColor( veh, color1[1], color1[2], color1[3], color2[1], color2[2], color2[3],  color3[1], color3[2], color3[3], color4[1], color4[2], color4[3]) then
			...
			exports.titan_anticheat:changeProtectedElementDataEx( veh, "job:previewtimer", newTimer, false )
		end
	end
end
...
function endColorPreview( veh )
	if veh then
		...
		if colors then
			...
			exports.titan_anticheat:changeProtectedElementDataEx( veh, "oldcolors" )
		end
	end
end
...
function previewPaintjob( veh, paintjob )
	if veh then
		if not getElementData( veh, "oldpaintjob" ) then
			exports.titan_anticheat:changeProtectedElementDataEx( veh, "oldpaintjob", getVehiclePaintjob( veh ), false )
		end
		if setVehiclePaintjob( veh, paintjob ) then
			local col1, col2 = getVehicleColor( veh )
			if col1 == 0 or col2 == 0 then
				if not getElementData( veh, "oldcolors" ) then
					exports.titan_anticheat:changeProtectedElementDataEx( veh, "oldcolors", { getVehicleColor( veh ) }, false )
				end
			...
			local newTimer = setTimer(endColorPreview, 45000, 1, veh)
			exports.titan_anticheat:changeProtectedElementDataEx( veh, "job:ppreviewtimer", newTimer, false )
		end
	end
end
...
function endPaintjobPreview( veh )
	if veh then
		...
		if paintjob then
			...
			exports.titan_anticheat:changeProtectedElementDataEx( veh, "oldpaintjob" )
		end
		local colors = getElementData( veh, "oldcolors" )
		if colors then
			setVehicleColor( veh, unpack( colors ) )
