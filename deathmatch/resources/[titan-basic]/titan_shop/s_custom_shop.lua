...
function addItemToCustomShop(element, slot, event, worldItem)
	...
	if slot ~= -1 then
		item = exports['titan_items']:getItems( source )[ slot ]
		itemID = item[1]
		itemValue = item[2]
	...
	if exports.titan_global:hasItem(source, 4, dbid) or exports.titan_global:hasItem(source, 5, dbid) then
		if element then
...
function updateItemToShop(source, item, price, desc, npcID, slot, worldItem, shopElement)
	...
	if item[1] == 3 or item[1] == 4 or item[1] == 5 then --Keys , to prevent alt->alt
		exports.titan_global:sendLocalText(source, "* "..pedName.." laughs at "..playerName..".", 255, 51, 102, 30, {}, true)
	...
	local itemName = exports["titan_items"]:getItemName(item[1], item[2])
	if tonumber(price) < 0 then
		exports.titan_global:sendLocalText(source, "* "..pedName.." doesn't agree with "..playerName.." on the price of a "..itemName..".", 255, 51, 102, 30, {}, true)
	...
	if addToShop then
		if slot == -1 and worldItem and isElement(worldItem) then 
			...
			exports['titan_items']:takeItemFromSlot( source, slot )
		end
		
		exports.titan_global:sendLocalMeAction(source, "hands "..pedName.." a "..itemName..".")
        ...
        function(qh, source)
            ...
            if rows > 0 then
                ...
                if checkingItem then
                    ...
                    if addToShop then
                        exports.titan_global:sendLocalMeAction(source, "discusses with "..pedName.." about a "..itemName..".")
    ...
    dbQuery(
        function(qh, source)
            ...
            if rows > 0 then
                ...
                checkingItem = check["pID"]
                if checkingItem then 
                    local success, reason = exports["titan_items"]:giveItem( source, itemI, itemV, false, false )
                    if success then
                        exports.titan_global:sendLocalText(source, "* "..playerName.." takes a "..itemName.." from "..pedName..".", 255, 51, 102, 30, {}, true)
                        local playerGender = getElementData(source,"gender")
                        if playerGender == 0 then
        ...
        function(qh, source)
            ...
            if rows > 0 then
                ...
                if checkingItem and checkingPrice and (tonumber(checkingPrice) == tonumber(itemPrice) ) then
                    local success, reason = exports["titan_items"]:giveItem( thePlayer, itemI, itemV, false, false )
                    if success then
                        ...
                        if not payByBank then
                            ...
                            if exports.titan_global:takeMoney(thePlayer, itemPrice) then
                                --
                            else
                                storeKeeperSay(thePlayer, "Well, I'm sorry but this seems not enough..", pedName)
                                exports.titan_global:takeItem( thePlayer, itemI, itemV ) 
                                setElementData(thePlayer, "shop:NoAccess", false, true )
                                triggerClientEvent(thePlayer, "hideGeneralshopUI", thePlayer)
                                return false
                            end
                            ownerNoti = "A customer bought a "..itemName.." for $"..exports.titan_global:formatMoney(itemPrice).."."
                            logString = "- "..timeString.." : A customer bought a "..itemName.." for $"..exports.titan_global:formatMoney(itemPrice)..".\n"
                        else
                            playBuySound(shopElement)
                            if playerGender == 0 then
                            ...
                            ownerNoti = getPlayerName(thePlayer):gsub("_", " ").."(Credit Card) bought a "..itemName.." for $"..exports.titan_global:formatMoney(itemPrice).."."
                            
                            logString = "- "..timeString.." : "..getPlayerName(thePlayer):gsub("_", " ").."(Credit Card) bought a "..itemName.." for $"..exports.titan_global:formatMoney(itemPrice)..".\n"
                        end
                        ...
                        exports.titan_global:sendLocalText(thePlayer, "* "..pedName.." gave "..playerName.." a "..itemName..additionalText..".", 255, 51, 102, 30, {}, true)
...
function solvePendingWage(thePlayer, shopElement)
	...
	if sPendingWage > 0 then
		exports.titan_global:sendLocalText(thePlayer, "* "..playerName.." gives "..pedName.." a couple of dollar notes.", 255, 51, 102, 30, {}, true)
		if exports.titan_global:takeMoney(thePlayer, sPendingWage) then
			...
			exports.titan_global:sendLocalText(thePlayer, "* "..pedName.." accepts the money from "..playerName..".", 255, 51, 102, 30, {}, true)
...
function storeKeeperSay(thePlayer, content, pedName)
	...
	exports.titan_global:sendLocalText(thePlayer, "["..languagename.."] "..tostring(pedName).." says: "..content, 255, 255, 255, 30, {}, true)
end
...
function notifyAllShopOwners(shopElement, content)
	local maxDebt = exports.titan_global:formatMoney(limitDebtAmount)
	local warningDebt = exports.titan_global:formatMoney(warningDebtAmount)
	local contentList = {
	...
	local pedName = getPedName(shopElement)
	setTimer(function()
		--exports.titan_global:sendLocalText(shopElement, "*"..pedName.." takes out a cellphone and starts sending text messages.", 255, 51, 102, 30, {}, true)
		...
		for _, owner in ipairs(possibleOwners) do
			...
			if isBizOwner then 
				if exports.titan_global:hasItem(owner, 2) then 
					...
					exports.titan_global:sendLocalText(owner, "*"..ownerName.." receives a text message.", 255, 51, 102, 30, {}, true)
...
function resetPendingWage(thePlayer)
	if (exports.titan_integration:isPlayerTrialAdmin(thePlayer)) then
		...
		if hiddenAdmin == 0 then
			exports.titan_global:sendMessageToAdmins("[BIZ-SYSTEM]: "..adminTitle.." ".. getPlayerName(thePlayer):gsub("_", " ").. " ("..adminUsername..") has reset "..count.." custom shop wages to $0.")
		else
			exports.titan_global:sendMessageToAdmins("[BIZ-SYSTEM]: A hidden admin has reset "..count.." custom shop wages to $0.")
		end
	end
end
...
function shopLeaveNoteOnLeave(shopElement, content)
	...
	local protected = 0
	local modelid = exports['titan_items']:getItemModel(itemID, itemValue)
	local rx, ry, rz, zoffset = exports['titan_items']:getItemRotInfo(itemID)
	local id = SmallestID()
	...
	if insert then
		local obj = exports["titan_item-world"]:createItem(id, itemID, itemValue, modelid, x, y, z + ( zoffset or 0 ), rx, ry, rz+rz2)
...
function updateShopSalary(thePlayer)
			...
			if shopID then
				...
				if (sProfit) >= (0-limitDebtAmount) then
					...
					if delete and delete2 then
						...
						destroyElement(shop)
						exports.titan_global:sendMessageToAdmins("[BIZ-SYSTEM] Shop ID#"..shopID.." Deleted itself due to the debt exceeded $"..exports.titan_global:formatMoney(limitDebtAmount)..".")
					else
						if thePlayer then
						end
						count = count - 1
	...
	if thePlayer then
		...
		if hiddenAdmin == 0 then
			exports.titan_global:sendMessageToAdmins("[BIZ-SYSTEM]: "..adminTitle.." ".. getPlayerName(thePlayer):gsub("_", " ").. " ("..adminUsername..") has forced "..count.." custom shops to take wage.")
		else
			exports.titan_global:sendMessageToAdmins("[BIZ-SYSTEM]: A hidden admin has forced "..count.." custom shops to take wage.")
		end
	end
end
