...
function reloadPed(id)
    ...
    local thePed = exports.titan_pool:getElement("ped", tonumber(id))
        ...
        function(qh)
            ...
            if rows > 0 then
                for index, row in ipairs(res) do
                    ...
                    if ped then
                        exports.titan_anticheat:changeProtectedElementDataEx(ped, "dbid", row.id)
                        exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.dbid", row.id)
                        exports.titan_pool:allocateElement(ped, row.id, true)
                        ...
                        --exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.spawnpos", row.x..","..row.y..","..row.z..","..row.rotation..","..row.interior..","..row.dimension)
                        ...
                        if row.type then
                            exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.type", row.type)
                        end
                        exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.name", pedname)
                        exports.titan_anticheat:changeProtectedElementDataEx(ped, "ped:name", pedname) -- For chat system
                        exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.gender", gender)
                        exports.titan_anticheat:changeProtectedElementDataEx(ped, "name", pedname) --for owl
                        if row.nametag then
                            exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.nametag", true)
                            exports.titan_anticheat:changeProtectedElementDataEx(ped, "nametag", true) --for owl
                        end
                        exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.behav", row.behaviour)
                        if row.money then --if ped have money, set the amount
                            exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.money", row.money)
                            ...
                            if(row.owner_type ~= 0 and row.owner ~= 0) then
                                exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.owner_type", row.owner_type)
                                exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.owner", row.owner)
                            end
                        end
                        ...
                        if row.animation then
                            exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.animation", row.animation)
                            --outputDebugString(tostring(row.animation))
                            local animTable = exports.titan_global:split(row.animation, ";")
                        ...
                        local badges = {}
                        badges = exports['titan_items']:getBadges()
                        --outputDebugString(tostring(#badges).." badges")
                        for k,v in pairs(badges) do
                            local hasItem = exports['titan_items']:hasItem(ped, k)
                            if hasItem then
                                local itemresult = exports['titan_items']:npcUseItem(ped, k)
                        ...
                        exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.createdBy", row.created_by)
                        exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.createdByUsername", exports.titan_global:getUserNameFromID(row.created_by))
                        exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.createdAt", row.created_at)
                        exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.comment", row.comment)
                        exports.titan_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.synced", tonumber(row.synced) == 1 or false)
...
function makeTempPed(thePlayer, command, behav, gender, skin, name) --create temporary ped (no dbid)
    if exports.titan_integration:isPlayerTrialAdmin(thePlayer) then
        ...
        if name then
            local splitname = exports.titan_global:split(name, "_")
            if(#splitname >= 2) then
...
function makeFuelPed(thePlayer, command) --Create temporary fuel ped (no dbid)
    if global:isPlayerAdmin(thePlayer) then
        ...
        exports.titan_pool:allocateElement(theNewPed)
        setPedRotation (theNewPed, rz)
        setElementFrozen(theNewPed, true)
        --setPedAnimation(theNewPed, "FOOD", "FF_Sit_Loop",  -1, true, false, true)
        exports.titan_anticheat:changeProtectedElementDataEx(theNewPed, "rpp.npc.type", "fuel", true)
        exports.titan_anticheat:changeProtectedElementDataEx(theNewPed, "rpp.npc.name", getRandomName("full", "male"), true)
        exports.titan_anticheat:changeProtectedElementDataEx(theNewPed, "rpp.npc.nametag", "true", true)
        exports.titan_anticheat:changeProtectedElementDataEx(theNewPed, "rpp.npc.behav", "1", true)

        exports.titan_anticheat:changeProtectedElementDataEx(theNewPed, "fuel:priceratio" , 100, false)

        -- For the language system
        exports.titan_anticheat:changeProtectedElementDataEx(theNewPed, "languages.lang1" , 1, false)
        exports.titan_anticheat:changeProtectedElementDataEx(theNewPed, "languages.lang1skill", 100, false)
        exports.titan_anticheat:changeProtectedElementDataEx(theNewPed, "languages.lang2" , 2, false)
        exports.titan_anticheat:changeProtectedElementDataEx(theNewPed, "languages.lang2skill", 100, false)
        exports.titan_anticheat:changeProtectedElementDataEx(theNewPed, "languages.current", 1, false)
    end
end
...
function pedMoveToTram(ped, tram)
    ...
    --else
        ...
        pedMoveTimer[ped] = nil
        setPedAnimation(ped)
        exports.titan_trams:npcEnterTramPassenger(ped, tram)
    --end
end
...
function respawnPed(player, ped)
    if(getElementType(ped) == "ped") then
        if getElementData(ped, "dbid") then
            ...
            --if(ped == exports.titan_pool:getElement("ped", tonumber(dbid))) then --extra security check
            ...
            --else
            --  outputDebugString("Failed to respawn ped with dbid "..tostring(dbid).." (#1) ("..tostring(ped).." = "..tostring(exports.titan_pool:getElement("ped", tonumber(dbid)))..")")
            --  return false
            --end
        ...
        elseif getElementData(ped, "rpp.npc.spawnpos2") then
            ...
            local pos = exports.titan_global:split(posstring, ",")
        ...
        for k, v in next, getAllElementData(ped) do
            exports.titan_anticheat:changeProtectedElementDataEx(new, k, v)
        end
        exports.titan_anticheat:changeProtectedElementDataEx(new, "activeConvo", 0)
...
function healPed(player, ped)
    if(getElementType(ped) == "ped") then
        ...
        if not (factionType==4) then
            ...
            local foundkit, slot, itemValue = exports.titan_global:hasItem(player, 70)
            if foundkit then
                if isPedDead(ped) then
                    ...
                    exports.titan_global:sendLocalText(source, " *" ..  string.gsub(name, "_", " ").. ( message:sub( 1, 1 ) == "'" and "" or " " ) .. message, 255, 51, 102)
                    if itemValue > 1 then
                        exports['titan_items']:updateItemValue(player, slot, itemValue - 1)
                    else
                        exports.titan_global:takeItem(thePlayer, 70, itemValue)
                        if not exports.titan_global:hasItem(thePlayer, 70) then
                    ...
                    local price = 100                                   
                    exports.titan_global:giveMoney( getTeamFromName("San Fierro Emergency Services"), math.ceil((1-tax)*price) )
                    exports.titan_global:giveMoney( getTeamFromName("Government of San Fierro"), math.ceil(tax*price) )
                    if pedName then
                        outputChatBox("You healed " ..pedName.. " (NPC).", player, 0, 255, 0)
                    else
                        outputChatBox("You healed a NPC.", player, 0, 255, 0)
                    end
                    exports.titan_logs:dbLog(player, 35, ped, "HEAL NPC FOR $" .. price)
                end
            else
...
function deletePed(player, ped)
    if(getElementType(ped) == "ped") then
        if exports.titan_integration:isPlayerAdmin(player) then
            local dbid = tonumber(getElementData(ped, "rpp.npc.dbid")) or 0
            if dbid > 0 then
                local pedFunction = getElementData(ped, "rpp.npc.type")
                if pedFunction then
                    --if(allowedFunctionPeds[pedFunction] or exports.titan_global:isPlayerHeadAdmin(player)) then
                ...
                destroyElement(ped)
                exports.titan_global:sendMessageToAdmins("[Peds] Temp NPC '"..tostring(pedName).."' was deleted by " .. getPlayerName(player) .. ".")
            end
        end
    end
end
...
function deleteDbPed(player, pedID, ped)
    local mysql = exports.titan_mysql
    if exports.titan_integration:isPlayerTrialAdmin(player) then
        if not ped then
            ped = exports.titan_global:getPoolElement("ped", pedID)
        end
        if not ped then
        ...
        if pedType then
            if(not allowedFunctionPeds[pedFunction]) then
                if not exports.titan_integration:isPlayerAdmin(player) then
...
function hideMyID(thePlayer, command)
    if(exports.titan_integration:isPlayerScripter(thePlayer)) then
