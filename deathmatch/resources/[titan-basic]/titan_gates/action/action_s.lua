...
function moveGate(theGate, secondtime)
	...
	if not (isGateBusy) or (secondtime) then
		exports.titan_anticheat:changeProtectedElementDataEx(theGate, "gate:busy", true, false)
		...
		exports.titan_anticheat:changeProtectedElementDataEx(theGate, "gate:parameters", gateParameters, false)
	end
end
...
function resetBusyState(theGate)
	local isGateBusy = getElementData(theGate, "gate:busy")
	if (isGateBusy) then
		exports.titan_anticheat:changeProtectedElementDataEx(theGate, "gate:busy", false, false)
	end
...
function canPlayerControlGate(theGate, thePlayer, password)
	...
	elseif gateProtection == 3 then
		...
		for _, itemID in ipairs(tempAccess) do
			if (exports.titan_global:hasItem(thePlayer, tonumber(itemID))) then
				return true
			end
		end
	...
	elseif gateProtection == 4 then
		local tempAccess = split(gateParameters["gateSecurityParameters"], " ")
		local hasItem, slotID, itemValue, databaseID = exports.titan_global:hasItem(thePlayer, tonumber(tempAccess[1]))
		if (hasItem) then
	...
	elseif gateProtection == 7 then --for faction ID
		...
		for _, factionID in ipairs(tempAccess) do
			if exports.titan_factions:isPlayerInFaction(thePlayer, tonumber(factionID)) then
				return true
			end 
		end
	elseif gateProtection == 8 then --Exciter Query string
		local tempAccess = split(gateParameters["gateSecurityParameters"], " AND ")
		local badges = exports["titan_items"]:getBadges()
			...
			if(#orString > 1) then
				...
				for k, v in ipairs(orString) do
					...
					if isNumeric(theItem[1]) then --if number, then treat it as a itemID
						local hasItem, key, value2, value3 = exports.titan_global:hasItem(thePlayer, tonumber(theItem[1]))
						if hasItem then
							...
							if theItem[2] then
								...
								--outputDebugString(tostring(exports['titan_items']:countItems(thePlayer, tonumber(theItem[1]), theItem[2])))
								if(tonumber(exports['titan_items']:countItems(thePlayer, tonumber(theItem[1]), theItem[2])) > 0) then
									countOr = countOr + 1
								...
								--if exports["titan_items"]:isBadge(theItem[1]) then
									--if(getElementData(thePlayer, badges[theItem[1]][1])) then
									--if exports["titan_items"]:isWearingBadge(thePlayer, theItem[1]) then
									--	countOr = countOr + 1
						...
						if textFunction == "PILOT" then --check for pilot licenses
							local pilotlicenses = exports['titan_mdc']:getPlayerPilotLicenses(thePlayer) or {}
							...
							if theItem[2] then --if a value is specified
								...
								if isNumeric(checkFaction[1]) then --if it is a number, we're checking for faction ID
									if isNumeric(checkFaction[2]) then --if faction rank is specified and is a number (valid)
										if exports.titan_factions:isPlayerInFaction(thePlayer, tonumber(checkFaction[1]), tonumber(checkFaction[2])) then
											countOr = countOr + 1
										end
									else
										if exports.titan_factions:isPlayerInFaction(thePlayer, tonumber(checkFaction[1])) then
											countOr = countOr + 1
										end
									end
									...
									local factionID = exports.titan_factions:getFactionIDFromName(checkFactionName)
									if factionID then
										if exports.titan_factions:isPlayerInFaction(thePlayer, factionID) then
											countOr = countOr + 1
										end
									end
								end
						...
						elseif textFunction == "FL" or textFunction == "FACTIONLEADER" then --check for faction leadership
							if theItem[2] then --if a value is specified
								...
								if isNumeric(checkFaction[1]) then --if it is a number, we're checking for faction ID
									if isNumeric(checkFaction[2]) then --if faction rank is specified and is a number (valid)
										local isMember, rank, isLeader = exports.titan_factions:isPlayerInFaction(thePlayer, tonumber(checkFaction[1]), tonumber(checkFaction[2]))
										if isMember and isLeader then
											countOr = countOr + 1
										end
									else
										local isMember, rank, isLeader = exports.titan_factions:isPlayerInFaction(thePlayer, tonumber(checkFaction[1]))
										if isMember and isLeader then
											countOr = countOr + 1
										end
									end
									...
									local factionID = exports.titan_factions:getFactionIDFromName(checkFactionName)
									if factionID then
										local isMember, rank, isLeader = exports.titan_factions:isPlayerInFaction(thePlayer, factionID)
										if isMember and isLeader then
											countOr = countOr + 1
										end
									end
	...
	elseif gateProtection == 9 then --If player has access to the given vehicle
		...
		for _, vehID in ipairs(tempAccess) do
			...
			if veh then
				if(exports.titan_global:isAdminOnDuty(thePlayer) or exports.titan_global:hasItem(thePlayer, 3, tonumber(vehID)) or (getElementData(veh, "faction") > 0 and exports.titan_factions:isPlayerInFaction(thePlayer, getElementData(veh, "faction"))) ) then
					return true
				end
			end
		end		
	...
	elseif gateProtection == 10 then --keycard
		...
		if(exports.titan_global:hasItem(thePlayer, keycardItemID, gateParameters["gateSecurityParameters"])) then
			return true
		end
	else
