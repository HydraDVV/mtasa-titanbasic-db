...
function startGUI(player)
    ...
    if (logged==1) then
        ...
        if factionType==2 or exports.titan_integration:isPlayerTrialAdmin(player) then
                            ...
                            dbQuery(
                                function(qh, client)
                                    ...
                                    if rows > 0 then
                                        ...
                                        exports.titan_logs:dbLog(client, 35, returnWhat(name, online), "Added to jail cell: "..row.cell.." character: "..row.charactername.." JailStamp: "..row.jail_time.." Charges: "..row.charges.." Fine: "..row.fine)
                                        if online then
                                            ...
                                            if tonumber(fine)>0 then -- Issue Fine
                                                ...
                                                exports.titan_anticheat:changeProtectedElementDataEx(name, "bankmoney", bankmoney-amount) -- Take from player
                                                dbExec(mysql:getConnection(), "UPDATE characters SET bankmoney=bankmoney-" .. (amount) .." WHERE charactername='".. (returnWhat(name, online)) .. "'")
                                                
                                                local tax = exports.titan_global:getTaxAmount() -- Split between gov and PD
                                                local theTeam = getPlayerTeam(client) 
                                                exports.titan_global:giveMoney(getTeamFromName("Government of Los Santos"), amount*tax)
                                            ...
                                            if restrainedObj then
                                                ...
                                                exports.titan_anticheat:changeProtectedElementDataEx(name, "restrain", 0, true)
                                                exports.titan_anticheat:changeProtectedElementDataEx(name, "restrainedBy", false, true)
                                                exports.titan_anticheat:changeProtectedElementDataEx(name, "restrainedObj", false, true)
                ...
                function(qh, client)
                    ...
                    if rows > 0 then
                        for index, row in ipairs(res) do
                            ...
                            table.insert(t, #t+1, { row.id, row.charid, row.charactername, row.jail_time, row.convictionDate, row.updatedBy, row.charges, row.cell, row.fine })
                            triggerClientEvent(client, "PrisonGUI:Refresh", client, t)
                            exports.titan_logs:dbLog(client, 35, returnWhat(name, online), "Updated prisoner data. New data: "..row.cell.." character: "..row.charactername.." JailStamp: "..row.jail_time.." Charges: "..row.charges.." Fine: "..row.fine)
...
function assignSkin(source)
    ...
    -- I put all the id's in g_prison for you.
        ...
        local items = exports['titan_items']:getItems( source ) -- [] [1] = itemID [2] = itemValue
...
function sendPrisonMsg(string)
    ...
    for _, v in ipairs(exports.titan_pool:getPoolElementsByType("player")) do
...
function processSpeakerMessage(thePlayer, commandName, ...)
    if not (...) then
        ...
        if (getElementInterior(thePlayer) == speakerInt and speakerDimensions[getElementDimension(thePlayer)] and exports.titan_factions:isPlayerInFaction(thePlayer, 59)) or (getDistanceBetweenPoints3D( px, py, pz, speakerOutX, speakerOutY, speakerOutZ ) < 100 and exports.titan_factions:isPlayerInFaction(thePlayer, 59)) then
            for k, v in ipairs(exports.titan_pool:getPoolElementsByType("player")) do
                ...
                if arrayInt == speakerInt and speakerDimensions[arrayDim] then -- speakerInt and speakerDimensions are set in g_ config file
                    if exports.titan_factions:isPlayerInFaction(v, 59) then
