...
function createHandlingWindow(mode)
	...
	if (exports.titan_integration:isPlayerVehicleConsultant(localPlayer) or exports.titan_integration:isPlayerAdmin(localPlayer)) then
	...
	elseif exports.titan_integration:isPlayerVCTMember(localPlayer) then
		if bReset and isElement(bReset) then
			guiSetEnabled(bReset, true)
		end
	else
...
function showDriveTestTimer(veh, driveTestTimeSec, thePed)
	...
	addEventHandler( "onClientGUIClick", GUIEditor_Button["finish"], function()
		if source == GUIEditor_Button["finish"] then
			...
			setTimer(function ()
				exports.titan_global:fadeFromBlack(source)
			end, 3000, 1)
		end
	end)
	if thePed then
	...
	setTimer(function()
		exports.titan_global:fadeFromBlack()
	end, 1000, 1)
end
