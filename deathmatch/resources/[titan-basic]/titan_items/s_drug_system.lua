function mixDrugs(drug1, drug2, drug1name, drug2name)
	...
	if (drugName == nil or drugID == nil) then
		outputChatBox("Error #1000 - Report on https:/www."..exports["titan_pool"]:getServerName().."roleplay.com/ucp", source, 255, 0, 0)
		return
	end
	
	exports.titan_global:takeItem(source, drug1)
	exports.titan_global:takeItem(source, drug2)
	local given = exports.titan_global:giveItem(source, drugID, 1)
	
	if (given) then
		outputChatBox("You mixed '" .. drug1name .. "' and '" .. drug2name .. "' to form '" .. drugName .. "'", source)
		exports.titan_global:sendLocalMeAction(source, "mixes some chemicals together.")
	else
		outputChatBox("You do not have enough space to mix these chemicals.", source, 255, 0, 0)
		exports.titan_global:giveItem(source, drug1, 1)
		exports.titan_global:giveItem(source, drug2, 1)
	end
end
...
function bagimlilikOraniKontrol(thePlayer)
	...
	exports["titan_bildirim"]:addNotification(thePlayer, "[ Uyuşturucu Bağımlılık Oranınız: %" .. tostring(bagimlilikOrani) .. "]", "error")	
end
