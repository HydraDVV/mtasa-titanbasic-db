...
addCommandHandler("itemprotect",
	function(command, f)
		...
		exports.titan_hud:sendBottomNotification(localPlayer, "Protect set to faction: " .. ( fp == -100 and "admins only" or tostring(fp)))
	end
)

local pickupTimer = nil -- Hanz
function pickupItem(button, state, item)
	if (button=="left") then
		local restrain = getElementData(localPlayer, "restrain")
		
		if (restrain) and (restrain==1) then
			exports.titan_hud:sendBottomNotification(localPlayer, "Pick up item failed", "You are cuffed.", 255, 0, 0)
		elseif getElementData(item, "itemID") > 0 and not hasSpaceForItem(localPlayer, getElementData(item, "itemID"), getElementData(item, "itemValue")) then
			exports.titan_hud:sendBottomNotification(localPlayer, "Pick up item failed", "Your Inventory is full. "..("%.2f/%.2f" ):format( getCarriedWeight( localPlayer ), getMaxWeight( localPlayer ) ).." kg(s)." , 255, 0, 0)
		elseif isElement(item) then
			...
			elseif getKeyState("n") then
				-- don't pick it up otherwise
				if exports.titan_global:isStaffOnDuty(localPlayer) or (getElementDimension(localPlayer) ~= 0 and hasItem(localPlayer, 4, getElementDimension(localPlayer)) or hasItem(localPlayer, 5, getElementDimension(localPlayer))) then
...
function copyClipboard()
	...
	if setClipboard(text) then
		exports.titan_hud:sendBottomNotification(localPlayer, "Copying", "Başarıyla '" .. text .. "' kopyalandı.")
	else
		exports.titan_hud:sendBottomNotification(localPlayer, "Copying", "Bir hata oluştu.")
	end
end
			...
			if (row==-1) or (col==-1) then
				...
				if not items[slot] then
					...
					if (desc=="A Dictionary.") then
						local res = getResourceFromName("titan_languages")
...
--function useItem(button)
	...
	if itemSlot > 0 then -- ITEMS
		...
		elseif (itemID==6) then -- radio
            outputChatBox(""..exports["titan_pool"]:getServerName()..":#f9f9f9 /tuneradio komutunu kullanmalısın.", 50, 50, 50, true)
            hideInventory()			
			return
		elseif (itemID==7) then -- phonebook
			exports.titan_hud:sendBottomNotification(localPlayer, "Item Usage", "Use /phonebook to use this item.", 255, 194, 14)
			return
		elseif (itemID==18) then -- City Guide
			triggerEvent( "showCityGuide", getLocalPlayer( ) )
			return
		elseif (itemID==19) then -- MP3 PLayer
			exports.titan_vehicle_radio:showMP3GUI()
			hideInventory()
			return
		elseif (itemID==248) then -- Flashlight Hanz
			exports["titan_billiard"]:billardTableShop(getLocalPlayer())	
			hideInventory()
			return				
		...
		elseif (itemID==30) or (itemID==31) or (itemID==32) or (itemID==33) then
			exports.titan_hud:sendBottomNotification(localPlayer, "Item Usage", "Use the chemistry set purchasable from 24/7 to use this item.", 255, 0, 0)
			return
		--[[
		...
		elseif (itemID==45) or (itemID==46) or (itemID==47) or (itemID==66) then
			exports.titan_hud:sendBottomNotification(source, "Item Usage", "Right click a player to use this item.")
			return
		elseif (itemID==50) or (itemID==51) or (itemID==52) then
			triggerServerEvent("useItem", localPlayer, itemSlot, additional)
			hideInventory()
		elseif (itemID==53) then -- Breathalizer
			exports.titan_hud:sendBottomNotification(localPlayer, "Item Usage", "Use /breathtest to use this item.")
			return
		...
		elseif (itemID==61) then -- Emergency Light Becon
			exports.titan_hud:sendBottomNotification(localPlayer, "Item Usage", "Put it in your car inventory and press 'P' to toggle it.")
			return
		...
		elseif (itemID==67) then -- GPS
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "Put it in your car inventory and Press 'F5'.")
			return
		...
		elseif (itemID==71) then -- Notebook
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "Use /writenote [text] to write a note. There are " .. itemValue .. " pages left.")
			return
		elseif (itemID==72) then -- Note
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "The Note reads: " .. itemValue)
		elseif (itemID==78) then
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "This San Andreas Pilot License was issued for " .. itemValue .. ".")
			return
		elseif (itemID==81) then
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "Drop this Fridge in an Interior.")
			return
		elseif (itemID==84) then
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "Put it in a car or carry it to know when police is around.")
			return
		elseif (itemID==85) then -- Emergency Light Becon
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "Put it in your car inventory and press 'N' to toggle it.")
			return
		...
		elseif (itemID==103) then
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "Drop this shelf in an interior.")
			return
		elseif (itemID==111) then
			exports.titan_hud:sendBottomNotification(localPlayer, itemName,  "A personal GPS, shiny. Got the latest maps installed." )
			return
		elseif itemID == 115 then
			local info = split(tostring(itemValue), ':')
			if info and info[1] == '46' then
				exports.titan_hud:sendBottomNotification(localPlayer, itemName, "Scroll to your parachute while in the air to use it.")
			else
				setClipboard(info[2])
				exports.titan_hud:sendBottomNotification(localPlayer, itemName, "Copied the weapon serial to your clipboard!")
				--exports.titan_hud:sendBottomNotification(localPlayer, itemName, "This item is (part of) a weapon, you cannot use it with clicking on it. If you have all the parts needed, the gun will appear in your scrollmenu.")
			end
			return
		elseif itemID == 116 then
			local parts = exports.titan_global:explode(':', itemValue)
			local ammo_id = tonumber(parts[1])
			local rounds = tonumber(parts[2])
			local weapons
			if ammo_id and rounds then
				local ammo = exports.titan_weapon:getAmmo( ammo_id )
				if ammo and ammo.weapons then
					exports.titan_hud:sendBottomNotification(localPlayer, ammo.cartridge..' '..itemName, "Cartridge: "..ammo.cartridge..", Bullet Style: "..ammo.bullet_style..", Bullet Weight: "
						..exports.titan_global:round(ammo.bullet_weight*1000,2).." grams, Application: "..ammo.application..", Compactible with "..exports.titan_weapon:formatWeaponNames(ammo.weapons)..".")
				end
			end
			return
		elseif itemID == 117 then
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "Please place this item into a vehicle, then control it over the vehicles right mouse button menu.")
			return
		elseif itemID == 174 then
			triggerServerEvent("startFAAmapGUI", localPlayer)
			return
		elseif (itemID==118) then
			exports.titan_hud:sendBottomNotification(localPlayer, itemName,  "Put it in your car and approach a toll booth.")
			return
		elseif itemID == 121 then
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "A heavy box full of supplies!")
			return
		elseif itemID == 145 then -- flashlight
			exports.titan_dynamic_lighting_flash:toggleFlashLight(nil, item[3])
		elseif (itemID==147) then -- Picture Frame --  Adams
			if(itemValue and itemValue ~= 1) then
				...
				if(#split > 1) then
					exports.titan_hud:sendBottomNotification(localPlayer, itemName, "Drop this in an interior to replace the texture '"..tostring(split[2]).."'.")
				else
					local url = itemValue
		...
		elseif (itemID==148) then
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "OCW permit - " .. itemValue .. ".")
			return
		elseif (itemID==149) then
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "CCW permit - " .. itemValue .. ".")
			return
		elseif (itemID==150) then -- ATM card
			local itemExploded = explode(";", itemValue ) 
			if tonumber( itemExploded[2] ) > 0 then
				exports.titan_hud:sendBottomNotification(localPlayer, g_items[itemID][1] , "Card Number: '"..itemExploded[1].."', Owner: '"..tostring(exports.titan_cache:getCharacterNameFromID(itemExploded[2])):gsub("_", " ").."'. Drag & Drop to a nearby ATM Machine.")
			else
				exports.titan_hud:sendBottomNotification(localPlayer, g_items[itemID][1] , "Card Number: '"..itemExploded[1].."', Owner: '"..tostring(exports.titan_cache:getBusinessNameFromID(math.abs(itemExploded[2]))).."'. Drag & Drop to a nearby ATM Machine.")
			end
			return
		elseif (itemID==152) then -- ID card
			...
			exports.titan_hud:sendBottomNotification(localPlayer, g_items[itemID][1] , text)
			return
		elseif (itemID==133) or (itemID==153) or (itemID==154) or (itemID==155)then
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "This "..itemName.." was issued for " .. itemValue .. ".")
			return
		elseif (itemID==169) then -- Keypad door lock
			exports.titan_hud:sendBottomNotification(localPlayer, g_items[itemID][1].." - Equipping for interior ID #"..itemValue , "Attach this device to a stable wall to install. You can uninstall this device and reinstall it freely later on.")
			return
		elseif (itemID==170) then -- Keycard
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "Swipe this card at "..tostring(itemValue).." to gain access.")
			return
		...
		elseif itemID == 232 then -- car battery charger
			for i, veh in pairs( exports.titan_global:getNearbyElements( localPlayer, 'vehicle', 5 ) ) do
				if veh and isElement(veh) then
					local doors = exports.titan_vehicle:getDoorsFor( getElementModel( veh ) , 0 )
					if #doors > 0 and doors[1] and doors[1][1] == 'Hood' and getVehicleDoorOpenRatio ( veh, 0 ) < 0.5 then
						exports.titan_hud:sendBottomNotification(localPlayer, itemName, "Please open the hood of this vehicle first." )
					elseif isVehicleLocked ( veh ) then
						exports.titan_hud:sendBottomNotification(localPlayer, itemName, "Please unlock this vehicle first." )
					else
			...
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "There is no vehicle around you." )
		else
			triggerServerEvent("useItem", localPlayer, itemSlot, additional)
		end
	else
		if itemSlot == -100 then
			exports.titan_hud:sendBottomNotification(localPlayer, itemName, "You wear Body Armor.")
		else
...
addCommandHandler("carried",
	function( command, detailed )
		exports.titan_hud:sendBottomNotification(localPlayer, "Carried Weight", ("%.2f/%.2f" ):format( getCarriedWeight( localPlayer ), getMaxWeight( localPlayer ) ) )
		if detailed == "detailed" then
			for k, v in ipairs( getItems( localPlayer ) ) do
				exports.titan_hud:sendBottomNotification(localPlayer, "Carried Weight",  "  %.2f %s; %s", getItemWeight( v[1], v[2] ), getItemName( v[1], v[2] ), getItemDescription( v[1], v[2] ) )
			end
		end
	end
)
...
function isWatchingTV()
	return exports['titan_freecam']:isFreecamEnabled()
end
