...
function changeWarnStyle(thePlayer, commandName)
	if exports.titan_integration:isPlayerTrialAdmin(thePlayer) then
		...
		local query = dbExec(exports.titan_mysql:getConnection(), "UPDATE accounts SET warn_style='".. warnStyle .."' WHERE id = '" .. dbid .. "'")
		if query then
			exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "wrn:style", warnStyle)
		...
		if (exports.titan_integration:isPlayerTrialAdmin(thePlayer)) then
			if showToOffDutyAdmins or (getElementData(thePlayer, "duty_admin") == 1) then
				if getElementData(thePlayer, "report_panel_mod") == "2" or getElementData(thePlayer, "report_panel_mod") == "3" then
					exports["titan_reports"]:showToAdminPanel(message, thePlayer, 255,0,0)
				else
...
function findPlayerByPartialNick(thePlayer, partialNick, fromBankSystem)
	...
	elseif tonumber(partialNick) then
		matchPlayer = exports.titan_pool:getElement("player", tonumber(partialNick))
		...
		if isElement( thePlayer ) then
			if #candidates == 0 then
				if not fromBankSystem then
					outputChatBox("[-]#f9f9f9 Böyle bir oyuncu bulunamadı.", thePlayer, 255, 0, 0,true)
				end
			else
...
function sendLocalText(root, message, r, g, b, distance, exclude, useFocusColors, ignoreDeaths, useHex)
	...
	if getElementType(root) == "player" and #affectedPlayers > 0 then -- TV SHOW!
		exports['titan_freecam']:add(affectedPlayers)
	end
...
function sendLocalDoAction(thePlayer, message, ignoreDeaths)
	...
	local state, affectedPlayers = sendLocalText(thePlayer, "* " .. message:gsub('"([^"]-)"', '#ffffff"%1#ffffff"#84cbad') .. "      ((" .. getPlayerName(thePlayer):gsub("_", " ") .. "))", 132, 203, 173, 30, {}, true, ignoreDeaths)
	return state, affectedPlayers
end
