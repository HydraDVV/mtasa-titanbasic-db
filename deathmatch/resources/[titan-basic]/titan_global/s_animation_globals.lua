function applyAnimation(thePlayer, block, name, animtime, loop, updatePosition, forced, taser)
	...
	if isElement(thePlayer) and getElementType(thePlayer)=="player" and not getPedOccupiedVehicle(thePlayer) and getElementData(thePlayer, "freeze") ~= 1 then
		...
		if (forced) then
			exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "forcedanimation", 1, false)
		else
			exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "forcedanimation", 0, false)
		end
		...
		if animtime > 50 then
			exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "animationt", setTimer(removeAnimation, animtime, 1, thePlayer), false)
		end
		return setanim
	else
		return false
...
function onSpawn()
	...
	exports.titan_anticheat:changeProtectedElementDataEx(source, "forcedanimation", 0, false)
...
function removeAnimation(thePlayer, tazer)
	if isElement(thePlayer) and getElementType(thePlayer)=="player" and getElementData(thePlayer, "freeze") ~= 1 and not getElementData(thePlayer, "injuriedanimation") and not getElementData(thePlayer, "superman:flying") then
		...
		exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "tazed", false, false)
		local setanim = setPedAnimation(thePlayer)
		exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "forcedanimation", 0, false)
		exports.titan_anticheat:changeProtectedElementDataEx(thePlayer, "animationt", 0, false)
