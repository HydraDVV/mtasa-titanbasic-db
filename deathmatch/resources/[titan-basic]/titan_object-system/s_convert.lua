                            ...
                            for index, result3 in ipairs(res) do
                                 if (result3) then
                                    ...
                                    if (result4) and (result5) then
                                        exports['titan_interiors']:realReloadInterior(tonumber(dimension))
...
function deleteInterior(dimensionID)
    ...
    exports['titan_interiors']:realReloadInterior(dimensionID)
    ...
    if result1 and result2 then
        exports.titan_global:sendMessageToAdmins( "[UCP] Custom interior processing for interior #"..tostring(dimensionID).." has completed!")
    else
        exports.titan_global:sendMessageToAdmins( "[UCP] Warning! Could not clean temp objects.")
        return false
    end
end
...
function saveInterior(dimensionID)
    ...
    if result then
        exports.titan_global:sendMessageToAdmins( "[UCP] Custom interior has been saved! Now reloading..")
        deleteInterior(dimensionID)
    else
        exports.titan_global:sendMessageToAdmins( "[UCP] Failed! Something went wrong. Error code: x0983CF")
    end
end
...
function testInterior(thePlayer, dimensionID)
    ...
    if (count > 0) then
        exports.titan_global:sendMessageToAdmins("[UCP] Loaded " .. count .. " interior custom objects to interior ID#".. dimensionID)
        dbQuery(
            function(qh)
                local res, rows, err = dbPoll(qh, 0)
                if rows > 0 then
                    for index, result in ipairs(res) do
                        if (result) then
                            ...
                            exports.titan_global:sendMessageToAdmins( "[UCP] Failed! Internal error code: x9889DF")
                            return false
                        end
                    end
                end
        ...
        exports.titan_global:sendMessageToAdmins( "[UCP] ERROR: No temporary objects found for interior ID#".. dimensionID)
        return false
    end
    return true
end
...
function processCustomInterior(thePlayer, commandName, dimensionID, playerUsername)
    if thePlayer and isElement(thePlayer) then
        if exports.titan_integration:isPlayerSeniorAdmin(thePlayer) or exports.titan_integration:isPlayerScripter(thePlayer) then
    ...
    if thePlayer and isElement(thePlayer) then
        ...
        exports.titan_global:sendMessageToAdmins("[UCP] Player "..playerUsername.." has uploaded a custom interior for property ID#"..dimensionID.."." )
    end
