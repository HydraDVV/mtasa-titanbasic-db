...
function createVehManagerWindow(data)
    if not exports.titan_integration:isPlayerTrialAdmin(localPlayer) and not exports.titan_integration:isPlayerVCTMember(localPlayer) and not exports.titan_integration:isPlayerScripter(localPlayer) then
    ...
    -- vehList1 = vehList
    gWin = guiCreateWindow(sx / 2 - 400, sy / 2 - 300, 800, 600, "Araç Yönetimi @ 2020 - "..exports["titan_pool"]:getServerName().."", false)
    ...
    local lTotal = guiCreateLabel(460, 25, 164, 17, "Standard Vehicles: " .. exports.titan_global:formatMoney(#standardVehs), false, gWin)
    local lActive = guiCreateLabel(460, 42, 164, 17, "Unique Vehicles: " .. exports.titan_global:formatMoney(#uniqueVehs), false, gWin)
    ...
    function fetchSearchResults(interiorsResultList)
        if interiorsResultList then
            ...
            for _, record in ipairs(interiorsResultList) do
                ...
                --guiGridListSetItemText(targetList[i], row, colOwner[i], exports.titan_cache:getCharacterNameFromID(getElementData(veh, "owner")) or "Unknown", false, false)
...
function loadTab(vehs, i)
    for index, veh in ipairs(vehs) do
        ...
        guiGridListSetItemText(targetList[i], row, colName[i], exports.titan_global:getVehicleName(veh), false, false)
...
function formatLastUsed(sec)
    if sec or not tonumber(sec) then
        ...
        return exports.titan_datetime:formatTimeInterval(sec).." ago"
    end
end
...
function createCheckVehWindow(result, adminTitle, history, notes1)
    ...
    if result[1][21] then
        ...
        false)
        guiSetEnabled(bRemoveVeh1, exports.titan_integration:isPlayerSeniorAdmin(localPlayer))
    ...
    function(button)
        if button == "left" then
            ...
            if noteMemo or(currentNote and currentNote.edit and currentNote.note ~= noteMemo) then
                ...
                exports.titan_global:playSoundSuccess()
                closeCheckVehWindow()
            else
