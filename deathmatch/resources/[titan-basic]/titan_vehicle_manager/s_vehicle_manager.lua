							...
							if rows > 0 then
								...
								dbQuery(
									function(qh, thePlayer)
										...
										triggerClientEvent(thePlayer, "createCheckVehWindow", thePlayer, result, exports.titan_global:getPlayerAdminTitle(thePlayer), result2, notes)
									end,
...
function getVehicleWeight (thePlayer, commandName, specifiedVehicle)
	if exports.titan_integration:isPlayerVehicleConsultant(thePlayer) or exports.titan_integration:isPlayerAdmin(thePlayer) or exports.titan_integration:isPlayerVCTMember(thePlayer) then
		...
		elseif tonumber(specifiedVehicle) and tonumber(specifiedVehicle) > 0 then
			local exists = exports.titan_pool:getElement("vehicle", tonumber(specifiedVehicle)) or false
			if not exists then
...
function systemDeleteVehicle(vehid, reason) --This function is meant to be used by the system or to be triggered from UCP remotely. Works similarly to the delveh however, it's simplier and doesn't use mta elements. / Hanz / 2020.7.17
	...
	call( getResourceFromName( "titan_items" ), "deleteAll", 3 , vehid )
	...
	dbQuery(
		function(qh)
			...
			if rows > 0 then
				...
				local theVehicle = exports.titan_pool:getElement("vehicle", vehid)
				if theVehicle then
