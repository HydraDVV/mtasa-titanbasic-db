...
function showFactionMenu(motd, memberUsernames, memberRanks, memberPerks, memberLeaders, memberOnline, memberLastLogin, --[[memberLocation,]] factionRanks, factionWages, factionTheTeam, note, fnote, vehicleIDs, vehicleModels, vehiclePlates, vehicleLocations, memberOnDuty, towstats, phone, membersPhone, fromShowF, factionID, birlikseviye,birlikonay)
	if (not isElement(gFactionWindow)) then
		...
		--[[
		birlikSeviyesiLbl = guiCreateLabel(0.83, 0.04, 0.16, 0.05, "Birlik Seviyesi: "..birlikseviye, true, gFactionWindow)
...
function fillFinance(factionID, bankThisWeek, bankPrevWeek, bankmoney, vehiclesvalue, propertiesvalue)
	...
	tabWeeklyStatement = guiCreateTab("Haftalık Açıklama", financeTabs)
		...
		guiGridListSetItemText(assetsGridlist, row, 2, "$"..tostring(exports.titan_global:formatMoney(bankmoney)), false, false)
		local row = guiGridListAddRow(assetsGridlist)
		guiGridListSetItemText(assetsGridlist, row, 1, "Araç", false, false)
		if not vehiclesvalue then vehiclesvalue = 0 end
		guiGridListSetItemText(assetsGridlist, row, 2, "$"..tostring(exports.titan_global:formatMoney(vehiclesvalue)), false, false)
		local row = guiGridListAddRow(assetsGridlist)
		guiGridListSetItemText(assetsGridlist, row, 1, "Mülker", false, false)
		if not propertiesvalue then propertiesvalue = 0 end
		guiGridListSetItemText(assetsGridlist, row, 2, "$"..tostring(exports.titan_global:formatMoney(propertiesvalue)), false, false)

		local row = guiGridListAddRow(assetsGridlist)
		guiGridListSetItemText(assetsGridlist, row, 1, "Toplam", false, false)
		guiGridListSetItemText(assetsGridlist, row, 2, "$"..tostring(exports.titan_global:formatMoney(bankmoney+vehiclesvalue+propertiesvalue)), false, false)
	...
	for k,v in ipairs(insertStatement) do
		...
		guiGridListSetItemText(weeklyStatementGridlist, row, statementColLast, "$"..tostring(exports.titan_global:formatMoney(lastWeek[rowVar])), false, false)
		guiGridListSetItemText(weeklyStatementGridlist, row, statementColThis, "$"..tostring(exports.titan_global:formatMoney(thisWeek[rowVar])), false, false)
		...
		elseif transTypes then
			...
			for k,v in ipairs(bankThisWeek) do
				if transTypes[v.type] then
					...
					guiGridListSetItemText(transactionDetailsGridlist, row, 5, tostring(exports.titan_global:formatMoney(v.amount)), false, false)
					if v.amount > 0 then
	...
	for k,v in ipairs(bankThisWeek) do
		...
		guiGridListSetItemText(transactionsGridlist, row, 5, tostring(exports.titan_global:formatMoney(v.amount)), false, false)
		if v.amount > 0 then
...
-- Made by Hanz for anadolu roleplay, 18 mayıs.
Duty = {
    gridlist = {},
    button = {},
    label = {}
	...
	if customEditID ~= 0 then
		...
		for k,v in pairs(customg[customEditID][5]) do
			if tonumber(v[2]) >= 0 then -- Items
				...
				guiGridListSetItemText(DutyCreate.gridlist[4], row, 1, exports["titan_items"]:getItemName(v[2]), false, false) -- Item Name
				...
				if tonumber(v[2]) == -100 then
					...
					guiGridListSetItemText(DutyCreate.gridlist[3], row, 1, exports["titan_items"]:getItemName(v[2]), false, false) -- Weapon Name
					guiGridListSetItemText(DutyCreate.gridlist[3], row, 2, tostring(v[3]), false, false) -- Ammo
					guiGridListSetItemData(DutyCreate.gridlist[3], row, 2, { v[1], tonumber(v[2]), v[3], v[4] })
				end
	...
	for k,v in pairs(allowList) do
		if tonumber(v[2]) >= 0 then -- Items
			if customEditID == 0 or (customEditID ~= 0 and not customg[customEditID][5][tostring(v[1])]) then
				...
				guiGridListSetItemText(DutyCreate.gridlist[2], row, 1, exports["titan_items"]:getItemName(v[2]), false, false)
				guiGridListSetItemText(DutyCreate.gridlist[2], row, 2, exports["titan_items"]:getItemDescription(v[2], v[3]), false, false)
			...
			if customEditID == 0 or (customEditID ~= 0 and not customg[customEditID][5][tostring(v[1])]) then
				...
				if tonumber(v[2]) == -100 then
					...
					guiGridListSetItemText(DutyCreate.gridlist[1], row, 1, exports["titan_items"]:getItemName(v[2]), false, false)
					guiGridListSetItemText(DutyCreate.gridlist[1], row, 2, v[3], false, false)
					guiGridListSetItemData(DutyCreate.gridlist[1], row, 1, { v[1], tonumber(v[2]), v[3] })
				end
			end
...
function addDutyItem()
	...
	if r >= 0 then
		...
		guiGridListSetItemText(DutyCreate.gridlist[4], row, 1, exports["titan_items"]:getItemName(info[2]), false, false) -- Item Name
	...
	if r >= 0 then
		...
		if red == 255 and g ~= 0 and b ~= 0 then
			...
			if tonumber(info[1]) == -100 then
				...
				guiGridListSetItemText(DutyCreate.gridlist[1], row, 1, exports["titan_items"]:getItemName(info[2]), false, false)
				guiGridListSetItemText(DutyCreate.gridlist[1], row, 2, tostring(info[4]), false, false)
				guiGridListSetItemData(DutyCreate.gridlist[1], row, 1, info)
			end
		end
...
function removeDutyItem()
	...
	if r >= 0 then
		...
		if red == 255 and g ~= 0 and b ~= 0 then
			...
			guiGridListSetItemText(DutyCreate.gridlist[2], row, 1, exports["titan_items"]:getItemName(tonumber(info[2])), false, false)
			guiGridListSetItemText(DutyCreate.gridlist[2], row, 2, exports["titan_items"]:getItemDescription(tonumber(info[2]), info[3]), false, false)
    ...
    addEventHandler("onClientGUIClick", DutyCreate.button[3], function()
		...
		if r >= 0 then
			...
			if tonumber(info[2]) == -100 then
				...
				guiGridListSetItemText(DutyCreate.gridlist[3], row, 1, exports["titan_items"]:getItemName(tonumber(info[2])), false, false) -- Weapon Name
				guiGridListSetItemData(DutyCreate.gridlist[3], row, 2, info)
				guiGridListSetItemText(DutyCreate.gridlist[3], row, 2, ammo, false, false) -- Ammo
			end
