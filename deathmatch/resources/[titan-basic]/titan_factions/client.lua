function openDutyWindow()
	...
	if #availablePackages > 0 then
		...
		outputChatBox("#822828"..exports["titan_pool"]:getServerName()..":#f9f9f9 Bu bölgede iş başı yapamazsın.", 255, 0, 0, true)
	end
end
...
function selectPackageGUI_open(availablePackages, allowList)
	...
	for pIndex, packageDetails in ipairs(availablePackages) do
		...
		for index, itemDetails in pairs(packageDetails[5]) do
			...
			gui[guiPrefix.."pushButton1"] = guiCreateButton(xAxis, yAxis, 71, 51, exports['titan_items']:getItemName(itemDetails[2]), false, gui[guiTabName])
		...
		for skinIndex, skinID in pairs(skinTable) do
			...
			gui[guiTabName.."-skin-"..skinID] = guiCreateStaticImage (xAxis, yAxis, 75, 75, ":titan_account/img/" .. skinImg..".png", false, gui[guiTabName] )
		...
		if btnAction then
			...
			elseif btnAction == "Go" then
				...
				if grantID then
					...
					--	outputChatBox("i: "..a[1].." (".. exports['titan_items']:getItemName(a[1]) ..") v:"..a[2])
					--end
