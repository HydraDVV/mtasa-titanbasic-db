	...
	if takeItemFromSlot(source, itemSlot) then
		if addVehicleUpgrade(vehicle, itemValue) then
			...
			if old_upgrade then
				...
				exports.titan_logs:dbLog(source, 6, { vehicle }, "MODDING AUTOREMOVEUPGRADE ".. itemValue .. " " .. getItemDescription(itemID, itemValue))
				if data[vehicle] then
					data[vehicle][old_upgrade] = nil
				end
			end
			exports.titan_logs:dbLog(source, 6, { vehicle }, "MODDING ADDUPGRADE ".. itemValue .. " " .. getItemDescription(itemID, itemValue))
			exports.titan_global:sendLocalMeAction(source, "adds " .. getItemDescription(itemID, itemValue) .. " to their " .. exports.titan_global:getVehicleName(vehicle).."("..getVehicleName(vehicle) .. ").")
			exports['titan_vehicle']:saveVehicleMods(vehicle)
			...
			if isElement(source) then
				exports.titan_anticheat:changeProtectedElementDataEx( source, "v:mods", data, true)
			else
				outputDebugString("AntiCheat Error on line 35", 2)
			end
		else
			outputChatBox("This upgrade is not compatible to this " .. exports.titan_global:getVehicleName(vehicle) .. ".", source, 255, 194, 14)
...
function moveUpgradeFromElement( upgrade )
	...
	if not data or not data[ source ] or not data[ source ][ upgrade ] then
		if not exports.titan_integration:isPlayerTrialAdmin( client ) then
			triggerClientEvent( client, "finishItemMove", client )
			return
		else
			admin = true
	...
	if hasSpaceForItem( client, 114, upgrade ) then
		if removeVehicleUpgrade( source, upgrade ) then
			...
			exports.titan_logs:dbLog(client, 6, { client, source }, "REMOVEUPGRADE ".. upgrade .. " " .. getVehicleUpgradeSlotName(upgrade) .. " | Only cause of being an admin = " .. tostring(admin))
			x_output_wrapper( source, client, 114, upgrade )
			triggerClientEvent( client, "forceElementMoveUpdate", client )
			
			exports['titan_vehicle']:saveVehicleMods(source)
		else
			...
			if data then
				...
				if not anyAtAll then
					if isElement(player) then
						exports.titan_anticheat:changeProtectedElementDataEx( player, "v:mods", data, true)
					else
