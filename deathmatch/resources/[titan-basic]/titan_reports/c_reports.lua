...
addEventHandler("reports.send",root,function(msg)
	for index, value in ipairs(aracTamiri) do
		if string.find(msg, value) then 
			if value then 
					outputChatBox("#575757"..exports["titan_pool"]:getServerName()..":#f9f9f9 Ayrıca /aracgetir yazarak aracınızı yanınıza çekebilirsiniz.", 255, 0, 0, true) 
				return false 
			end
		return end
	end
	for index, value in ipairs(kufurler) do
		if string.find(msg, value) then 
			if value then 
					outputChatBox("#575757"..exports["titan_pool"]:getServerName()..":#f9f9f9 Küfür etmene hiç gerek yok, kalbimi kırıyorsun :(", 255, 0, 0, true) 
				return false 
			end
		return end
	end	
	if msg == "" then outputChatBox("#575757"..exports["titan_pool"]:getServerName()..": #ffffffLütfen rapor içeriğini boş bırakma.", 245, 66, 66, true) return end
	if getElementData(localPlayer, "reportNum") then
		outputChatBox("#575757"..exports["titan_pool"]:getServerName()..": #ffffffZaten sırada beklemede olan bir rapor kaydın var. Raporunu sonlandırmak için ( /er )", 245, 66, 66, true)
	else
	
		outputChatBox("#575757"..exports["titan_pool"]:getServerName()..": #ffffffBaşarıyla rapor kaydın oluşturuldu, lütfen sabırla bekle.", 255, 194, 14, true)
...
function checkBinds()
	if ( exports.titan_integration:isPlayerTrialAdmin(getLocalPlayer()) or getElementData( getLocalPlayer(), "account:gmlevel" )  ) then
...
function toggleVehTheft()
	if exports.titan_integration:isPlayerTrialAdmin(getLocalPlayer()) then
		local status = getElementData(resourceRoot, "vehtheft")
		local numPdMembers = #getPlayersInTeam(exports.titan_factions:getTeamFromFactionID(1)) + #getPlayersInTeam(exports.titan_factions:getTeamFromFactionID(59)) --PD and HP
...
function showReportMainUI()
	...
	if (logged==1) then
		if (wReportMain==nil)  then
			reportedPlayer = nil
			wReportMain = guiCreateWindow(0.2, 0.2, 0.2, 0.25, ""..exports["titan_pool"]:getServerName().." Roleplay - Yardım Arayüzü", true)
			playSound("sound.mp3")
			scale(wReportMain)
