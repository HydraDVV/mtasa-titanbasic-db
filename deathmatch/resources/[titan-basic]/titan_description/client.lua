...
local fonte = exports.titan_fonts:getFont("RobotoB",12)
local fontElement = exports.titan_fonts:getFont("FontAwesome",8)
local noPlateVehs = {
	[481] = "BMX",
	[509] = "Bike",
	[510] = "Mountain Bike",
...
function showNearbyVehicleDescriptions()
	...
	if enableOverlayDescription ~= "0" and enableOverlayDescriptionVeh ~= "0" then
		...
		if not showing then
			for index, nearbyVehicle in ipairs( exports.titan_global:getNearbyElements(getLocalPlayer(), "vehicle") ) do
			...
			if getDistanceBetweenPoints3D(cx,cy,cz,x,y,z) <= viewDistance then --Within radius viewDistance
				...
				if px and isLineOfSightClear(cx, cy, cz, x, y, z, true, false, false, true, true, false, false) then				
					...
					if getElementData(theVehicle, "carshop") then
						...
						toBeShowed = toBeShowed.."Fiyat: "..exports.titan_global:formatMoney(price).."$\n Vergi: "..exports.titan_global:formatMoney(taxes) .. "$"
						lines = lines+ 1
					else
						...
						toBeShowed = toBeShowed.." Araç ID: "..vin
						lines = lines + 1
						

						--GET IMPOUND
						if (exports["titan_vehicle"]:isVehicleImpounded(theVehicle)) then
						...
						if vowner == getElementData(localPlayer, "account:id") or exports.titan_global:isStaffOnDuty(localPlayer) or exports.titan_integration:isPlayerScripter(localPlayer) or exports.titan_integration:isPlayerVCTMember(localPlayer) then
							toBeShowed = toBeShowed.."\n Liste ID: "..(getElementData(theVehicle, "vehicle_shop_id") or "None")
							lines = lines + 1

							local ownerName = 'Devlet'
							if vowner > 0 then
								ownerName = exports.titan_cache:getCharacterNameFromID(vowner)
							elseif vfaction > 0 then
								ownerName = exports.titan_cache:getFactionNameFromId(vfaction)
							end
...
--Activity / MAXIME
							...
							if vowner > 0 then 
								local protected, details = true, true, true--exports['titan_vehicle']:isProtected(theVehicle) 
					            if protected then
					                ...
					                local active, details2, secs = true, true, true --exports['titan_vehicle']:isActive(theVehicle)
					                if active and (powner == getElementData(localPlayer, "dbid") or exports.titan_integration:isPlayerStaff(localPlayer)) then
					...
					if not noPlateVehs[getElementModel(theVehicle)] then
						if not (getElementData(theVehicle, "show_plate") == 0) then
							...
							dxDrawImage(px - 70, py-115, 150, 35, "plaka.png", 0, 0, 0, tocolor(255, 255, 255, 255), false)
							dxDrawText(plate, px-70+5, py-200, px-90+175, py+10, tocolor(0, 0, 0, 255), 1, fonte, "center", "center", false, false, false, false, false)
						end
					end
					...
					if own == -2 then
						dxDrawText(getVehicleName(theVehicle), px + 5.5, py+marg-65, px + fontWidth + 5.5, (py + 2000), textColor2, 1, fontElement, "center")
					end
...
function showEditDescription()
	if not exports.titan_integration:isPlayerTrialAdmin(getLocalPlayer()) then
		return
	end
	if getPedOccupiedVehicle(getLocalPlayer()) then
		...
		if exports.titan_global:hasItem(getLocalPlayer(), 3, dbid) or exports.titan_global:hasItem(theVehicle, 3, dbid) or getElementData(getLocalPlayer(), "faction") == factionid or exports.titan_integration:isPlayerTrialAdmin(getLocalPlayer()) and getElementData(getLocalPlayer(), "duty_admin") == 1 then
			if dbid > 0 then
				...
				exports.titan_hud:sendBottomNotification(getLocalPlayer(), "Vehicle Description", "You cannot set descriptions on temporary vehicles.")
			end
		else
			exports.titan_hud:sendBottomNotification(getLocalPlayer(), "Vehicle Description", "You are not the owner of this vehicle.")
		end
	else
		exports.titan_hud:sendBottomNotification(getLocalPlayer(), "Vehicle Description", "You must be in the vehicle you wish to change the description of.")
	end
end
